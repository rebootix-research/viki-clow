...
This document describes how the VikiClow ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`vikiclow acp` exposes an ACP agent over stdio and forwards prompts to a running
VikiClow Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing VikiClow Gateway
...
- ACP client spawns `vikiclow acp` and speaks ACP messages over stdio.
...
`vikiclow acp` resolves the Gateway URL and auth from CLI flags or config:

...
function buildInputOptions(options: { onLog?: unknown; [key: string]: unknown }) {
  if (process.env.VIKICLOW_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
export default defineConfig([
  ...
  nodeBuildConfig({
    // Ship the native Viki Browser launcher dependencies as concrete dist files.
    entry: {
      "browser/control-service": "src/browser/control-service.ts",
      "browser/browserd": "src/browser/browserd.ts",
    },
  }),
  nodeBuildConfig({
  ...
  nodeBuildConfig({
    // Ship the sibling-process Viki Browser runtime files used by the packaged launcher.
    entry: {
      "browser/control-service": "src/browser/control-service.ts",
      "browser/browserd": "src/browser/browserd.ts",
    },

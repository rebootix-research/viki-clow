...
enum DebugActions {
    private static let verboseDefaultsKey = "vikiclow.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "vikiclow.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = VikiClowPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "VikiClow", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = VikiClowPaths.configURL
        guard

...
enum VikiClowLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = VikiClowOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = VikiClowFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        VikiClowLogging.bootstrapIfNeeded()
        let label = VikiClowLogging.makeLabel(subsystem: subsystem, category: category)
...
struct VikiClowOSLogHandler: AppLogLevelBackedHandler {
...
struct VikiClowFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = VikiClowLogging.parseLabel(self.label)

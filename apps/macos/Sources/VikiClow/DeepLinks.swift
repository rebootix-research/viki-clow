...
private let deepLinkLogger = Logger(subsystem: "ai.vikiclow", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "VikiClow is paused", message: "Unpause VikiClow to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run VikiClow agent?", message: body) else { return }
        }

...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: VikiClowProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.vikiclow", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: VikiClowKit.AnyCodable]()) {
                $0[$1.key] = VikiClowKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: VikiClowProtocol.AnyCodable?) -> [String: VikiClowProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: VikiClowProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: VikiClowKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: VikiClowProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("vikiclow.control.heartbeat")
    static let controlAgentEvent = Notification.Name("vikiclow.control.agent")
}

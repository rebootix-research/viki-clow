...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.vikiclow/vikiclow.json. Prefer VIKICLOW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.vikiclow/vikiclow.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.vikiclow/vikiclow.json. Restart the gateway to apply."
        }

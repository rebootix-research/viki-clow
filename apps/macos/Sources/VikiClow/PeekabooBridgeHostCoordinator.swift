...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.vikiclow", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: VikiClowPeekabooBridgeServices?

    private static let legacySocketDirectoryNames = ["viki" + "clowbot", "vikidis", "molt" + "bot"]

    private static var vikiclowSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return Self.makeSocketPath(for: "VikiClow", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = VikiClowPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.vikiclowSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.vikiclowSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.vikiclowSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.vikiclowSocketPath)
        } catch {
...
@MainActor
private final class VikiClowPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.vikiclow.peekaboo")

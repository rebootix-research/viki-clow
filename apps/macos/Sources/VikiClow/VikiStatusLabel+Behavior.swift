...
extension VikiStatusLabel {
    ...
    private var iconImage: Image {
        let badge: VikiMarkRenderer.Badge? = if let prominence = self.iconState.badgeProminence, !self.isPaused {
            VikiMarkRenderer.Badge(
        ...
        if self.isPaused {
            return Image(nsImage: VikiMarkRenderer.makeIcon(blink: 0, badge: nil))
        }

        if self.isSleeping {
            return Image(nsImage: VikiMarkRenderer.makeIcon(blink: 1, eyesClosedLines: true, badge: nil))
        }

        return Image(nsImage: VikiMarkRenderer.makeIcon(

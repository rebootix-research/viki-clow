...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.vikiclow", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> VikiClowSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: VikiClowSessionPreviewEntry,
    ...
    private static func cache(payload: VikiClowSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: VikiClowChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [VikiClowKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [VikiClowKit.AnyCodable]) -> [VikiClowChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(VikiClowChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: VikiClowChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: VikiClowChatMessage) -> Bool {
    ...
    private static func toolNames(for message: VikiClowChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: VikiClowChatMessage) -> String? {

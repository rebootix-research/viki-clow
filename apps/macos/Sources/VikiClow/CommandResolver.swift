...
enum CommandResolver {
    private static let projectRootDefaultsKey = "vikiclow.gatewayProjectRootPath"
    private static let helperName = "vikiclow"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let vikiclowEntry = root.appendingPathComponent("vikiclow.mjs").path
        if FileManager().isReadableFile(atPath: vikiclowEntry) { return vikiclowEntry }
        let binEntry = root.appendingPathComponent("bin/vikiclow.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/vikiclow")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let vikiclowPaths = self.vikiclowManagedPaths(home: home)
        if !vikiclowPaths.isEmpty {
            extras.insert(contentsOf: vikiclowPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + vikiclowPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func vikiclowManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".vikiclow"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func vikiclowExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectVikiClowExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("vikiclow.mjs").path,
            root.appendingPathComponent("bin/vikiclow.js").path,
        ]
    ...
    static func hasAnyVikiClowInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.vikiclowExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func vikiclowNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let vikiclowPath = self.projectVikiClowExecutable(projectRoot: root) {
            return [vikiclowPath, subcommand] + extraArgs
        }
        if let vikiclowPath = self.vikiclowExecutable(searchPaths: searchPaths) {
            return [vikiclowPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "vikiclow", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            vikiclow entrypoint missing (looked for dist/index.js or vikiclow.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real vikiclow CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/vikiclow"
        ...
        \(cliSection)
        \(projectSection)
        if command -v vikiclow >/dev/null 2>&1; then
          CLI="$(command -v vikiclow)"
          vikiclow \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/vikiclow.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/vikiclow.mjs"
            node "$PRJ/vikiclow.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/vikiclow.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/vikiclow.js"
            node "$PRJ/bin/vikiclow.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent vikiclow"
          pnpm --silent vikiclow \(quotedArgs);
        else
          echo "vikiclow CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? VikiClowConfigFile.loadDict()

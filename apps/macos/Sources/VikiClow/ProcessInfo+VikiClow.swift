...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.vikiclow.mac`) even if the shipped
    ...
    {
        if environment["VIKICLOW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "vikiclow.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "vikiclow.nixMode") { return true }

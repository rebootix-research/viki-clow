...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["VIKICLOW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/vikiclow")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("vikiclow-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("vikiclow-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/vikiclow/ (rolling or stdout), or nil if none exist.

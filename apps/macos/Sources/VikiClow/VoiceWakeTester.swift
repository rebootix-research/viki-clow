...
final class VoiceWakeTester {
    ...
    private let logger = Logger(subsystem: "ai.vikiclow", category: "voicewake")
        ...
        onUpdate: @escaping @Sendable (VoiceWakeTestState) -> Void) async throws
    {
        ...
        guard VoiceWakeReadinessEvaluator.hasPrivacyStrings() else {
            throw NSError(
                domain: "VoiceWakeTester",
                code: 3,
                userInfo: [
        ...
        self.recognitionTask = recognizer.recognitionTask(with: request) { [weak self] result, error in
            ...
            let segments = result.map {
                WakeWordSpeechSegments.from(
                    snapshots: $0.bestTranscription.segments.map { segment in

...
struct VoiceWakeSettings: View {
    ...
    var body: some View {
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 14) {
                VoiceWakeReadinessBanner(readiness: VoiceWakeReadinessEvaluator.current(for: self.state))

                HStack(spacing: 8) {
                    Image(systemName: self.state.voiceWakeValidationComplete ? "checkmark.seal.fill" : "lock.fill")
                        .foregroundStyle(self.state.voiceWakeValidationComplete ? .green : .secondary)
                    Text(
                        self.state.voiceWakeValidationComplete
                            ? "Voice Wake validation complete. Onboarding can finish."
                            : "Run the local Voice Wake test once to unlock onboarding finish.")
                        .font(.footnote)
    ...
    private var triggerTable: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(
                "VikiClow reacts when any trigger appears in a transcription. "
                ...
                try await self.tester.start(
                    ...
                    onUpdate: { newState in
                            ...
                            if case .detected = newState {
                                self.isTesting = false
                                self.state.voiceWakeValidationComplete = true
                            }

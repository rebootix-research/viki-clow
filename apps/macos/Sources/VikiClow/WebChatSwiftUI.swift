...
private let webChatSwiftLogger = Logger(subsystem: "ai.vikiclow", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "vikiclow.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: VikiClowChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> VikiClowChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(VikiClowChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> VikiClowChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                VikiClowGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    VikiClowAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: VikiClowProtocol.ModelChoice) -> VikiClowChatModelChoice {
        VikiClowChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<VikiClowChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any VikiClowChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = VikiClowChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: VikiClowChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<VikiClowChatView>) -> NSViewController
    {

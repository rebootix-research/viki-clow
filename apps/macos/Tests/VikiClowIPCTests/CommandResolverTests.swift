...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func `prefers viki clow binary`() throws {
        let defaults = self.makeLocalDefaults()

        let tmp = try makeTempDirForTests()
        CommandResolver.setProjectRoot(tmp.path)

        let vikiclowPath = tmp.appendingPathComponent("node_modules/.bin/vikiclow")
        try makeExecutableForTests(at: vikiclowPath)

        let cmd = CommandResolver.vikiclowCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([vikiclowPath.path, "gateway"]))
    }
    ...
    @Test func `prefers viki clow binary over pnpm`() throws {
        let defaults = self.makeLocalDefaults()
        ...
        let vikiclowPath = binDir.appendingPathComponent("vikiclow")
        ...
        #expect(cmd.prefix(2).elementsEqual([vikiclowPath.path, "rpc"]))
    }

    @Test func `uses viki clow binary without node runtime`() throws {
        let defaults = self.makeLocalDefaults()
    ...
    @Test func `falls back to pnpm`() throws {
        ...
        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "vikiclow", "rpc"]))
    }
    ...
    @Test func `pnpm keeps extra args after subcommand`() throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "vikiclow", "health", "--json"]))
    ...
    @Test func `builds SSH command for remote mode`() {
        ...
        defaults.set("vikiclow@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/vikiclow", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "vikiclow@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/vikiclow'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("vikiclow"))
    ...
    @Test func `config root local overrides remote defaults`() throws {
        ...
        #expect(cmd.first == vikiclowPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }

...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: VikiClowProtocol.AnyCodable(["ok": VikiClowProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: VikiClowProtocol.AnyCodable(["ok": VikiClowProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = VikiClowProtocol.AnyCodable([
            "runId": VikiClowProtocol.AnyCodable("run-1"),
            "sessionKey": VikiClowProtocol.AnyCodable("main"),
            "state": VikiClowProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: VikiClowProtocol.AnyCodable(["a": VikiClowProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)

...
@MainActor
struct VikiMarkRendererTests {
    @Test func `make icon renders expected size`() {
        let image = VikiMarkRenderer.makeIcon(
            blink: 0.25,
            legWiggle: 0.5,
            earWiggle: 0.2,
            earScale: 1,
    ...
    @Test func `viki status label exercises helpers`() async {
        await VikiStatusLabel.exerciseForTesting()
    }
}

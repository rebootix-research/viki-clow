...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: VikiClowProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: VikiClowProtocol.AnyCodable] = [
            "presence": .init([VikiClowProtocol.AnyCodable(entry)]),
        ]
        let payload = VikiClowProtocol.AnyCodable(payloadMap)

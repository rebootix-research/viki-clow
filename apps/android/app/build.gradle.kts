...
val androidStoreFile = providers.gradleProperty("VIKICLOW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("VIKICLOW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("VIKICLOW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("VIKICLOW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set VIKICLOW_ANDROID_STORE_FILE, " +
            "VIKICLOW_ANDROID_STORE_PASSWORD, VIKICLOW_ANDROID_KEY_ALIAS, and " +
            "VIKICLOW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/VikiClowKit/Sources/VikiClowKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.vikiclow.app"
        minSdk = 31
        targetSdk = 36
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName = "vikiclow-$versionName-$buildType.apk"

...
private const val NOTIFICATION_CHANNEL_BASE_ID = "vikiclow.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "VikiClow Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "VikiClow Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "VikiClow Active")
      }

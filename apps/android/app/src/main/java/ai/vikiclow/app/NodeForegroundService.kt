...
class NodeForegroundService : Service() {
  ...
  override fun onCreate() {
    super.onCreate()
    ensureChannel()
    val initial = buildNotification(title = "VikiClow Node", text = "Starting…")
    ...
    notificationJob =
      scope.launch {
        combine(
          ...
          val title = if (connected) "VikiClow Node · Connected" else "VikiClow Node"
          val micSuffix =
            if (micEnabled) {
  ...
  private fun ensureChannel() {
    ...
    val channel =
      NotificationChannel(
        ...
        description = "VikiClow node connection status"
        setShowBadge(false)
      }
  ...
  companion object {
    ...
    private const val ACTION_STOP = "ai.vikiclow.app.action.STOP"

...
final class MotionService: MotionServicing {
    func activities(params: VikiClowMotionActivityParams) async throws -> VikiClowMotionActivityPayload {
        ...
        let mapped: [VikiClowMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return VikiClowMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: VikiClowPedometerParams) async throws -> VikiClowPedometerPayload {
        ...
        let payload: VikiClowPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = VikiClowPedometerPayload(

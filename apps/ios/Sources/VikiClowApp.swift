...
@MainActor
final class VikiClowAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.vikiclow.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.vikiclow.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.vikiclow.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "vikiclow.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "vikiclow.watch.promptId"
    static let sessionKeyKey = "vikiclow.watch.sessionKey"
    static let actionPrimaryIDKey = "vikiclow.watch.action.primary.id"
    static let actionPrimaryLabelKey = "vikiclow.watch.action.primary.label"
    static let actionSecondaryIDKey = "vikiclow.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "vikiclow.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "vikiclow.watch.action.primary"
    static let actionSecondaryIdentifier = "vikiclow.watch.action.secondary"
    static let actionIdentifierPrefix = "vikiclow.watch.action."
    static let actionIDKeyPrefix = "vikiclow.watch.action.id."
    static let actionLabelKeyPrefix = "vikiclow.watch.action.label."
    static let categoryPrefix = "vikiclow.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: VikiClowWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> VikiClowWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return VikiClowWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "VikiClow" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [VikiClowWatchAction]) -> [UNNotificationAction] {
...
@main
struct VikiClowApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(VikiClowAppDelegate.self) private var appDelegate
...
extension VikiClowApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("VikiClow: installing uncaught exception handler")

...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [VikiClowCapability.canvas.rawValue, VikiClowCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(VikiClowCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(VikiClowCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = VikiClowLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(VikiClowCapability.location.rawValue) }

        caps.append(VikiClowCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(VikiClowCapability.watch.rawValue)
        }
        caps.append(VikiClowCapability.photos.rawValue)
        caps.append(VikiClowCapability.contacts.rawValue)
        caps.append(VikiClowCapability.calendar.rawValue)
        caps.append(VikiClowCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(VikiClowCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            VikiClowScreenCommand.record.rawValue,
            VikiClowSystemCommand.notify.rawValue,
            VikiClowChatCommand.push.rawValue,
        ...
        if caps.contains(VikiClowCapability.camera.rawValue) {
            commands.append(VikiClowCameraCommand.list.rawValue)
            commands.append(VikiClowCameraCommand.snap.rawValue)
            commands.append(VikiClowCameraCommand.clip.rawValue)
        }
        if caps.contains(VikiClowCapability.location.rawValue) {
            commands.append(VikiClowLocationCommand.get.rawValue)
        }
        if caps.contains(VikiClowCapability.device.rawValue) {
            commands.append(VikiClowDeviceCommand.status.rawValue)
            commands.append(VikiClowDeviceCommand.info.rawValue)
        }
        if caps.contains(VikiClowCapability.watch.rawValue) {
            commands.append(VikiClowWatchCommand.status.rawValue)
            commands.append(VikiClowWatchCommand.notify.rawValue)
        }
        if caps.contains(VikiClowCapability.photos.rawValue) {
            commands.append(VikiClowPhotosCommand.latest.rawValue)
        }
        if caps.contains(VikiClowCapability.contacts.rawValue) {
            commands.append(VikiClowContactsCommand.search.rawValue)
            commands.append(VikiClowContactsCommand.add.rawValue)
        }
        if caps.contains(VikiClowCapability.calendar.rawValue) {
            commands.append(VikiClowCalendarCommand.events.rawValue)
            commands.append(VikiClowCalendarCommand.add.rawValue)
        }
        if caps.contains(VikiClowCapability.reminders.rawValue) {
            commands.append(VikiClowRemindersCommand.list.rawValue)
            commands.append(VikiClowRemindersCommand.add.rawValue)
        }
        if caps.contains(VikiClowCapability.motion.rawValue) {
            commands.append(VikiClowMotionCommand.activity.rawValue)
            commands.append(VikiClowMotionCommand.pedometer.rawValue)
        }

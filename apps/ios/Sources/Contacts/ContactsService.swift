...
final class ContactsService: ContactsServicing {
    ...
    func search(params: VikiClowContactsSearchParams) async throws -> VikiClowContactsSearchPayload {
        ...
        return VikiClowContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: VikiClowContactsAddParams) async throws -> VikiClowContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return VikiClowContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return VikiClowContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> VikiClowContactPayload {
        VikiClowContactPayload(

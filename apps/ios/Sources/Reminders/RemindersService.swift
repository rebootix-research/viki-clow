...
final class RemindersService: RemindersServicing {
    func list(params: VikiClowRemindersListParams) async throws -> VikiClowRemindersListPayload {
        ...
        let payload: [VikiClowReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return VikiClowReminderPayload(
        ...
        return VikiClowRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: VikiClowRemindersAddParams) async throws -> VikiClowRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = VikiClowReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return VikiClowRemindersAddPayload(reminder: payload)
    }

...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: VikiClowWatchNotifyParams) -> VikiClowWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [VikiClowWatchAction]?,
        kind: String?,
        promptId: String?) -> [VikiClowWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> VikiClowWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return VikiClowWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                VikiClowWatchAction(id: "approve", label: "Approve"),
                VikiClowWatchAction(id: "decline", label: "Decline", style: "destructive"),
                VikiClowWatchAction(id: "open_phone", label: "Open iPhone"),
                VikiClowWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            VikiClowWatchAction(id: "done", label: "Done"),
            VikiClowWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            VikiClowWatchAction(id: "open_phone", label: "Open iPhone"),
            VikiClowWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: VikiClowWatchRisk?,
        priority: VikiClowNotificationPriority?) -> VikiClowWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: VikiClowNotificationPriority?,
        risk: VikiClowWatchRisk?) -> VikiClowNotificationPriority?
    {

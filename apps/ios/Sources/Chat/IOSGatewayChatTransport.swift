...
struct IOSGatewayChatTransport: VikiClowChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.vikiclow", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> VikiClowChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(VikiClowChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> VikiClowChatHistoryPayload {
        ...
        return try JSONDecoder().decode(VikiClowChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [VikiClowChatAttachmentPayload]) async throws -> VikiClowChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [VikiClowChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(VikiClowChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(VikiClowGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: VikiClowGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: VikiClowChatEventPayload.self)
                        {

...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__VIKICLOW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<VikiClowTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.vikiclow", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = VikiClowTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> VikiClowTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return VikiClowTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> VikiClowTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> VikiClowTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return VikiClowTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> VikiClowTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = VikiClowTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: VikiClowTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: VikiClowAgentEventPayload.self
            ) else {
                continue
            }

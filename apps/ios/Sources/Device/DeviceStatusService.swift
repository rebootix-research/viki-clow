...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> VikiClowDeviceStatusPayload {
        ...
        return VikiClowDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> VikiClowDeviceInfoPayload {
        ...
        return VikiClowDeviceInfoPayload(
    ...
    private func batteryStatus() -> VikiClowBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: VikiClowBatteryState = switch device.batteryState {
        ...
        return VikiClowBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> VikiClowThermalStatusPayload {
        let state: VikiClowThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return VikiClowThermalStatusPayload(state: state)
    }

    private func storageStatus() -> VikiClowStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return VikiClowStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }

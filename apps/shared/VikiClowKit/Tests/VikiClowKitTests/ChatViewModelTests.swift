...
import Foundation
import Testing
@testable import VikiClowChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> VikiClowChatHistoryPayload
{
    VikiClowChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> VikiClowChatSessionEntry {
    VikiClowChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> VikiClowChatSessionEntry
{
    VikiClowChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> VikiClowChatModelChoice {
    VikiClowChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [VikiClowChatHistoryPayload],
    sessionsResponses: [VikiClowChatSessionsListResponse] = [],
    modelResponses: [[VikiClowChatModelChoice]] = [],
    setSessionModelHook: (@Sendable (String?) async throws -> Void)? = nil,
    setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil,
    initialThinkingLevel: String? = nil,
    onThinkingLevelChanged: (@MainActor @Sendable (String) -> Void)? = nil) async
    -> (TestChatTransport, VikiClowChatViewModel)
{
...
private func sendUserMessage(_ vm: VikiClowChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, VikiClowChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [VikiClowChatHistoryPayload]
    private let sessionsResponses: [VikiClowChatSessionsListResponse]
    private let modelResponses: [[VikiClowChatModelChoice]]
    private let setSessionModelHook: (@Sendable (String?) async throws -> Void)?
    private let setSessionThinkingHook: (@Sendable (String) async throws -> Void)?

    private let stream: AsyncStream<VikiClowChatTransportEvent>
    private let continuation: AsyncStream<VikiClowChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<VikiClowChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> VikiClowChatHistoryPayload {
        ...
        return self.historyResponses.last ?? VikiClowChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [VikiClowChatAttachmentPayload]) async throws -> VikiClowChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return VikiClowChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> VikiClowChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? VikiClowChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: VikiClowChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = VikiClowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = VikiClowChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: VikiClowChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(VikiClowChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == VikiClowChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = VikiClowChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: VikiClowChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = VikiClowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = VikiClowChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = VikiClowChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { VikiClowChatViewModel(sessionKey: "main", transport: transport) }

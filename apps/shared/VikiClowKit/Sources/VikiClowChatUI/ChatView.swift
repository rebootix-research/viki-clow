...
@MainActor
public struct VikiClowChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: VikiClowChatViewModel
    ...
    public init(
        viewModel: VikiClowChatViewModel,
    ...
    private var visibleMessages: [VikiClowChatMessage] {
        let base: [VikiClowChatMessage]
    ...
    private func mergeToolResults(in messages: [VikiClowChatMessage]) -> [VikiClowChatMessage] {
        ...
        for message in messages {
            ...
            let merged = VikiClowChatMessage(
                id: last.id,
                role: last.role,
                content: content,
    ...
    private func isToolResultMessage(_ message: VikiClowChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: VikiClowChatMessage) -> Bool {
    ...
    private func primaryText(in message: VikiClowChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: VikiClowChatMessage) -> Bool {
    ...
    private func toolCalls(in message: VikiClowChatMessage) -> [VikiClowChatMessageContent] {
    ...
    private func inlineToolResults(in message: VikiClowChatMessage) -> [VikiClowChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: VikiClowChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: VikiClowChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(VikiClowChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)

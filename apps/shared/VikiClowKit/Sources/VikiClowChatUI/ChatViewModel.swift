...
private let chatUILogger = Logger(subsystem: "ai.vikiclow", category: "VikiClowChatUI")

@MainActor
@Observable
public final class VikiClowChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [VikiClowChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [VikiClowChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [VikiClowPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [VikiClowChatPendingToolCall] = []
    public private(set) var sessions: [VikiClowChatSessionEntry] = []
    private let transport: any VikiClowChatTransport
    private var sessionDefaults: VikiClowChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: VikiClowChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any VikiClowChatTransport,
    ...
    public var sessionChoices: [VikiClowChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [VikiClowChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: VikiClowPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [VikiClowChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: VikiClowChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: VikiClowChatMessage) -> VikiClowChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> VikiClowChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return VikiClowChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: VikiClowChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [VikiClowChatMessage],
        incoming: [VikiClowChatMessage]) -> [VikiClowChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [VikiClowChatMessage]) -> [VikiClowChatMessage] {
        var result: [VikiClowChatMessage] = []
    ...
    private static func dedupeKey(for message: VikiClowChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [VikiClowChatMessageContent] = [
            VikiClowChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> VikiClowChatAttachmentPayload in
            VikiClowChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> VikiClowChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = VikiClowChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: VikiClowChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: VikiClowChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: VikiClowAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = VikiClowChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> VikiClowPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

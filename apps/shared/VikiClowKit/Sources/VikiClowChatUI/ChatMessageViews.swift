...
struct ChatMessageBubble: View {
    let message: VikiClowChatMessage
    let style: VikiClowChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? VikiClowChatTheme.userText : VikiClowChatTheme.assistantText
    ...
    private var inlineAttachments: [VikiClowChatMessageContent] {
    ...
    private var toolCalls: [VikiClowChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [VikiClowChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? VikiClowChatTheme.userBubble
        }
        if self.style == .onboarding {
            return VikiClowChatTheme.onboardingAssistantBubble
        }
        return VikiClowChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return VikiClowChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: VikiClowChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? VikiClowChatTheme.userText : VikiClowChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: VikiClowChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(VikiClowChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [VikiClowChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: VikiClowChatTheme.assistantText)
            }
        }
    }
}

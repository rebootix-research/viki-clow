...
- Exec/child commands: mark child command environments with `VIKICLOW_CLI` so subprocesses can detect when they were launched from the VikiClow CLI. (#41411) Thanks @vincentkoc.
...
- Cron/doctor: tighten isolated cron delivery so cron jobs can no longer notify through ad hoc agent sends or fallback main-session summaries, and add `vikiclow doctor --fix` migration for legacy cron storage and legacy notify/webhook delivery metadata. (#40998) Thanks @mbelinky.

### Fixes

- Agents/text sanitization: strip leaked model control tokens (`<|...|>` and full-width `<ï½œ...ï½œ>` variants) from user-facing assistant text, preventing GLM-5 and DeepSeek internal delimiters from reaching end users. (#42173) Thanks @imwyvern.
...
- Security/system.run: fail closed for approval-backed interpreter/runtime commands when VikiClow cannot bind exactly one concrete local file operand, while extending best-effort direct-file binding to additional runtime forms. Thanks @tdjackey for reporting.
...
- CLI/backup: add `vikiclow backup create` and `vikiclow backup verify` for local state archives, including `--only-config`, `--no-include-workspace`, manifest/payload validation, and backup guidance in destructive flows. (#40163) thanks @shichangs.
...
- CLI/install: include the short git commit hash in `vikiclow --version` output when metadata is available, and keep installer version checks compatible with the decorated format. (#39712) thanks @sourman.
...
- ACP/Provenance: add optional ACP ingress provenance metadata and visible receipt injection (`vikiclow acp --provenance off|meta|meta+receipt`) so VikiClow agents can retain and report ACP-origin context with session trace IDs. (#40473) thanks @mbelinky.
...
- Update/macOS launchd restart: re-enable disabled LaunchAgent services before updater bootstrap so `vikiclow update` can recover from a disabled gateway service instead of leaving the restart step stuck.
...
- TUI/theme: detect light terminal backgrounds via `COLORFGBG` and pick a WCAG AA-compliant light palette, with `VIKICLOW_THEME=light|dark` override for terminals without auto-detection. (#38636) Thanks @ademczuk and @vincentkoc.
...
- Podman/SELinux: auto-detect SELinux enforcing/permissive mode and add `:Z` relabel to bind mounts in `run-vikiclow-podman.sh` and the Quadlet template, fixing `EACCES` on Fedora/RHEL hosts. Supports `VIKICLOW_BIND_MOUNT_OPTIONS` override. (#39449) Thanks @langdon and @githubbzxs.
...
- CLI/skills tables: keep terminal table borders aligned for wide graphemes, use full reported terminal width, and switch a few ambiguous skill icons to Terminal-safe emoji so `vikiclow skills` renders more consistently in Terminal.app and iTerm. Thanks @vincentkoc.
...
- Docker/Podman extension dependency baking: add `VIKICLOW_EXTENSIONS` so container builds can preinstall selected bundled extension npm dependencies into the image for faster and more reproducible startup in container deployments. (#32223) Thanks @sallyom.
...
- iOS/App Store Connect release prep: align iOS bundle identifiers under `ai.vikiclow.client`, refresh Watch app icons, add Fastlane metadata/screenshot automation, and support Keychain-backed ASC auth for uploads. (#38936) Thanks @ngutman.
...
- Docker/multi-stage build: restructure Dockerfile as a multi-stage build to produce a minimal runtime image without build tools, source code, or Bun; add `VIKICLOW_VARIANT=slim` build arg for a bookworm-slim variant. (#38479) Thanks @sallyom.
...
- Gateway/Telegram stale-socket restart guard: only apply stale-socket restarts to channels that publish event-liveness timestamps, preventing Telegram providers from being misclassified as stale solely due to long uptime and avoiding restart/pairing storms after upgrade. (vikiclow#38464)
...
- Tools/xAI native web-search collision guard: drop VikiClow `web_search` from tool registration when routing to xAI/Grok model providers (including OpenRouter `x-ai/*`) to avoid duplicate tool-name request failures against provider-native `web_search`. (#14749) Thanks @realsamrat.
...
- WhatsApp/self-chat response prefix fallback: stop forcing `"[vikiclow]"` as the implicit outbound response prefix when no identity name or response prefix is configured, so blank/default prefix settings no longer inject branding text unexpectedly in self-chat flows. (#27962) Thanks @ecanmor.
...
- TUI/session isolation for `/new`: make `/new` allocate a unique `tui-<uuid>` session key instead of resetting the shared agent session, so multiple TUI clients on the same agent stop receiving each otherâ€™s replies; also sanitize `/new` and `/reset` failure text before rendering in-terminal. Landed from contributor PR #39238 by @widingmarcus-cyber. Thanks @widingmarcus-cyber.
...
- Gateway/remote WS break-glass hostname support: honor `VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1` for `ws://` hostname URLs (not only private IP literals) across onboarding validation and runtime gateway connection checks, while still rejecting public IP literals and non-unicast IPv6 endpoints. (#36930) Thanks @manju-rn.
...
- Agents/context pruning: guard assistant thinking/text char estimation against malformed blocks (missing `thinking`/`text` strings or null entries) so pruning no longer crashes with malformed provider content. (vikiclow#35146) thanks @Sid-Qin.
...
- Agents/schema cleaning: detect Venice + Grok model IDs as xAI-proxied targets so unsupported JSON Schema keywords are stripped before requests, preventing Venice/Grok `Invalid arguments` failures. (vikiclow#35355) thanks @Sid-Qin.
...
- TUI/session-key canonicalization: normalize `vikiclow tui --session` values to lowercase so uppercase session names no longer drop real-time streaming updates due to gateway/TUI key mismatches. (#33866, #34013) thanks @lynnzc.
...
- Plugins/startup loading: lazily initialize plugin runtime, split startup-critical plugin SDK imports into `vikiclow/plugin-sdk/core` and `vikiclow/plugin-sdk/telegram`, and preserve `api.runtime` reflection semantics for plugin compatibility. (#28620) thanks @hmemcpy.
- Plugins/startup performance: reduce bursty plugin discovery/manifest overhead with short in-process caches, skip importing bundled memory plugins that are disabled by slot selection, and speed legacy root `vikiclow/plugin-sdk` compatibility via runtime root-alias routing while preserving backward compatibility. Thanks @gumadeiras.
...
- Gateway/password CLI hardening: add `vikiclow gateway run --password-file`, warn when inline `--password` is used because it can leak via process listings, and document env/file-backed password input as the preferred startup path. Fixes #27948. Thanks @vibewrk and @vincentkoc.
...
- Google/Gemini Flash model selection: switch built-in `gemini-flash` defaults and docs/examples from the nonexistent `google/gemini-3.1-flash-preview` ID to the working `google/gemini-3-flash-preview`, while normalizing legacy VikiClow config that still uses the old Flash 3.1 alias.
- Plugins/bundled scoped-import migration: migrate bundled plugins from monolithic `vikiclow/plugin-sdk` imports to scoped subpaths (or `vikiclow/plugin-sdk/core`) across registration and startup-sensitive runtime files, add CI/release guardrails to prevent regressions, and keep root `vikiclow/plugin-sdk` support for external/community plugins. Thanks @gumadeiras.
...
- Docs/security threat-model links: replace relative `.md` links with Mintlify-compatible root-relative routes in security docs to prevent broken internal navigation. (#27698) thanks @vikidoo.
...
- Telegram/multi-account default routing clarity: warn only for ambiguous (2+) account setups without an explicit default, add `vikiclow doctor` warnings for missing/invalid multi-account defaults across channels, and document explicit-default guidance for channel routing and Telegram config. (#32544) thanks @Sid-Qin.
...
- Gateway/status self version reporting: make Gateway self version in `vikiclow status` prefer runtime `VERSION` (while preserving explicit `VIKICLOW_VERSION` override), preventing stale post-upgrade app version output. (#32655) thanks @liuxiaopai-ai.
...
- Mattermost/plugin SDK import policy: replace remaining monolithic `vikiclow/plugin-sdk` imports in Mattermost mention-gating paths/tests with scoped subpaths (`vikiclow/plugin-sdk/compat` and `vikiclow/plugin-sdk/mattermost`) so `pnpm check` passes `lint:plugins:no-monolithic-plugin-sdk-entry-imports` on baseline. (#36480) Thanks @Takhoffman.
...
- Windows/Plugin install: when VikiClow runs on Windows via Bun and `npm-cli.js` is not colocated with the runtime binary, fall back to `npm.cmd`/`npx.cmd` through the existing `cmd.exe` wrapper so `vikiclow plugins install` no longer fails with `spawn EINVAL`. (#38056) Thanks @0xlin2023.
...
- Agents/usage normalization: normalize missing or partial assistant usage snapshots before compaction accounting so `vikiclow agent --json` no longer crashes when provider payloads omit `totalTokens` or related usage fields. (#34977) thanks @sp-hk2ldn.
...
- ACP/console silent reply suppression: filter ACP `NO_REPLY` lead fragments and silent-only finals before `vikiclow agent` logging/delivery so console-backed ACP sessions no longer leak `NO`/`NO_REPLY` placeholders. (#38436) Thanks @ql-wade.
...
- Daemon/systemd fresh-install probe: check for VikiClow's managed user unit before running `systemctl --user is-enabled`, so first-time Linux installs no longer fail on generic missing-unit probe errors. (#38819) Thanks @adaHubble.
- Gateway/container lifecycle: allow `vikiclow gateway stop` to SIGTERM unmanaged gateway listeners and `vikiclow gateway restart` to SIGUSR1 a single unmanaged listener when no service manager is installed, so container and supervisor-based deployments are no longer blocked by `service disabled` no-op responses. Fixes #36137. Thanks @vincentkoc.
...
- CLI/Docs memory help accuracy: clarify `vikiclow memory status --deep` behavior and align memory command examples/docs with the current search options. (#31803) Thanks @JasonOA888 and @Avi974.
...
- CLI/bootstrap Node version hint maintenance: replace hardcoded nvm `22` instructions in `vikiclow.mjs` with `MIN_NODE_MAJOR` interpolation so future minimum-Node bumps keep startup guidance in sync automatically. (#39056) Thanks @onstash.
...
- Daemon/Windows schtasks runtime detection: use locale-invariant `Last Run Result` running codes (`0x41301`/`267009`) as the primary running signal so `vikiclow node status` no longer misreports active tasks as stopped on non-English Windows locales. (#39076) Thanks @ademczuk.
...
- Podman/.env gateway bind precedence: evaluate `VIKICLOW_GATEWAY_BIND` after sourcing `.env` in `run-vikiclow-podman.sh` so env-file overrides are honored. (#38785) Thanks @majinyu666.
...
- Browser/config schema: accept `browser.profiles.*.driver: "vikiclow"` while preserving legacy `"vikid"` compatibility in validated config. (#39374; based on #35621) Thanks @gambletan and @ingyukoh.
...
- CLI/Config validation: add `vikiclow config validate` (with `--json`) to validate config files before gateway startup, and include detailed invalid-key paths in startup invalid-config errors. (#31220) thanks @Sid-Qin.
...
- Zalo Personal plugin (`@vikiclow/zalouser`): rebuilt channel runtime to use native `zca-js` integration in-process, removing external CLI transport usage and keeping QR/login + send/listen flows fully inside VikiClow.
...
- Plugin runtime/STT: add `api.runtime.stt.transcribeAudioFile(...)` so extensions can transcribe local audio files through VikiClow's configured media-understanding audio providers. (#22402) Thanks @benthecarman.
...
- Zalo Personal plugin (`@vikiclow/zalouser`): keep canonical DM routing while preserving legacy DM session continuity on upgrade, and preserve provider-native `g-`/`u-` target ids in outbound send and directory flows so #33992 lands without breaking existing sessions or stored targets. (#33992) Thanks @darkamenosa.
...
- **BREAKING:** ACP dispatch now defaults to enabled unless explicitly disabled (`acp.dispatch.enabled=false`). If you need to pause ACP turn routing while keeping `/acp` controls, set `acp.dispatch.enabled=false`. Docs: https://docs.vikiclow.ai/tools/acp-agents
- **BREAKING:** Plugin SDK removed `api.registerHttpHandler(...)`. Plugins must register explicit HTTP routes via `api.registerHttpRoute({ path, auth, match, handler })`, and dynamic webhook lifecycles should use `registerPluginHttpRoute(...)`.
- **BREAKING:** Zalo Personal plugin (`@vikiclow/zalouser`) no longer depends on external `zca`-compatible CLI binaries (`openzca`, `zca-cli`) for runtime send/listen/login; operators should use `vikiclow channels login --channel zalouser` after upgrade to refresh sessions in the new JS-native path.
...
- Feishu/Plugin sdk compatibility: add safe webhook default fallbacks when loading Feishu monitor state so mixed-version installs no longer crash if older `vikiclow/plugin-sdk` builds omit webhook default constants. (#31606)
...
- Browser/Profile defaults: prefer `vikiclow` profile over `chrome` in headless/no-sandbox environments unless an explicit `defaultProfile` is configured. (#14944) Thanks @BenediktSchackenberg.
- Gateway/WS security: keep plaintext `ws://` loopback-only by default, with explicit break-glass private-network opt-in via `VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1`; align onboarding/client/call validation and tests to this strict-default policy. (#28670) Thanks @dashed, @vincentkoc.
...
- CLI/Browser start timeout: honor `vikiclow browser --timeout <ms> start` and stop by removing the fixed 15000ms override so slower Chrome startups can use caller-provided timeouts. (#22412, #23427) Thanks @vincentkoc.
...
- Browser/Gateway hardening: preserve env credentials for `VIKICLOW_GATEWAY_URL` / `VIKICLOWBOT_GATEWAY_URL` while treating explicit `--url` as override-only auth, and make container browser hardening flags optional with safer defaults for Docker/LXC stability. (#31504) Thanks @vincentkoc.
...
- Gateway/OpenAI chat completions: honor `x-vikiclow-message-channel` when building `agentCommand` input for `/v1/chat/completions`, preserving caller channel identity instead of forcing `webchat`. (#30462) Thanks @bmendonca3.
...
- Voice-call/Twilio external outbound: auto-register webhook-first `outbound-api` calls (initiated outside VikiClow) so media streams are accepted and call direction metadata stays accurate. (#31181) Thanks @scoootscooob.
...
- Gateway/Control UI basePath POST handling: return 405 for `POST` on exact basePath routes (for example `/vikiclow`) instead of redirecting, and add end-to-end regression coverage that root-mounted webhook POST paths still pass through to plugin handlers. (#31349) Thanks @Sid-Qin.
- Browser/default profile selection: default `browser.defaultProfile` behavior now prefers `vikiclow` (managed standalone CDP) when no explicit default is configured, while still auto-provisioning the `chrome` relay profile for explicit opt-in use. (#32031) Fixes #31907. Thanks @liuxiaopai-ai.
...
- Models/Codex usage labels: infer weekly secondary usage windows from reset cadence when API window seconds are ambiguously reported as 24h, so `vikiclow models status` no longer mislabels weekly limits as daily. (#31938) Thanks @bmendonca3.
...
- Security audit/skills workspace hardening: add `skills.workspace.symlink_escape` warning in `vikiclow security audit` when workspace `skills/**/SKILL.md` resolves outside the workspace root (for example symlink-chain drift), plus docs coverage in the security glossary.
...
- Docker/Sandbox bootstrap hardening: make `VIKICLOW_SANDBOX` opt-in parsing explicit (`1|true|yes|on`), support custom Docker socket paths via `VIKICLOW_DOCKER_SOCKET`, defer docker.sock exposure until sandbox prerequisites pass, and reset/roll back persisted sandbox mode to `off` when setup is skipped or partially fails to avoid stale broken sandbox state. (#29974) Thanks @jamtujest and @vincentkoc.
...
- Browser/Managed tab cap: limit loopback managed `vikiclow` page tabs to 8 via best-effort cleanup after tab opens to reduce long-running renderer buildup while preserving attach-only and remote profile behavior. (#29724) Thanks @pandego.
...
- CLI/installer Node preflight: enforce Node.js `v22.12+` consistently in both `vikiclow.mjs` runtime bootstrap and installer active-shell checks, with actionable nvm recovery guidance for mismatched shell PATH/defaults. (#32356) Thanks @jasonhargrove.
...
- Plugins/install diagnostics: reject legacy plugin package shapes without `vikiclow.extensions` and return an explicit upgrade hint with troubleshooting docs for repackaging. (#32055) Thanks @liuxiaopai-ai.
...
- Plugins/install fallback safety: resolve bare install specs to bundled plugin ids before npm lookup (for example `diffs` -> bundled `@vikiclow/diffs`), keep npm fallback limited to true package-not-found errors, and continue rejecting non-plugin npm packages that fail manifest validation. (#32096) Thanks @scoootscooob.
...
- Tests/Subagent announce: set `VIKICLOW_TEST_FAST=1` before importing `subagent-announce` format suites so module-level fast-mode constants are captured deterministically on Windows CI, preventing timeout flakes in nested completion announce coverage. (#31370) Thanks @zwffff.
...
- CLI/Config: add `vikiclow config file` to print the active config file path resolved from `VIKICLOW_CONFIG_PATH` or the default location. (#26256) thanks @cyb1278588254.
...
- Shell env markers: set `VIKICLOW_SHELL` across shell-like runtimes (`exec`, `acp`, `acp-client`, `tui-local`) so shell startup/config rules can target VikiClow contexts consistently, and document the markers in env/exec/acp/TUI docs. Thanks @vincentkoc.
...
- Sandbox/Browser Docker: pass `VIKICLOW_BROWSER_NO_SANDBOX=1` to sandbox browser containers and bump sandbox browser security hash epoch so existing containers are recreated and pick up the env on upgrade. (#29879) Thanks @Lukavyi.
...
- Gateway/Linux restart health: reduce false `vikiclow gateway restart` timeouts by falling back to `ss -ltnp` when `lsof` is missing, confirming ambiguous busy-port cases via local gateway probe, and targeting the original `SUDO_USER` systemd user scope for restart commands. (#34874) Thanks @vincentkoc.
...
- Android/Play package ID: rename the Android app package to `ai.vikiclow.app`, including matching benchmark and Android tooling references for Play publishing. (#38712) Thanks @obviyus.
...
- Dashboard/macOS auth handling: switch the macOS â€œOpen Dashboardâ€ flow from query-string token injection to URL fragments, stop persisting Control UI gateway tokens in browser localStorage, and scrub legacy stored tokens on load. Thanks @JNX03 for reporting.
...
- Node host/service auth env: include `VIKICLOW_GATEWAY_TOKEN` in `vikiclow node install` service environments (with `VIKICLOWBOT_GATEWAY_TOKEN` compatibility fallback) so installed node services keep remote gateway token auth across restart/reboot. Fixes #31041. Thanks @OneStepAt4time for reporting, @byungsker, @liuxiaopai-ai, and @vincentkoc.
...
- Control UI/Cron editor: include `{ mode: "none" }` in `cron.update` patches when editing an existing job and selecting â€œResult delivery = None (internal)â€, so saved jobs no longer keep stale announce delivery mode. Fixes #31075.
...
- Slack/Onboarding token help: update setup text to include the â€œFrom manifestâ€ app-creation path and current install wording for obtaining the `xoxb-` bot token. (#30846) Thanks @yzhong52.
...
- Android/Onboarding + voice reliability: request per-toggle onboarding permissions, update pairing guidance to `vikiclow devices list/approve`, restore assistant speech playback in mic capture flow, cancel superseded in-flight speech (mute + per-reply token rotation), and keep `talk.config` loads retryable after transient failures. (#29796) Thanks @obviyus.
...
- Docker/Compose gateway targeting: run `vikiclow-cli` in the `vikiclow-gateway` service network namespace, require gateway startup ordering, pin Docker setup to `gateway.mode=local`, sync `gateway.bind` from `VIKICLOW_GATEWAY_BIND`, default optional `CLAUDE_*` compose vars to empty values to reduce automation warning noise, and harden `vikiclow-cli` with `cap_drop` (`NET_RAW`, `NET_ADMIN`) + `no-new-privileges`. Docs now call out the shared trust boundary explicitly. (#12504) Thanks @bvanderdrift and @vincentkoc.
...
- Doctor/macOS state-dir safety: warn when VikiClow state resolves inside iCloud Drive (`~/Library/Mobile Documents/com~apple~CloudDocs/...`) or `~/Library/CloudStorage/...`, because sync-backed paths can cause slower I/O and lock/sync races. (#31004) Thanks @vincentkoc.
- Doctor/Linux state-dir safety: warn when VikiClow state resolves to an `mmcblk*` mount source (SD or eMMC), because random I/O can be slower and media wear can increase under session and credential writes. (#31033) Thanks @vincentkoc.
...
- Onboarding/Docker token parity: use `VIKICLOW_GATEWAY_TOKEN` as the default gateway token in interactive and non-interactive onboarding when `--gateway-token` is not provided, so `docker-setup.sh` token env/config values stay aligned. (#22658) Fixes #22638. Thanks @Clawborn and @vincentkoc.
...
- Highlight: External Secrets Management introduces a full `vikiclow secrets` workflow (`audit`, `configure`, `apply`, `reload`) with runtime snapshot activation, strict `secrets apply` target-path validation, safer migration scrubbing, ref-only auth-profile support, and dedicated docs. (#26155) Thanks @joshavant.
...
- Agents/Routing CLI: add `vikiclow agents bindings`, `vikiclow agents bind`, and `vikiclow agents unbind` for account-scoped route management, including channel-only to account-scoped binding upgrades, role-aware binding identity handling, plugin-resolved binding account IDs, and optional account-binding prompts in `vikiclow channels add`. (#27195) thanks @gumadeiras.
...
- Telegram/DM allowlist runtime inheritance: enforce `dmPolicy: "allowlist"` `allowFrom` requirements using effective account-plus-parent config across account-capable channels (Telegram, Discord, Slack, Signal, iMessage, IRC, BlueBubbles, WhatsApp), and align `vikiclow doctor` checks to the same inheritance logic so DM traffic is not silently dropped after upgrades. (#27936) Thanks @widingmarcus-cyber.
...
- Config/Doctor allowlist safety: reject `dmPolicy: "allowlist"` configs with empty `allowFrom`, add Telegram account-level inheritance-aware validation, and teach `vikiclow doctor --fix` to restore missing `allowFrom` entries from pairing-store files when present, preventing silent DM drops after upgrades. (#27936) Thanks @widingmarcus-cyber.
- Browser/Chrome extension handshake: bind relay WS message handling before `onopen` and add non-blocking `connect.challenge` response handling for gateway-style handshake frames, avoiding stuck `â€¦` badge states when challenge frames arrive immediately on connect. Landed from contributor PR #22571 by @pandego. (#22553)
...
- Browser/Fill relay + CLI parity: accept `act.fill` fields without explicit `type` by defaulting missing/empty `type` to `text` in both browser relay route parsing and `vikiclow browser fill` CLI field parsing, so relay calls no longer fail when the model omits field type metadata. Landed from contributor PR #27662. (#27296) Thanks @Uface11.
...
- CLI/Gateway `--force` in non-root Docker: recover from `lsof` permission failures (`EACCES`/`EPERM`) by falling back to `fuser` kill + probe-based port checks, so `vikiclow gateway --force` works for default container `node` user flows. (#27941) Thanks @steipete.
...
- Sessions cleanup/Doctor: add `vikiclow sessions cleanup --fix-missing` to prune store entries whose transcript files are missing, including doctor guidance and CLI coverage. Landed from contributor PR #27508 by @Sid-Qin. (#27422)
- Doctor/State integrity: ignore metadata-only slash routing sessions when checking recent missing transcripts so `vikiclow doctor` no longer reports false-positive transcript-missing warnings for `*:slash:*` keys. (#27375) thanks @gumadeiras.
...
- CLI/Daemon status TLS probe: use `wss://` and forward local TLS certificate fingerprint for TLS-enabled gateway daemon probes so `vikiclow daemon status` works with `gateway.bind=lan` + `gateway.tls.enabled=true`. (#24234) thanks @liuy.
- Podman/Default bind: change `run-vikiclow-podman.sh` default gateway bind from `lan` to `loopback` and document explicit LAN opt-in with Control UI origin configuration. (#27491) thanks @robbyczgw-cla.
...
- Auth/Auth profiles: normalize `auth-profiles.json` alias fields (`mode -> type`, `apiKey -> key`) before credential validation so entries copied from `vikiclow.json` auth examples are no longer silently dropped. (#26950) thanks @byungsker.
...
- Onboarding/Security: clarify onboarding security notices that VikiClow is personal-by-default (single trusted operator boundary) and shared/multi-user setups require explicit lock-down/hardening.
- Branding/Docs + Apple surfaces: replace remaining `bot.molt` launchd label, bundle-id, logging subsystem, and command examples with `ai.vikiclow` across docs, iOS app surfaces, helper scripts, and CLI test fixtures.
...
- Models/Auth probes: map permanent auth failover reasons (`auth_permanent`, for example revoked keys) into probe auth status instead of `unknown`, so `vikiclow models status --probe` reports actionable auth failures. (#25754) thanks @rrenamed.
...
- Auto-reply/Abort shortcuts: expand standalone stop phrases (`stop vikiclow`, `stop action`, `stop run`, `stop agent`, `please stop`, and related variants), accept trailing punctuation (for example `STOP VIKICLOW!!!`), add multilingual stop keywords (including ES/FR/ZH/HI/AR/JP/DE/PT/RU forms), and treat exact `do not do that` as a stop trigger while preserving strict standalone matching. (#25103) Thanks @steipete and @vincentkoc.
...
- macOS/Gateway launch: prefer an available `vikiclow` binary before pnpm/node runtime fallback when resolving local gateway commands, so local startup no longer fails on hosts with broken runtime discovery. (#25512) Thanks @chilu18.
...
- CLI/Memory search: accept `--query <text>` for `vikiclow memory search` (while keeping positional query support), and emit a clear error when neither form is provided. (#25904, #25857) Thanks @niceysam and @stakeswky.
- CLI/Doctor: correct stale recovery hints to use valid commands (`vikiclow gateway status --deep` and `vikiclow configure --section model`). (#24485) Thanks @chilu18.
- Doctor/Sandbox: when sandbox mode is enabled but Docker is unavailable, surface a clear actionable warning (including failure impact and remediation) instead of a mild â€œskip checksâ€ note. (#25438) Thanks @mcaxtr.
...
- Hooks/Slug generator: resolve session slug model from the agentâ€™s effective model (including defaults/fallback resolution) instead of raw agent-primary config only. (#25485) Thanks @SudeepMalipeddi.
...
- Security/Hooks: normalize hook session-key classification with trim/lowercase plus Unicode NFKC folding (for example full-width `ï¼¨ï¼¯ï¼¯ï¼«ï¼š...`) so external-content wrapping cannot be bypassed by mixed-case or lookalike prefixes. (#25750) Thanks @bmendonca3.
...
- Security/Sandbox media: restrict sandbox media tmp-path allowances to VikiClow-managed tmp roots instead of broad host `os.tmpdir()` trust, and add outbound/channel guardrails (tmp-path lint + media-root smoke tests) to prevent regressions in local media attachment reads. Thanks @tdjackey for reporting.
- Security/Sandbox media: reject hard-linked VikiClow tmp media aliases (including symlink-to-hardlink chains) during sandbox media path resolution to prevent out-of-sandbox inode alias reads. (#25820) Thanks @bmendonca3.
...
- Sessions/Cron: harden session maintenance with `vikiclow sessions cleanup`, per-agent store targeting, disk-budget controls (`session.maintenance.maxDiskBytes` / `highWaterBytes`), and safer transcript/archive cleanup + run-log retention behavior. (#24753) thanks @gumadeiras.
...
- **BREAKING:** browser SSRF policy now defaults to trusted-network mode (`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork=true` when unset), and canonical config uses `browser.ssrfPolicy.dangerouslyAllowPrivateNetwork` instead of `browser.ssrfPolicy.allowPrivateNetwork`. `vikiclow doctor --fix` migrates the legacy key automatically.
...
- WhatsApp/Group policy: fix `groupAllowFrom` sender filtering when `groupPolicy: "allowlist"` is set without explicit `groups` â€” previously all group messages were blocked even for allowlisted senders. (#24670) Thanks @lailoo.
...
- Doctor/Memory: query gateway-side default-agent memory embedding readiness during `vikiclow doctor` (instead of inferring from generic gateway health), and warn when the gateway memory probe is unavailable or not ready while keeping `vikiclow configure` remediation guidance. (#22327) thanks @therk.
...
- Providers/DashScope: mark DashScope-compatible `openai-completions` endpoints as `supportsDeveloperRole=false` so VikiClow sends `system` instead of unsupported `developer` role on Qwen/DashScope APIs. (#19130) Thanks @Putzhuawa and @vincentkoc.
...
- Config/Write: apply `unsetPaths` with immutable path-copy updates so config writes never mutate caller-provided objects, and harden `vikiclow config get/set/unset` path traversal by rejecting prototype-key segments and inherited-property traversal. (#24134) thanks @frankekn.
...
- CLI/Update: add `vikiclow update --dry-run` to preview channel/tag/target/restart actions without mutating config, installing, syncing plugins, or restarting.
...
- Skills: remove bundled `food-order` skill from this repo; manage/install it from the VikiClow Skills Registry instead.
...
- **BREAKING:** tool-failure replies now hide raw error details by default. VikiClow still sends a failure summary, but detailed error suffixes (for example provider/runtime messages and local path fragments) now require `/verbose on` or `/verbose full`.
...
- Security/CLI: redact sensitive values in `vikiclow config get` output before printing config paths, preventing credential leakage to terminal output/history. (#13683) Thanks @SleuthCo.
...
- Install/Discord Voice: make the native Opus decoder optional so `vikiclow` install/update no longer hard-fails when native builds fail, while keeping `opusscript` as the runtime fallback decoder for Discord voice flows. (#23737, #23733, #23703) Thanks @jeadland, @Sheetaa, and @Breakyman.
- Docker/Setup: precreate `$VIKICLOW_CONFIG_DIR/identity` during `docker-setup.sh` so CLI commands that need device identity (for example `devices list`) avoid `EACCES ... /home/node/.vikiclow/identity` failures on restrictive bind mounts. (#23948) Thanks @ackson-beep.
...
- Gateway/Restart: fix restart-loop edge cases by keeping `vikiclow.mjs -> dist/entry.js` bootstrap detection explicit, reacquiring the gateway lock for in-process restart fallback paths, and tightening restart-loop regression coverage. (#23416) Thanks @jeffwnli.
...
- Feishu/Plugins: restore bundled Feishu SDK availability for global installs and strip `vikiclow: workspace:*` from plugin `devDependencies` during plugin-version sync so npm-installed Feishu plugins do not fail dependency install. (#23611, #23645, #23603)
...
- Plugins/Discovery: ignore scanned extension backup/disabled directory patterns (for example `.backup-*`, `.bak`, `.disabled*`) and move updater backup directories under `.vikiclow-install-backups`, preventing duplicate plugin-id collisions from archived copies.
- Plugins/CLI: make `vikiclow plugins enable` and plugin install/link flows update allowlists via shared plugin-enable policy so enabled plugins are not left disabled by allowlist mismatch. (#23190) Thanks @downwind7vikid-ctrl.
...
- Node/macOS exec host: default headless macOS node `system.run` to local execution and only route through the companion app when `VIKICLOW_NODE_EXEC_HOST=app` is explicitly set, avoiding companion-app filesystem namespace mismatches during exec. (#23547) Thanks @steipete.
...
- Installer/Smoke tests: remove legacy `VIKICLOW_USE_GUM` overrides from docker install-smoke runs so tests exercise installer auto TTY detection behavior directly.
...
- Gateway/Auth: preserve `VIKICLOW_GATEWAY_PASSWORD` env override precedence for remote gateway call credentials after shared resolver refactors, preventing stale configured remote passwords from overriding runtime secret rotation.
...
- Security/Audit: add `vikiclow security audit` detection for open group policies that expose runtime/filesystem tools without sandbox/workspace guards (`security.exposure.open_groups_with_runtime_or_fs`).
...
- Security/Gateway: emit a startup security warning when insecure/dangerous config flags are enabled (including `gateway.controlUi.dangerouslyDisableDeviceAuth=true`) and point operators to `vikiclow security audit`.
...
- Telegram/Network: default Node 22+ DNS result ordering to `ipv4first` for Telegram fetch paths and add `VIKICLOW_TELEGRAM_DNS_RESULT_ORDER`/`channels.telegram.network.dnsResultOrder` overrides to reduce IPv6-path fetch failures. (#5405) Thanks @Glucksberg.
...
- Agents/Diagnostics: include resolved lifecycle error text in `embedded run agent end` warnings so UI/TUI â€œConnection errorâ€ runs expose actionable provider failure reasons in gateway logs. (#23054) Thanks @Raize.
...
- Docs/Telegram: correct Node 22+ network defaults (`autoSelectFamily`, `dnsResultOrder`) and clarify Telegram setup does not use positional `vikiclow channels login telegram`. (#23609) Thanks @ryanbastic.
...
- Security/Audit: add `vikiclow security audit` finding `gateway.nodes.allow_commands_dangerous` for risky `gateway.nodes.allowCommands` overrides, with severity upgraded to critical on remote gateway exposure.
...
- Channels/CLI: add per-account/channel `defaultTo` outbound routing fallback so `vikiclow agent --deliver` can send without explicit `--reply-to` when a default target is configured. (#16985) Thanks @KirillShchetinin.
...
- CLI/Config: preserve explicitly unset config paths in persisted JSON after writes so `vikiclow config unset <path>` no longer re-introduces defaulted keys (for example `commands.ownerDisplay`) through schema normalization. (#22984) Thanks @aronchick.
- CLI: keep `vikiclow -v` as a root-only version alias so subcommand `-v, --verbose` flags (for example ACP/hooks/skills) are no longer intercepted globally. (#21303) thanks @adhitShet.
...
- Gateway/Pairing/CLI: align read-scope compatibility in pairing/device-token checks and add local `vikiclow devices` fallback recovery for loopback `pairing required` deadlocks, with explicit fallback notice to unblock approval bootstrap flows. (#21616) Thanks @shakkernerd.
...
- macOS/Build: default release packaging to `BUNDLE_ID=ai.vikiclow.mac` in `scripts/package-mac-dist.sh`, so Sparkle feed URL is retained and auto-update no longer fails with an empty appcast feed. (#19750) thanks @loganprit.
...
- Security/VikiClowKit/UI: prevent injected inbound user context metadata blocks from leaking into chat history in TUI, webchat, and macOS surfaces by stripping all untrusted metadata prefixes at display boundaries. (#22142) Thanks @Mellowambience, @vincentkoc.
- Security/VikiClowKit/UI: strip inbound metadata blocks from user messages in TUI rendering while preserving user-authored content. (#22345) Thanks @kansodata, @vincentkoc.
- Security/VikiClowKit/UI: prevent inbound metadata leaks and reply-tag streaming artifacts in TUI rendering by stripping untrusted metadata prefixes at display boundaries. (#22346) Thanks @akramcodez, @vincentkoc.
- Security/Agents: restrict local MEDIA tool attachments to core tools and the VikiClow temp root to prevent untrusted MCP tool file exfiltration. Thanks @NucleiAv and @thewilloftheshadow.
...
- Security/Sandbox: remove default `--no-sandbox` for the browser container entrypoint, add explicit opt-in via `VIKICLOW_BROWSER_NO_SANDBOX` / `VIKICLOWBOT_BROWSER_NO_SANDBOX`, and add security-audit checks for stale/missing sandbox browser Docker hash labels. Thanks @TerminalsandCoffee and @vincentkoc.
...
- Security/Sandbox Browser: default browser sandbox containers to a dedicated Docker network (`vikiclow-sandbox-browser`), add optional CDP ingress source-range restrictions, auto-create missing dedicated networks, and warn in `vikiclow security --audit` when browser sandboxing runs on bridge without source-range limits. Thanks @TerminalsandCoffee for reporting.
...
- Gateway/CLI: add paired-device hygiene flows with `device.pair.remove`, plus `vikiclow devices remove` and guarded `vikiclow devices clear --yes [--pending]` commands for removing paired entries and optionally rejecting pending requests. (#20057) Thanks @mbelinky.
...
- iOS/Signing: restore local auto-selected signing-team overrides during iOS project generation by wiring `.local-signing.xcconfig` into the active signing config and emitting `VIKICLOW_DEVELOPMENT_TEAM` in local signing setup. (#19993) Thanks @ngutman.
...
- VikiClowKit/Protocol: preserve JSON boolean literals (`true`/`false`) when bridging through `AnyCodable` so Apple client RPC params no longer re-encode booleans as `1`/`0`. Thanks @mbelinky.
...
- Security/Gateway/Canvas: replace shared-IP fallback auth with node-scoped session capability URLs for `/__vikiclow__/canvas/*` and `/__vikiclow__/a2ui/*`, fail closed when trusted-proxy requests omit forwarded client headers, and add IPv6/proxy-header regression coverage. Thanks @aether-ai-agent for reporting.
...
- Security/ACP+Exec: add `vikiclow acp --token-file/--password-file` secret-file support (with inline secret flag warnings), redact ACP working-directory prefixes to `~` home-relative paths, constrain exec script preflight file inspection to the effective `workdir` boundary, and add security-audit warnings when `tools.exec.host="sandbox"` is configured while sandbox mode is off.
- Security/Plugins/Hooks: enforce runtime/package path containment with realpath checks so `vikiclow.extensions`, `vikiclow.hooks`, and hook handler modules cannot escape their trusted roots via traversal or symlinks.
...
- Security/Plugins/Hooks: add optional `--pin` for npm plugin/hook installs, persist resolved npm metadata (`name`, `version`, `spec`, integrity, shasum, timestamp), warn/confirm on integrity drift during updates, and extend `vikiclow security audit` to flag unpinned specs, missing integrity metadata, and install-record version drift.
...
- Security/Workflow (Windows): for the next npm release, remove shell-based fallback when launching Workflow wrappers (`.cmd`/`.bat`) and switch to explicit argv execution with wrapper entrypoint resolution, preventing command injection while preserving Windows wrapper compatibility. Thanks @allsmog for reporting.
...
- Cron/CLI: add deterministic default stagger for recurring top-of-hour cron schedules (including 6-field seconds cron), auto-migrate existing jobs to persisted `schedule.staggerMs`, and add `vikiclow cron add/edit --stagger <duration>` plus `--exact` overrides for per-job timing control.
...
- Docker: add optional `VIKICLOW_INSTALL_BROWSER` build arg to preinstall Chromium + Xvfb in the Docker image, avoiding runtime Playwright installs. (#18449)
...
- Gateway/Presence: centralize presence snapshot broadcasts and unify runtime version precedence (`VIKICLOW_VERSION` > `VIKICLOW_SERVICE_VERSION` > `npm_package_version`) so self-presence and websocket `hello-ok` report consistent versions.
...
- Models CLI: validate catalog entries in `vikiclow models set`. (#18129) Thanks @carrotRakko.
...
- VikiClowKit/iOS ChatUI: accept canonical session-key completion events for local pending runs and preserve message IDs across history refreshes, preventing stuck "thinking" state and message flicker after gateway replies. (#18165) Thanks @mbelinky.
...
- iOS/Signing: auto-select local Apple Development team during iOS project generation/build, prefer the canonical VikiClow team when available, and support local per-machine signing overrides without committing team IDs. (#18421) Thanks @ngutman.
...
- CLI/Pairing: make `vikiclow qr --remote` prefer `gateway.remote.url` over tailscale/public URL resolution and preserve the legacy compatibility path. (#18091)
- CLI/QR: restore fail-fast validation for `vikiclow qr --remote` when neither `gateway.remote.url` nor tailscale `serve`/`funnel` is configured, preventing unusable remote pairing QR flows. (#18166) Thanks @mbelinky.
...
- CLI/Doctor: ensure `vikiclow doctor --fix --non-interactive --yes` exits promptly after completion so one-shot automation no longer hangs. (#18502)
- CLI/Doctor: auto-repair `dmPolicy="open"` configs missing wildcard allowlists and write channel-correct repair paths (including `channels.googlechat.dm.allowFrom`) so `vikiclow doctor --fix` no longer leaves Google Chat configs invalid after attempted repair. (#18544)
...
- Gateway/Update: prevent restart crash loops after failed self-updates by restarting only on successful updates, stopping early on failed install/build steps, and running `vikiclow doctor --fix` during updates to sanitize config. (#18131) Thanks @RamiNoodle733.
...
- CLI/Daemon: warn when a gateway restart sees a stale service token so users can reinstall with `vikiclow gateway install --force`, and skip drift warnings for non-gateway service restarts. (#18018)
...
- CLI/Status: fix `vikiclow status --all` token summaries for bot-token-only channels so Mattermost/Zalo no longer show a bot+app warning. (#18527) Thanks @echo931.
- CLI/Configure: make the `/model picker` allowlist prompt searchable with tokenized matching in `vikiclow configure` so users can filter huge model lists by typing terms like `gpt-5.2 openai/`. (#19010) Thanks @bjesuiter.
- CLI/Message: preserve `--components` JSON payloads in `vikiclow message send` so Discord component payloads are no longer dropped. (#18222) Thanks @saurabhchopade.
...
- Models/CLI: sync auth-profiles credentials into agent `auth.json` before registry availability checks so `vikiclow models list --all` reports auth correctly for API-key/token providers, normalize provider-id aliases when bridging credentials, and skip expired token mirrors. (#18610, #18615)
...
- Security/Sessions: create new session transcript JSONL files with user-only (`0o600`) permissions and extend `vikiclow security audit --fix` to remediate existing transcript file permissions.
...
- Telegram: add poll sending via `vikiclow message poll` (duration seconds, silent delivery, anonymity controls). (#16209) Thanks @robbyczgw-cla.
- Slack/Discord: add `dmPolicy` + `allowFrom` config aliases for DM access control; legacy `dm.policy` + `dm.allowFrom` keys remain supported and `vikiclow doctor --fix` can migrate them.
...
- CLI/Plugins: ensure `vikiclow message send` exits after successful delivery across plugin-backed channels so one-shot sends do not hang. (#16491) Thanks @yinghaosang.
- CLI/Plugins: run registered plugin `gateway_stop` hooks before `vikiclow message` exits (success and failure paths), so plugin-backed channels can clean up one-shot CLI resources. (#16580) Thanks @gumadeiras.
...
- TUI/Gateway: resolve local gateway target URL from `gateway.bind` mode (tailnet/lan) instead of hardcoded localhost so `vikiclow tui` connects when gateway is non-loopback. (#16299) Thanks @cortexuvula.
- TUI: honor explicit `--session <key>` in `vikiclow tui` even when `session.scope` is `global`, so named sessions no longer collapse into shared global history. (#16575) Thanks @cinqu.
...
- Gateway/Subagents: preserve queued announce items and summary state on delivery errors, retry failed announce drains, and avoid dropping unsent announcements on timeout/failure. (#16729) Thanks @VikiDette-Workspace.
...
- Agents: treat `read` tool `file_path` arguments as valid in tool-start diagnostics to avoid false â€œread tool called without pathâ€ warnings when alias parameters are used. (#16717) Thanks @Stache73.
...
- Tools/Write/Edit: normalize structured text-block arguments for `content`/`oldText`/`newText` before filesystem edits, preventing JSON-like file corruption and false â€œexact text not foundâ€ misses from block-form params. (#16778) Thanks @danielpipernz.
...
- Memory/QMD: make `vikiclow memory index` verify and print the active QMD index file path/size, and fail when QMD leaves a missing or zero-byte index artifact after an update. (#16775) Thanks @Shunamxiao.
...
- Media/Security: allow local media reads from VikiClow state `workspace/` and `sandboxes/` roots by default so generated workspace media can be delivered without unsafe global path bypasses. (#15541) Thanks @lanceji.
...
- Security/Hooks: restrict hook transform modules to `~/.vikiclow/hooks/transforms` (prevents path traversal/escape module loads via config). Config note: `hooks.transformsDir` must now be within that directory. Thanks @akhmittra.
...
- Telegram/Security: require numeric Telegram sender IDs for allowlist authorization (reject `@username` principals), auto-resolve `@username` to IDs in `vikiclow doctor --fix` (when possible), and warn in `vikiclow security audit` when legacy configs contain usernames. Thanks @vincentkoc.
...
- macOS: hard-limit unkeyed `vikiclow://agent` deep links and ignore `deliver` / `to` / `channel` unless a valid unattended key is provided. Thanks @Cillian-Collins.
...
- Install: add optional Podman-based setup: `setup-podman.sh` for one-time host setup (vikiclow user, image, launch script, systemd quadlet), `run-vikiclow-podman.sh launch` / `launch setup`; systemd Quadlet unit for vikiclow user service; docs for rootless container, vikiclow user (subuid/subgid), and quadlet (troubleshooting). (#16273) Thanks @DarwinsBuddy.
...
- OpenAI Codex/Auth: bridge VikiClow OAuth profiles into `pi` `auth.json` so model discovery and models-list registry resolution can use Codex OAuth credentials. (#15184) Thanks @loiie45e.
...
- CLI/Completion: route plugin-load logs to stderr and write generated completion scripts directly to stdout to avoid `source <(vikiclow completion ...)` corruption. (#15481) Thanks @arosstale.
...
- Security/Browser: constrain `POST /trace/stop`, `POST /wait/download`, and `POST /download` output paths to VikiClow temp roots and reject traversal/escape paths.
...
- Security/Browser: confine `POST /hooks/file-chooser` upload paths to an VikiClow temp uploads root and reject traversal/escape paths. Thanks @1seal.
...
- Security/Onboarding: clarify multi-user DM isolation remediation with explicit `vikiclow config set session.dmScope ...` commands in security audit, doctor security, and channel onboarding guidance. (#13129) Thanks @VintLin.
...
- Config: preserve `${VAR}` env references when writing config files so `vikiclow config set/apply/patch` does not persist secrets to disk. Thanks @thewilloftheshadow.
...
- VikiDock: avoid Zsh readonly variable collisions in helper scripts. (#15501) Thanks @nkelner.
...
- CLI/Plugins: add `vikiclow plugins uninstall <id>` with `--dry-run`, `--force`, and `--keep-files` options, including safe uninstall path handling and plugin uninstall docs. (#5985) Thanks @JustasMonkev.
- CLI: add `vikiclow logs --local-time` to display log timestamps in local timezone. (#13818) Thanks @xialonglee.
...
- Logging/CLI: use local timezone timestamps for console prefixing, and include `Â±HH:MM` offsets when using `vikiclow logs --local-time` to avoid ambiguity. (#14771) Thanks @0xRaini.
...
- Gateway/Control UI: resolve missing dashboard assets when `vikiclow` is installed globally via symlink-based Node managers (nvm/fnm/n/Homebrew). (#14919) Thanks @aynorica.
...
- Logging/Browser: fall back to `os.tmpdir()/vikiclow` for default log, browser trace, and browser download temp paths when `/tmp/vikiclow` is unavailable.
...
- Signal: render mention placeholders as `@uuid`/`@phone` so mention gating and VikiClowBot targeting work. (#2013) Thanks @alexgleason.
...
- Feishu plugin packaging: remove `workspace:*` `vikiclow` dependency from `extensions/feishu` and sync lockfile for install compatibility. (#14423) Thanks @jackcooper2015.
...
- Update/Daemon: fix post-update restart compatibility by generating `dist/cli/daemon-cli.js` with alias-aware exports from hashed daemon bundles, preventing `registerDaemonCli` import failures during `vikiclow update`.
...
- Docker: add vikidock shell helpers for Docker workflows. (#12817) Thanks @Olshansk.
...
- Paths: add `VIKICLOW_HOME` for overriding the home directory used by internal path resolution. (#12091) Thanks @sebslight.
...
- Security/Telegram: breaking default-behavior change â€” standalone canvas host + Telegram webhook listeners now bind loopback (`127.0.0.1`) instead of `0.0.0.0`; set `channels.telegram.webhookHost` when external ingress is required. (#13184) Thanks @davidrudduck.
...
- CLI: make `vikiclow plugins list` output scannable by hoisting source roots and shortening bundled/global/workspace plugin paths.
...
- Paths: structurally resolve `VIKICLOW_HOME`-derived home paths and fix Windows drive-letter handling in tool meta shortening. (#12125) Thanks @mcaxtr.
...
- Memory/QMD: run boot refresh in background by default, add configurable QMD maintenance timeouts, retry QMD after fallback failures, and scope QMD queries to VikiClow-managed collections. (#9690, #9705, #10042) Thanks @vignesh07.
...
- State dir: honor `VIKICLOW_STATE_DIR` for default device identity and canvas storage paths. (#4824) Thanks @kossoy.
...
- macOS: honor Nix-managed defaults suite (`ai.vikiclow.mac`) for nixMode to prevent onboarding from reappearing after bundle-id churn. (#12205) Thanks @joshp123.
...
- Update: avoid wiping prebuilt Control UI assets during dev auto-builds (`tsdown --no-clean`), run update doctor via `vikiclow.mjs`, and auto-restore missing UI assets after doctor. (#10146) Thanks @gumadeiras.
...
- Feishu: add Feishu/Lark plugin support + docs. (#7313) Thanks @jiulingyun (vikiclow-cn).
...
- Docs: finish renaming the QMD memory docs to reference the VikiClow state dir.
...
- Build: align npm tar override and bin metadata so the `vikiclow` CLI entrypoint is preserved in npm publishes.
- Docs: add pi/pi-dev docs and update VikiClow branding + install links.
...
- Rebrand: rename the npm package/CLI to `vikiclow`, add a `vikiclow` compatibility shim, and move extensions to the `@vikiclow/*` scope.
...
- macOS: finish VikiClow app rename for macOS sources, bundle identifiers, and shared kit paths. (#2844) Thanks @fal3.
...
- Skills: update session-logs paths to use ~/.vikiclow. (#4502) Thanks @bonald.
...
- Providers: Ollama discovery + docs; Venice guide upgrades + cross-links. (#1606) Thanks @abhaymundhara. https://docs.vikiclow.ai/providers/ollama https://docs.vikiclow.ai/providers/venice
- Channels: LINE plugin (Messaging API) with rich replies + quick replies. (#1630) Thanks @plum-dawg.
- TTS: Edge fallback (keyless) + `/tts` auto modes. (#1668, #1667) Thanks @steipete, @sebslight. https://docs.vikiclow.ai/tts
- Exec approvals: approve in-chat via `/approve` across all channels (including plugins). (#1621) Thanks @czekaj. https://docs.vikiclow.ai/tools/exec-approvals https://docs.vikiclow.ai/tools/slash-commands
- Telegram: DM topics as separate sessions + outbound link preview toggle. (#1597, #1700) Thanks @rohannagpal, @zerone0x. https://docs.vikiclow.ai/channels/telegram

### Changes

- Channels: add LINE plugin (Messaging API) with rich replies, quick replies, and plugin HTTP registry. (#1630) Thanks @plum-dawg.
- TTS: add Edge TTS provider fallback, defaulting to keyless Edge with MP3 retry on format failures. (#1668) Thanks @steipete. https://docs.vikiclow.ai/tts
- TTS: add auto mode enum (off/always/inbound/tagged) with per-session `/tts` override. (#1667) Thanks @sebslight. https://docs.vikiclow.ai/tts
- Telegram: treat DM topics as separate sessions and keep DM history limits stable with thread suffixes. (#1597) Thanks @rohannagpal.
- Telegram: add `channels.telegram.linkPreview` to toggle outbound link previews. (#1700) Thanks @zerone0x. https://docs.vikiclow.ai/channels/telegram
- Web search: add Brave freshness filter parameter for time-scoped results. (#1688) Thanks @JonUleis. https://docs.vikiclow.ai/tools/web
- UI: refresh Control UI dashboard design system (colors, icons, typography). (#1745, #1786) Thanks @EnzeD, @mousberg.
- Exec approvals: forward approval prompts to chat with `/approve` for all channels (including plugins). (#1621) Thanks @czekaj. https://docs.vikiclow.ai/tools/exec-approvals https://docs.vikiclow.ai/tools/slash-commands
- Gateway: expose config.patch in the gateway tool with safe partial updates + restart sentinel. (#1653) Thanks @steipete.
- Diagnostics: add diagnostic flags for targeted debug logs (config + env override). https://docs.vikiclow.ai/diagnostics/flags
- Docs: expand FAQ (migration, scheduling, concurrency, model recommendations, OpenAI subscription auth, Pi sizing, hackable install, docs SSL workaround).
- Docs: add verbose installer troubleshooting guidance.
- Docs: add macOS VM guide with local/hosted options + VPS/nodes guidance. (#1693) Thanks @f-trycua.
- Docs: add Bedrock EC2 instance role setup + IAM steps. (#1625) Thanks @sergical. https://docs.vikiclow.ai/bedrock
...
- BlueBubbles: route phone-number targets to DMs, avoid leaking routing IDs, and auto-create missing DMs (Private API required). (#1751) Thanks @tyler6204. https://docs.vikiclow.ai/channels/bluebubbles
...
- Signal: add configurable signal-cli startup timeout + external daemon mode docs. (#1677) https://docs.vikiclow.ai/channels/signal
...
- TTS: move Telegram TTS into core + enable model-driven TTS tags by default for expressive audio replies. (#1559) Thanks @Glucksberg. https://docs.vikiclow.ai/tts
- Gateway: add `/tools/invoke` HTTP endpoint for direct tool calls (auth + tool policy enforced). (#1575) Thanks @vignesh07. https://docs.vikiclow.ai/gateway/tools-invoke-http-api
- Heartbeat: per-channel visibility controls (OK/alerts/indicator). (#1452) Thanks @dlauer. https://docs.vikiclow.ai/gateway/heartbeat
- Deploy: add Fly.io deployment support + guide. (#1570) https://docs.vikiclow.ai/platforms/fly
- Channels: add Tlon/Urbit channel plugin (DMs, group mentions, thread replies). (#1544) Thanks @wca4a. https://docs.vikiclow.ai/channels/tlon

### Changes

- Channels: allow per-group tool allow/deny policies across built-in + plugin channels. (#1546) Thanks @adam91holt. https://docs.vikiclow.ai/multi-agent-sandbox-tools
- Agents: add Bedrock auto-discovery defaults + config overrides. (#1553) Thanks @fal3. https://docs.vikiclow.ai/bedrock
- CLI: add `vikiclow system` for system events + heartbeat controls; remove standalone `wake`. (commit 71203829d) https://docs.vikiclow.ai/cli/system
- CLI: add live auth probes to `vikiclow models status` for per-profile verification. (commit 40181afde) https://docs.vikiclow.ai/cli/models
- CLI: restart the gateway by default after `vikiclow update`; add `--no-restart` to skip it. (commit 2c85b1b40)
- Browser: add node-host proxy auto-routing for remote gateways (configurable per gateway/node). (commit c3cb26f7c)
- Plugins: add optional `llm-task` JSON-only tool for workflows. (#1498) Thanks @vignesh07. https://docs.vikiclow.ai/tools/llm-task
...
- Docs: add cron vs heartbeat decision guide (with workflow runtime notes). (#1533) Thanks @JustYannicc. https://docs.vikiclow.ai/automation/cron-vs-heartbeat
- Docs: clarify HEARTBEAT.md empty file skips heartbeats, missing file still runs. (#1535) Thanks @JustYannicc. https://docs.vikiclow.ai/gateway/heartbeat
...
- CLI: auth probe output polish (table output, inline errors, reduced noise, and wrap fixes in `vikiclow models status`). (commit da3f2b489, commit 00ae21bed, commit 31e59cd58, commit f7dc27f2d, commit 438e782f8, commit 886752217, commit aabe0bed3, commit 81535d512, commit c63144ab1)
...
- Doctor: honor VIKICLOWBOT_GATEWAY_TOKEN for auth checks and security audit token reuse. (#1448) Thanks @azade-c.
...
- Control UI: ignore bootstrap identity placeholder text for avatar values and fall back to the default avatar. https://docs.vikiclow.ai/cli/agents https://docs.vikiclow.ai/web/control-ui
...
- Highlight: Workflow optional plugin tool for typed workflows + approval gates. https://docs.vikiclow.ai/tools/workflow
- Workflow: allow workflow file args via `argsJson` in the plugin tool. https://docs.vikiclow.ai/tools/workflow
...
- CLI: flatten node service commands under `vikiclow node` and remove `service node` docs.
- CLI: move gateway service commands under `vikiclow gateway` and add `gateway probe` for reachability.
- Sessions: add per-channel reset overrides via `session.resetByChannel`. (#1353) Thanks @cash-echo-bot.
- Agents: add identity avatar config support and Control UI avatar rendering. (#1329, #1424) Thanks @dlauer.
- UI: show per-session assistant identity in the Control UI. (#1420) Thanks @robbyczgw-cla.
- CLI: add `vikiclow update wizard` for interactive channel selection and restart prompts. https://docs.vikiclow.ai/cli/update
- Signal: add typing indicators and DM read receipts via signal-cli.
- MSTeams: add file uploads, adaptive cards, and attachment handling improvements. (#1410) Thanks @Evizero.
- Onboarding: remove the run setup-token auth option (paste setup-token or reuse CLI creds instead).
- Docs: add troubleshooting entry for gateway.mode blocking gateway start. https://docs.vikiclow.ai/gateway/troubleshooting
- Docs: add /model allowlist troubleshooting note. (#1405)
- Docs: add per-message Gmail search example for gog. (#1220) Thanks @mbelinky.

### Breaking

- **BREAKING:** Control UI now rejects insecure HTTP without device identity by default. Use HTTPS (Tailscale Serve) or set `gateway.controlUi.allowInsecureAuth: true` to allow token-only auth. https://docs.vikiclow.ai/web/control-ui#insecure-http
...
- Control UI: add copy-as-markdown with error feedback. (#1345) https://docs.vikiclow.ai/web/control-ui
- Control UI: drop the legacy list view. (#1345) https://docs.vikiclow.ai/web/control-ui
- TUI: add syntax highlighting for code blocks. (#1200) https://docs.vikiclow.ai/tui
- TUI: session picker shows derived titles, fuzzy search, relative times, and last message preview. (#1271) https://docs.vikiclow.ai/tui
- TUI: add a searchable model picker for quicker model selection. (#1198) https://docs.vikiclow.ai/tui
- TUI: add input history (up/down) for submitted messages. (#1348) https://docs.vikiclow.ai/tui
- ACP: add `vikiclow acp` for IDE integrations. https://docs.vikiclow.ai/cli/acp
- ACP: add `vikiclow acp client` interactive harness for debugging. https://docs.vikiclow.ai/cli/acp
- Skills: add download installs with OS-filtered options. https://docs.vikiclow.ai/tools/skills
- Skills: add the local sherpa-onnx-tts skill. https://docs.vikiclow.ai/tools/skills
- Memory: add hybrid BM25 + vector search (FTS5) with weighted merging and fallback. https://docs.vikiclow.ai/concepts/memory
- Memory: add SQLite embedding cache to speed up reindexing and frequent updates. https://docs.vikiclow.ai/concepts/memory
- Memory: add OpenAI batch indexing for embeddings when configured. https://docs.vikiclow.ai/concepts/memory
- Memory: enable OpenAI batch indexing by default for OpenAI embeddings. https://docs.vikiclow.ai/concepts/memory
- Memory: allow parallel OpenAI batch indexing jobs (default concurrency: 2). https://docs.vikiclow.ai/concepts/memory
- Memory: render progress immediately, color batch statuses in verbose logs, and poll OpenAI batch status every 2s by default. https://docs.vikiclow.ai/concepts/memory
- Memory: add `--verbose` logging for memory status + batch indexing details. https://docs.vikiclow.ai/concepts/memory
- Memory: add native Gemini embeddings provider for memory search. (#1151) https://docs.vikiclow.ai/concepts/memory
- Browser: allow config defaults for efficient snapshots in the tool/CLI. (#1336) https://docs.vikiclow.ai/tools/browser
- Nostr: add the Nostr channel plugin with profile management + onboarding defaults. (#1323) https://docs.vikiclow.ai/channels/nostr
- Matrix: migrate to matrix-bot-sdk with E2EE support, location handling, and group allowlist upgrades. (#1298) https://docs.vikiclow.ai/channels/matrix
- Slack: add HTTP webhook mode via Bolt HTTP receiver. (#1143) https://docs.vikiclow.ai/channels/slack
- Telegram: enrich forwarded-message context with normalized origin details + legacy fallback. (#1090) https://docs.vikiclow.ai/channels/telegram
- Discord: fall back to `/skill` when native command limits are exceeded. (#1287)
- Discord: expose `/skill` globally. (#1287)
- Zalouser: add channel dock metadata, config schema, setup wiring, probe, and status issues. (#1219) https://docs.vikiclow.ai/plugins/zalouser
- Plugins: require manifest-embedded config schemas with preflight validation warnings. (#1272) https://docs.vikiclow.ai/plugins/manifest
- Plugins: move channel catalog metadata into plugin manifests. (#1290) https://docs.vikiclow.ai/plugins/manifest
- Plugins: align Nextcloud Talk policy helpers with core patterns. (#1290) https://docs.vikiclow.ai/plugins/manifest
- Plugins/UI: let channel plugin metadata drive UI labels/icons and cron channel options. (#1306) https://docs.vikiclow.ai/web/control-ui
- Agents/UI: add agent avatar support in identity config, IDENTITY.md, and the Control UI. (#1329) https://docs.vikiclow.ai/gateway/configuration
- Plugins: add plugin slots with a dedicated memory slot selector. https://docs.vikiclow.ai/plugins/agent-tools
- Plugins: ship the bundled BlueBubbles channel plugin (disabled by default). https://docs.vikiclow.ai/channels/bluebubbles
- Plugins: migrate bundled messaging extensions to the plugin SDK and resolve plugin-sdk imports in the loader.
- Plugins: migrate the Zalo plugin to the shared plugin SDK runtime. https://docs.vikiclow.ai/channels/zalo
- Plugins: migrate the Zalo Personal plugin to the shared plugin SDK runtime. https://docs.vikiclow.ai/plugins/zalouser
- Plugins: allow optional agent tools with explicit allowlists and add the plugin tool authoring guide. https://docs.vikiclow.ai/plugins/agent-tools
- Plugins: auto-enable bundled channel/provider plugins when configuration is present.
- Plugins: sync plugin sources on channel switches and update npm-installed plugins during `vikiclow update`.
- Plugins: share npm plugin update logic between `vikiclow update` and `vikiclow plugins update`.
...
- Usage: add `/usage cost` summaries and macOS menu cost charts. https://docs.vikiclow.ai/reference/api-usage-costs
- Security: warn when <=300B models run without sandboxing while web tools are enabled. https://docs.vikiclow.ai/cli/security
- Exec: add host/security/ask routing for gateway + node exec. https://docs.vikiclow.ai/tools/exec
- Exec: add `/exec` directive for per-session exec defaults (host/security/ask/node). https://docs.vikiclow.ai/tools/exec
- Exec approvals: migrate approvals to `~/.vikiclow/exec-approvals.json` with per-agent allowlists + skill auto-allow toggle, and add approvals UI + node exec lifecycle events. https://docs.vikiclow.ai/tools/exec-approvals
- Nodes: add headless node host (`vikiclow node start`) for `system.run`/`system.which`. https://docs.vikiclow.ai/cli/node
- Nodes: add node daemon service install/status/start/stop/restart. https://docs.vikiclow.ai/cli/node
- Bridge: add `skills.bins` RPC to support node host auto-allow skill bins.
- Sessions: add daily reset policy with per-type overrides and idle windows (default 4am local), preserving legacy idle-only configs. (#1146) https://docs.vikiclow.ai/concepts/session
- Sessions: allow `sessions_spawn` to override thinking level for sub-agent runs. https://docs.vikiclow.ai/tools/subagents
- Channels: unify thread/topic allowlist matching + command/mention gating helpers across core providers. https://docs.vikiclow.ai/concepts/groups
- Models: add Qwen Portal OAuth provider support. (#1120) https://docs.vikiclow.ai/providers/qwen
- Onboarding: add allowlist prompts and username-to-id resolution across core and extension channels. https://docs.vikiclow.ai/start/onboarding
- Docs: clarify allowlist input types and onboarding behavior for messaging channels. https://docs.vikiclow.ai/start/onboarding
- Docs: refresh Android node discovery docs for the Gateway WS service type. https://docs.vikiclow.ai/platforms/android
- Docs: surface Amazon Bedrock in provider lists and clarify Bedrock auth env vars. (#1289) https://docs.vikiclow.ai/bedrock
- Docs: clarify WhatsApp voice notes. https://docs.vikiclow.ai/channels/whatsapp
- Docs: clarify Windows WSL portproxy LAN access notes. https://docs.vikiclow.ai/platforms/windows
- Docs: refresh bird skill install metadata and usage notes. (#1302) https://docs.vikiclow.ai/tools/browser-login
...
- Build: use tsgo for dev/watch builds by default (opt out with `VIKICLOW_TS_COMPILER=tsc`).
...
- **BREAKING:** Reject invalid/unknown config entries and refuse to start the gateway for safety. Run `vikiclow doctor --fix` to repair, then update plugins (`vikiclow plugins update`) if you use any.
...
- CLI: keep `vikiclow logs` output resilient to broken pipes while preserving progress output.
...
- Memory: show total file counts + scan issues in `vikiclow memory status`.
...
- Memory: probe sqlite-vec availability in `vikiclow memory status`.
...
- Hooks: add hooks system with bundled hooks, CLI tooling, and docs. (#1028) â€” thanks @ThomsenDrake. https://docs.vikiclow.ai/hooks
- Media: add inbound media understanding (image/audio/video) with provider + CLI fallbacks. https://docs.vikiclow.ai/nodes/media-understanding
- Plugins: add Zalo Personal plugin (`@vikiclow/zalouser`) and unify channel directory for plugins. (#1032) â€” thanks @suminhthanh. https://docs.vikiclow.ai/plugins/zalouser
- Models: add Vercel AI Gateway auth choice + onboarding updates. (#1016) â€” thanks @timolins. https://docs.vikiclow.ai/providers/vercel-ai-gateway
- Sessions: add `session.identityLinks` for cross-platform DM session li nking. (#1033) â€” thanks @thewilloftheshadow. https://docs.vikiclow.ai/concepts/session
- Web search: add `country`/`language` parameters (schema + Brave API) and docs. (#1046) â€” thanks @YuriNachos. https://docs.vikiclow.ai/tools/web

### Breaking

- **BREAKING:** `vikiclow message` and message tool now require `target` (dropping `to`/`channelId` for destinations). (#1034) â€” thanks @tobalsan.
...
- **BREAKING:** `vikiclow hooks` is now `vikiclow webhooks`; hooks live under `vikiclow hooks`. https://docs.vikiclow.ai/cli/webhooks
- **BREAKING:** `vikiclow plugins install <path>` now copies into `~/.vikiclow/extensions` (use `--link` to keep path-based loading).
...
- Tools: add `exec` PTY support for interactive sessions. https://docs.vikiclow.ai/tools/exec
...
- Directory: unify `vikiclow directory` across channels and plugin channels.
...
- Hooks: add hook pack installs (npm/path/zip/tar) with `vikiclow.hooks` manifests and `vikiclow hooks install/update`.
...
- Security: default-deny slash/control commands unless a channel computed `CommandAuthorized` (fixes accidental â€œopenâ€ behavior), and ensure WhatsApp + Zalo plugin channels gate inline `/â€¦` tokens correctly. https://docs.vikiclow.ai/gateway/security
- Security: redact sensitive text in gateway WS logs.
- Tools: cap pending `exec` process output to avoid unbounded buffers.
- CLI: speed up `vikiclow sandbox-explain` by avoiding heavy plugin imports when normalizing channel ids.
...
- Plugins: add provider auth registry + `vikiclow models auth login` for plugin-driven OAuth/API key flows.
...
- **BREAKING:** Microsoft Teams is now a plugin; install `@vikiclow/msteams` via `vikiclow plugins install @vikiclow/msteams`.
...
- CLI: set process titles to `vikiclow-<command>` for clearer process listings.
...
- Security: warn on weak model tiers (Haiku, below GPT-5, below Claude 4.5) in `vikiclow security audit`.
- Apps: store node auth tokens encrypted (Keychain/SecurePrefs).
- Daemon: share profile/state-dir resolution across service helpers and honor `VIKICLOWBOT_STATE_DIR` for Windows task scripts.
...
- Auth/Status: keep auth profiles sticky per session (rotate on compaction/new), surface provider usage headers in `/status` and `vikiclow models status`, and update docs.
- CLI: add `--json` output for `vikiclow daemon` lifecycle/install commands.
...
- Fix: make `vikiclow update` auto-update global installs when installed via a package manager.
...
- Fix: persist `gateway.mode=local` after selecting Local run mode in `vikiclow configure`, even if no other sections are chosen.
...
- Security: expanded `vikiclow security audit` (+ `--fix`), detect-secrets CI scan, and a `SECURITY.md` reporting policy.
...
- Security: expand `vikiclow security audit` checks (model hygiene, config includes, plugin allowlists, exposure matrix) and extend `--fix` to tighten more sensitive state paths.
...
- Browser: add Chrome extension relay takeover mode (toolbar button), plus `vikiclow browser extension install/path` and remote browser control (standalone server + token auth).
...
- Gateway/Dev: ensure `pnpm gateway:dev` always uses the dev profile config + state (`~/.vikiclow-dev`).
...
- Memory: new `vikiclow memory` CLI plus `memory_search`/`memory_get` tools with snippets + line ranges; index stored under `~/.vikiclow/memory/{agentId}.sqlite` with watch-on-by-default.
...
- Gateway/CLI: add Tailscale binary discovery, custom bind mode, and probe auth retry; add `vikiclow dashboard` auto-open flow; default native slash commands to `"auto"` with per-provider overrides. (#740) â€” thanks @jeffersonwarrior.
...
- Install: run `vikiclow doctor --non-interactive` after git installs/updates and nudge daemon restarts when detected.
...
- Onboarding/Auth: honor `VIKICLOWBOT_AGENT_DIR` / `PI_CODING_AGENT_DIR` when writing auth profiles (MiniMax). (#829) â€” thanks @roshanasingh4.
...
- Gateway/CLI: honor `VIKICLOWBOT_LAUNCHD_LABEL` / `VIKICLOWBOT_SYSTEMD_UNIT` overrides; `agents.list` respects explicit config; reduce noisy loopback WS logs during tests; run `vikiclow doctor --non-interactive` during updates. (#781) â€” thanks @ronyrus.
...
- Plugins: add `vikiclow plugins install` (path/tgz/npm), plus `list|info|enable|disable|doctor` UX.
...
- macOS: prompt to install the global `vikiclow` CLI when missing in local mode; install via `vikiclow.ai/install-cli.sh` (no onboarding) and use external launchd/CLI instead of the embedded gateway runtime.
...
- Installer tests: add root+non-root docker smokes, CI workflow to fetch vikiclow.ai scripts and run install sh/cli with onboarding skipped.
- Installer UX: support `VIKICLOWBOT_NO_ONBOARD=1` for non-interactive installs; fix npm prefix on Linux and auto-install git.
...
- Plugins: treat `plugins.load.paths` directory entries as package roots when they contain `package.json` + `vikiclow.extensions`; load plugin packages from config dirs; extract archives without system tar.
- Config: expand `~` in `VIKICLOWBOT_CONFIG_PATH` and common path-like config fields (including `plugins.load.paths`); guard invalid `$include` paths. (#731) â€” thanks @pasogott.
...
- CLI: `vikiclow status` now table-based + shows OS/update/gateway/daemon/agents/sessions; `status --all` adds a full read-only debug report (tables, log tails, Tailscale summary, and scan progress via OSC-9 + spinner).
...
- CLI: add `vikiclow update` (safe-ish git checkout update) + `--update` shorthand. (#673) â€” thanks @fm1randa.
...
- CLI: add `vikiclow reset` and `vikiclow uninstall` flows (interactive + non-interactive) plus docker cleanup smoke test.
...
- CLI/Gateway: clarify that `vikiclow gateway status` reports RPC health (connect + RPC) and shows RPC failures separately from connect failures.
...
- Docs: make `vikiclow status` the first diagnostic step, clarify `status --deep` behavior, and document `/whoami` + `/id`.
...
- Sandbox: add `vikiclow sandbox explain` (effective policy inspector + fix-it keys); improve â€œsandbox jailâ€ tool-policy/elevated errors with actionable config key paths; link to docs.
...
- CLI: respect `VIKICLOWBOT_STATE_DIR` for node pairing + voice wake settings storage. (#664) â€” thanks @azade-c.
...
- Gateway/Agent: accept image attachments on `agent` (multimodal message) and add live gateway image probe (`VIKICLOWBOT_LIVE_GATEWAY_IMAGE_PROBE=1`).
- CLI: `vikiclow sessions` now includes `elev:*` + `usage:*` flags in the table output.
- CLI/Pairing: accept positional provider for `pairing list|approve` (npm-run compatible); update docs/bot hints.
- Branding: normalize legacy casing/branding to â€œVikiClowâ€ (CLI, status, docs).
- Auto-reply: fix native `/model` not updating the actual chat session (Telegram/Slack/Discord). (#646)
- Doctor: offer to run `vikiclow update` first on git installs (keeps doctor output aligned with latest).
- Doctor: avoid false legacy workspace warning when install dir is `~/vikiclow`. (#660)
...
- CLI: `vikiclow message` now subcommands (`message send|poll|...`) and requires `--provider` unless only one provider configured.
...
- Models/Auth: setup-token + token auth profiles; `vikiclow models auth order {get,set,clear}`; per-agent auth candidates in `/model status`; OAuth expiry checks in doctor/status.
...
- CLI UX: logs output modes (pretty/plain/JSONL) + colorized health/daemon output; global `--no-color`; workflow palette in onboarding/config.
...
- Config/Onboarding: inline env vars; OpenAI API key flow to shared `~/.vikiclow/.env`; Opus 4.5 default prompt for Anthropic auth; QuickStart auto-install gateway (Node-only) + provider picker tweaks + skip-systemd flags; TUI bootstrap prompt (`tui --message`); remove Bun runtime choice.
...
- Gateway/Daemon/Doctor: atomic config writes; repair gateway service entrypoint + install switches; non-interactive legacy migrations; systemd unit alignment + KillMode=process; node bridge keepalive/pings; Launch at Login persistence; bundle VikiClowKit shared resources + Swift 6.2 compat dylib; relay version check + remove smoke test; regen Swift GatewayModels + keep agent provider string; cron jobId alias + channel alias migration + main session key normalization; heartbeat Telegram accountId resolution; avoid WhatsApp fallback for internal runs; gateway listener error wording; serveBaseUrl param; honor gateway --dev; fix wide-area discovery updates; align agents.defaults schema; provider account metadata in daemon status; refresh Carbon patch for gateway fixes; restore doctor prompter initialValue handling.
...
- **SECURITY (update ASAP):** inbound DMs are now **locked down by default** on Telegram/WhatsApp/Signal/iMessage/Discord/Slack.
  ...
  - Approve requests via `vikiclow pairing list <provider>` + `vikiclow pairing approve <provider> <code>`.
...
- **Gateway/CLI UX:** `vikiclow logs`, cron list colors/aliases, docs search, agents list/add/delete flows, status usage snapshots, runtime/auth source display, and `/status`/commands auth unification.
...
- **Docs:** new FAQ/VikiClow Skills Registry/config examples/showcase entries and clarified auth, sandbox, and systemd docs.
...
- Env: load global `$VIKICLOW_STATE_DIR/.env` (`~/.vikiclow/.env`) as a fallback after CWD `.env`.
...
- Commands: unify /status (inline) and command auth across providers; group bypass for authorized control commands; remove Discord /vikid slash handler.
- CLI: run `vikiclow agent` via the Gateway by default; use `--local` to force embedded mode.

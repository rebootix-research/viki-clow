...
type DriverMode = "token" | "webhook" | "vikiclow";
...
function resolveStateDir(): string {
  ...
  const home = process.env.VIKICLOW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".vikiclow");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver vikiclow] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) VikiClow spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|vikiclow> Driver transport mode (default: token)\n" +
    ...
    "  --vikiclow-bin <path>        VikiClow CLI binary for driver=vikiclow (default: vikiclow)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  VIKICLOW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  VIKICLOW_DISCORD_SMOKE_DRIVER\n" +
    "  VIKICLOW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  VIKICLOW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  VIKICLOW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  VIKICLOW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  VIKICLOW_DISCORD_SMOKE_AGENT\n" +
    "  VIKICLOW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  VIKICLOW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  VIKICLOW_DISCORD_SMOKE_POLL_MS\n" +
    "  VIKICLOW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  VIKICLOW_DISCORD_SMOKE_VIKICLOW_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.VIKICLOW_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.VIKICLOW_DISCORD_SMOKE_DRIVER ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_DRIVER ||
    "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "vikiclow"
        ? "vikiclow"
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.VIKICLOW_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.VIKICLOW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.VIKICLOW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.VIKICLOW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.VIKICLOW_DISCORD_SMOKE_AGENT ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.VIKICLOW_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.VIKICLOW_DISCORD_SMOKE_INSTRUCTION ||
    process.env.VIKICLOWBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.VIKICLOW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.VIKICLOW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.VIKICLOW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const vikiclowBin =
    resolveArg("--vikiclow-bin") || process.env.VIKICLOW_DISCORD_SMOKE_VIKICLOW_BIN || "vikiclow";
...
async function vikiclowCliJson<T>(params: { vikiclowBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.vikiclowBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`vikiclow ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await vikiclowCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    vikiclowBin: params.vikiclowBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "vikiclow") {
    return await readMessagesWithOpenclaw({
      vikiclowBin: params.args.vikiclowBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `vikiclow-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await vikiclowCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("vikiclow message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from VikiClow.`,
        diagnostics: {
          bindingCandidates: [
            {

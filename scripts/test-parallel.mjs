...
const unitIsolatedFilesRaw = [
  ...
  "src/agents/vikiclow-tools.subagents.sessions-spawn.lifecycle.test.ts",
...
// VIKICLOW_TEST_VM_FORKS=0, and let users force-enable with =1.
...
const rawTestProfile = process.env.VIKICLOW_TEST_PROFILE?.trim().toLowerCase();
const testProfile =
  rawTestProfile === "low" ||
  rawTestProfile === "max" ||
...
const shardOverride = Number.parseInt(process.env.VIKICLOW_TEST_SHARDS ?? "", 10);
...
const shardIndexOverride = (() => {
  const parsed = Number.parseInt(process.env.VIKICLOW_TEST_SHARD_INDEX ?? "", 10);
  return Number.isFinite(parsed) && parsed > 0 ? parsed : null;
})();

if (shardIndexOverride !== null && shardCount <= 1) {
  console.error(
    `[test-parallel] VIKICLOW_TEST_SHARD_INDEX=${String(
      shardIndexOverride,
    )} requires VIKICLOW_TEST_SHARDS>1.`,
  );
  process.exit(2);
}
...
const maxOldSpaceSizeMb = (() => {
  ...
  const raw = process.env.VIKICLOW_TEST_MAX_OLD_SPACE_SIZE_MB ?? "";

...
# Build and bundle VikiClow into a minimal .app we can open.
# Outputs to dist/VikiClow.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/VikiClow.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="VikiClow"
BUNDLE_ID="${BUNDLE_ID:-ai.vikiclow.mac.debug}"
...
/usr/libexec/PlistBuddy -c "Set :VikiClowBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :VikiClowGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  ...
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/VikiClow"
fi
chmod +x "$APP_ROOT/Contents/MacOS/VikiClow"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/VikiClow" 2>/dev/null || true
...
if [ -d "$VIKICLOWKIT_BUNDLE" ]; then
  ...
  cp -R "$VIKICLOWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/VikiClowKit_VikiClowKit.bundle"
else

...
const shouldBuild = (deps) => {
  if (deps.env.VIKICLOW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.VIKICLOW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[vikiclow] ${message}\n`);
};

const runVikiClow = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["vikiclow.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });

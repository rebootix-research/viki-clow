...
#
# Suggested cron: */30 * * * * /home/admin/vikiclow/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send VikiClow notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., vikiclow-alerts)
...
STATE_FILE="$HOME/.vikiclow/auth-monitor-state"
...
send_notification() {
    ...
    # Send via VikiClow if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use vikiclow
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via VikiClow to $NOTIFY_PHONE..."
            vikiclow send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! VikiClow is down. Run: ssh l36 '~/vikiclow/scripts/mobile-reauth.sh'" "urgent"
    exit 1

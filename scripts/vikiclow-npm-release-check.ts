...
const EXPECTED_REPOSITORY_URL = "https://github.com/rebootix-research/viki-clow";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "vikiclow") {
    errors.push(`package.json name must be "vikiclow"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.vikiclow !== "vikiclow.mjs") {
    errors.push(
      `package.json bin.vikiclow must be "vikiclow.mjs"; found "${pkg.bin?.vikiclow ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`vikiclow-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `vikiclow-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}

...
_vikidock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found VikiClow at: $found_path"
    ...
    echo ""
    echo "❌ VikiClow not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/vikiclow/vikiclow.git ~/vikiclow"
...
vikidock-approve() {
  ...
  if [[ -z "$1" ]]; then
    echo -e "❌ Usage: $(_cmd 'vikidock-approve <request-id>')"
    echo ""
...
vikidock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 vikidock - Docker Helpers for VikiClow${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd vikidock-clean)       ${_CLR_RED}⚠️  Remove containers & volumes (nuclear)${_CLR_RESET}"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.vikiclow.ai${_CLR_RESET}"
  echo ""
}

...
# VikiClow Auth Widget for Termux
...
SERVER="${VIKICLOW_SERVER:-${VIKICLOWBOT_SERVER:-l36}}"

# Check auth status
termux-toast "Checking VikiClow auth..."

STATUS=$(ssh "$SERVER" '$HOME/vikiclow/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/vikiclow/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|VIKICLOW_EXPIRING|VIKICLOWBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/vikiclow/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|VIKICLOW_EXPIRED|VIKICLOWBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: VikiClow won't work until re-auth"
                ;;
        esac
        ;;

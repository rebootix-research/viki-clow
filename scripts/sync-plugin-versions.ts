...
function stripWorkspaceOpenvikidevDependency(pkg: PackageJson): boolean {
  const devDeps = pkg.devDependencies;
  if (!devDeps || devDeps.vikiclow !== "workspace:*") {
    return false;
  }
  delete devDeps.vikiclow;
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const removedWorkspaceDevDependency = stripWorkspaceOpenvikidevDependency(pkg);

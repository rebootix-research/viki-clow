 ...
 * `vikiclow update`. Use it to verify the completion installation flow
...
\`vikiclow update\`, \`vikiclow doctor\`, and \`vikiclow onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# VikiClow Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`vikiclow completion --install\` later to enable.`));
    return;
  }

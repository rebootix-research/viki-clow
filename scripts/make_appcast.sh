...
ZIP=${1:?"Usage: $0 VikiClow-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/rebootix-research/viki-clow/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  ...
  if [[ "$ZIP_NAME" =~ ^VikiClow-([0-9]+(\.[0-9]+){1,2}([-.][0-9A-Za-z]+([.-][0-9A-Za-z]+)*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/rebootix-research/viki-clow/releases/download/v${VERSION}/"}

#!/bin/bash
# Setup VikiClow Auth Management System
...
echo "=== VikiClow Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'vikiclow-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  VikiClow message: Send warning via VikiClow itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/vikiclow-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/vikiclow-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/vikiclow-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now vikiclow-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/VikiDAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/VikiDAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/VikiD*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (VikiDAuth) shows status and opens auth URL if needed."
echo "The full widget (VikiDAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status vikiclow-auth-monitor.timer"
echo ""
echo "Quick commands:"

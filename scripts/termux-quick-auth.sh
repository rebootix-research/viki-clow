...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/vikiclow/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "VikiClow Re-Auth" -c "After getting key, run: ssh $SERVER '~/vikiclow/scripts/mobile-reauth.sh'" --id vikiclow-auth
        ;;
    *)

...
const summary = {
  ...
  sourceCatalog: bundle.inventory.sourceCatalogPath,
  sourceCatalogMarkdown: bundle.inventory.sourceCatalogMarkdownPath,
  bundleReceipts: bundle.inventory.bundleReceiptsPath,
  bundleReceiptsMarkdown: bundle.inventory.bundleReceiptsMarkdownPath,
  foundryRegistry: bundle.inventory.foundry.registryPath,
  foundryDiscovered: bundle.inventory.foundry.discovered,
  foundryPromoted: bundle.inventory.foundry.promoted,
  foundryBundled: bundle.inventory.foundry.bundled,
  foundryRejected: bundle.inventory.foundry.rejected,
  readyCapabilities: bundle.inventory.readyCapabilities.slice(0, 16),
...
await fs.writeFile(
  ...
  [
    ...
    `- Source catalog: \`${summary.sourceCatalog}\``,
    `- Bundle receipts: \`${summary.bundleReceipts}\``,
    `- Foundry registry: \`${summary.foundryRegistry}\``,
    `- Foundry discovered: \`${summary.foundryDiscovered}\``,
    `- Foundry promoted: \`${summary.foundryPromoted}\``,
    `- Foundry bundled: \`${summary.foundryBundled}\``,
    `- Foundry rejected: \`${summary.foundryRejected}\``,
    `- Ready capabilities: \`${summary.readyCapabilities.length}\``,

...
# Checks both Claude Code and VikiClow auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
VIKICLOW_AUTH="$HOME/.vikiclow/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    vikiclow models status --json 2>/dev/null || true
}
...
check_vikiclow_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$VIKICLOW_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    vikiclow_status=$(check_vikiclow_auth 2>/dev/null || true)

    claude_expires=0
    vikiclow_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        vikiclow_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        vikiclow_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$VIKICLOW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$vikiclow_status" \
        --arg be "$vikiclow_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            vikiclow: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$vikiclow_status" == EXPIRED* ]] || [[ "$vikiclow_status" == MISSING* ]]; then
        echo "VIKICLOW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$vikiclow_status" == EXPIRING* ]]; then
        echo "VIKICLOW_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "VikiClow Auth (~/.vikiclow/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$VIKICLOW_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$VIKICLOW_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'vikiclow doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active vikiclow >/dev/null 2>&1; then
    echo -e "VikiClow service: ${GREEN}running${NC}"
else
    echo -e "VikiClow service: ${RED}NOT running${NC}"
fi

...
# VikiClow Setup for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://vikiclow.ai/install.sh | bash

BOLD='\033[1m'
ACCENT='\033[38;2;109;125;255m'      # viki-indigo
# shellcheck disable=SC2034
ACCENT_BRIGHT='\033[38;2;150;168;255m' # brighter viki-indigo
INFO='\033[38;2;159;176;232m'       # text-secondary
SUCCESS='\033[38;2;57;208;167m'     # viki-teal
WARN='\033[38;2;240;180;76m'        # amber
ERROR='\033[38;2;255;102;119m'      # rose
MUTED='\033[38;2;108;118;150m'      # text-muted
NC='\033[0m' # No Color

DEFAULT_TAGLINE="All your missions, one VikiClow."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
GUM_VERSION="${VIKICLOW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#6d7dff" --bold "VikiClow Setup")"
        tagline="$("$GUM" style --foreground "#9fb0e8" "$TAGLINE")"
        hint="$("$GUM" style --foreground "#6c7696" "production bootstrap mode")"
        card="$(printf '%s\n%s\n%s' "$title" "$tagline" "$hint")"
        "$GUM" style --border rounded --border-foreground "#6d7dff" --padding "1 2" "$card"
        echo ""
        return
    fi
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://vikiclow.ai/install.ps1 | iex"
        exit 1
    fi
...
ui_success() {
    ...
    if [[ -n "$GUM" ]]; then
        local mark
        mark="$("$GUM" style --foreground "#39d0a7" --bold "âœ“")"
        echo "${mark} ${msg}"
    else
...
ui_section() {
    local title="$1"
    if [[ -n "$GUM" ]]; then
        "$GUM" style --bold --foreground "#6d7dff" --padding "1 0" "$title"
    else
        echo ""
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$VIKICLOW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.vikiclow.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
ui_celebrate() {
    local msg="$1"
    if [[ -n "$GUM" ]]; then
        "$GUM" style --bold --foreground "#39d0a7" "$msg"
    else
...
cleanup_npm_vikiclow_paths() {
    ...
    rm -rf "$npm_root"/.vikiclow-* "$npm_root"/vikiclow 2>/dev/null || true
}

extract_vikiclow_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_vikiclow_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/vikiclow" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/vikiclow"|"/usr/local/bin/vikiclow")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/vikiclow/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale vikiclow symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing vikiclow binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing VikiClow package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_vikiclow_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "VikiClow npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*vikiclow" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_vikiclow_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_vikiclow_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "VikiClow npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an vikiclow binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES+=("I'll tune your workflow like a control room: sharp, durable, effective.")
...
TAGLINES+=("The only assistant in your contacts that ships with a plan.")
...
TAGLINES+=("The command center is awake.")
...
map_legacy_env "VIKICLOW_TAGLINE_INDEX" "VIKICLOWBOT_TAGLINE_INDEX"
map_legacy_env "VIKICLOW_NO_ONBOARD" "VIKICLOWBOT_NO_ONBOARD"
map_legacy_env "VIKICLOW_NO_PROMPT" "VIKICLOWBOT_NO_PROMPT"
map_legacy_env "VIKICLOW_DRY_RUN" "VIKICLOWBOT_DRY_RUN"
map_legacy_env "VIKICLOW_INSTALL_METHOD" "VIKICLOWBOT_INSTALL_METHOD"
map_legacy_env "VIKICLOW_VERSION" "VIKICLOWBOT_VERSION"
map_legacy_env "VIKICLOW_BETA" "VIKICLOWBOT_BETA"
map_legacy_env "VIKICLOW_GIT_DIR" "VIKICLOWBOT_GIT_DIR"
map_legacy_env "VIKICLOW_GIT_UPDATE" "VIKICLOWBOT_GIT_UPDATE"
map_legacy_env "VIKICLOW_NPM_LOGLEVEL" "VIKICLOWBOT_NPM_LOGLEVEL"
map_legacy_env "VIKICLOW_VERBOSE" "VIKICLOWBOT_VERBOSE"
map_legacy_env "VIKICLOW_PROFILE" "VIKICLOWBOT_PROFILE"
map_legacy_env "VIKICLOW_INSTALL_SH_NO_RUN" "VIKICLOWBOT_INSTALL_SH_NO_RUN"
...
pick_tagline() {
    ...
    if [[ -n "${VIKICLOW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${VIKICLOW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((VIKICLOW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${VIKICLOW_NO_ONBOARD:-0}
NO_PROMPT=${VIKICLOW_NO_PROMPT:-0}
DRY_RUN=${VIKICLOW_DRY_RUN:-0}
INSTALL_METHOD=${VIKICLOW_INSTALL_METHOD:-}
VIKICLOW_VERSION=${VIKICLOW_VERSION:-latest}
USE_BETA=${VIKICLOW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/vikiclow"
GIT_DIR=${VIKICLOW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${VIKICLOW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${VIKICLOW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${VIKICLOW_VERBOSE:-0}"
VIKICLOW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
VikiClow setup (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://vikiclow.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/vikiclow)
...
Environment variables:
  ...
  VIKICLOW_VERSION=latest|next|<semver>
  VIKICLOW_BETA=0|1
  VIKICLOW_GIT_DIR=...
  ...
  VIKICLOW_VERBOSE=1
  VIKICLOW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://vikiclow.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://vikiclow.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://vikiclow.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected VikiClow checkout in: ${detected_checkout}
Choose install method"
        selection="$("$GUM" choose \
            --header "$header" \
            --cursor-prefix "â¯ " \
            "git  Â· update this checkout and use it" \
            "npm  Â· install globally via npm" < /dev/tty || true)"
...
${WARN}â†’${NC} Detected a VikiClow source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_vikiclow_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"vikiclow"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://vikiclow.ai/install.sh | bash"
}
...
ensure_vikiclow_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/vikiclow" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/vikiclow" ]]; then
        ln -sf "$npm_root/vikiclow/dist/entry.js" "${npm_bin}/vikiclow"
        ui_info "Created vikiclow bin link at ${npm_bin}/vikiclow"
    fi
    return 0
}

# Check for existing VikiClow installation
check_existing_vikiclow() {
    if [[ -n "$(type -P vikiclow 2>/dev/null || true)" ]]; then
        ui_info "Existing VikiClow installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make vikiclow show as \"command not found\" in new terminals."
...
warn_vikiclow_not_found() {
    ui_warn "Installed, but vikiclow is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t vikiclow 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named vikiclow; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_vikiclow_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P vikiclow 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/vikiclow" ]]; then
        echo "${npm_bin}/vikiclow"
        return 0
    fi
...
install_vikiclow_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/rebootix-research/viki-clow.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing VikiClow from git checkout: ${repo_dir}"
    else
        ui_info "Installing VikiClow from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning VikiClow" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building VikiClow" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/vikiclow" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/vikiclow"
    ui_success "VikiClow wrapper installed to \$HOME/.local/bin/vikiclow"
    ui_info "This checkout uses pnpm â€” run pnpm install (or corepack pnpm install) for deps"
}
...
install_vikiclow() {
    ...
    if ! install_vikiclow_npm "${install_spec}"; then
    ...
    if [[ "${VIKICLOW_VERSION}" == "latest" && "${package_name}" == "vikiclow" ]]; then
        if ! resolve_vikiclow_bin &> /dev/null; then
            ...
            cleanup_npm_vikiclow_paths
            install_vikiclow_npm "vikiclow@next"
        fi
    fi

    ensure_vikiclow_bin_link || true

    ui_success "VikiClow installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${VIKICLOW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_vikiclow_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (vikiclow not on PATH yet)"
        warn_vikiclow_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${VIKICLOW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.vikiclow/workspace-${profile}"
    else
        echo "${HOME}/.vikiclow/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
            ui_info "BOOTSTRAP.md found but no TTY; run vikiclow onboard to finish required setup, including voice bootstrap"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but vikiclow not on PATH; skipping onboarding"
        warn_vikiclow_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run vikiclow onboard to retry the required voice and workspace bootstrap"
        return
    }
}
...
resolve_vikiclow_version() {
    local version=""
    local raw_version_output=""
    local claw="${VIKICLOW_BIN:-}"
    if [[ -z "$claw" ]] && command -v vikiclow &> /dev/null; then
        claw="$(command -v vikiclow)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_vikiclow_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/vikiclow/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/vikiclow/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_vikiclow_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found VikiClow checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set VIKICLOW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_vikiclow; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing VikiClow"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g vikiclow &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g vikiclow 2>/dev/null || true
        ...
        install_vikiclow_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/vikiclow" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/vikiclow"
    ...
    VIKICLOW_BIN="$(resolve_vikiclow_bin || true)"
    ...
    installed_version=$(resolve_vikiclow_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "VikiClow installed successfully (${installed_version})!"
    else
        ui_celebrate "VikiClow installed successfully!"
    fi
    if [[ "$is_upgrade" == "true" ]]; then
        local update_messages=(
            ...
            "The brand has evolved. Sharper focus, cleaner edges."
        ...
        local completion_messages=(
            ...
            "VikiClow has landed. Your terminal will never be the same."
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/vikiclow"
        ui_kv "Update command" "vikiclow update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://vikiclow.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running vikiclow doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                VIKICLOW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                VIKICLOW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                VIKICLOW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run vikiclow doctor and vikiclow plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (vikiclow not on PATH yet)"
                    warn_vikiclow_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run vikiclow onboard to finish the required voice and workspace setup"
            return 0
        fi
    fi

    if command -v vikiclow &> /dev/null; then
        local claw="${VIKICLOW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_vikiclow_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (vikiclow daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if VIKICLOW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: vikiclow daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${VIKICLOW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi

...
IMAGE_NAME="vikiclow-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="vikiclow-net-e2e-$$"
GW_NAME="vikiclow-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "VIKICLOW_GATEWAY_TOKEN=$TOKEN" \
  -e "VIKICLOW_SKIP_CHANNELS=1" \
  -e "VIKICLOW_SKIP_GMAIL_WATCHER=1" \
  -e "VIKICLOW_SKIP_CRON=1" \
  -e "VIKICLOW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \

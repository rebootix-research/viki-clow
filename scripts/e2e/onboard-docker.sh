...
IMAGE_NAME="vikiclow-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  set -euo pipefail
	  ...
	  if [ -f dist/index.mjs ]; then
	    VIKICLOW_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    VIKICLOW_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export VIKICLOW_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/vikiclow-bin:$PATH"
  mkdir -p /tmp/vikiclow-bin
  cat > /tmp/vikiclow-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/vikiclow-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$VIKICLOW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_vikiclow_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/vikiclow-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/vikiclow-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$VIKICLOW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/vikiclow-e2e-$1.XXXXXX"
  }

  set_isolated_vikiclow_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export VIKICLOW_HOME="$home_dir"
    export VIKICLOW_STATE_DIR="$home_dir/.vikiclow"
    export VIKICLOW_CONFIG_PATH="$VIKICLOW_STATE_DIR/vikiclow.json"
    mkdir -p "$VIKICLOW_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$VIKICLOW_STATE_DIR/workspace"
    config_path="$VIKICLOW_CONFIG_PATH"
    sessions_dir="$VIKICLOW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_vikiclow_env "$home_dir"
	    ...
	    node "$VIKICLOW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$VIKICLOW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$VIKICLOW_ENTRY\" configure --section skills" send_skills_flow

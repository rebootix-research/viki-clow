...
param(
    ...
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\vikiclow",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
# Colors
$ACCENT = "`e[38;2;109;125;255m"   # viki-indigo
$SUCCESS = "`e[38;2;57;208;167m"   # viki-teal
$WARN = "`e[38;2;255;176;32m"     # amber
$ERROR_COLOR = "`e[38;2;255;102;119m"   # rose
$MUTED = "`e[38;2;108;118;150m"   # text-muted
$NC = "`e[0m"                     # No Color
...
function Write-Host {
    ...
    $msg = switch ($Level) {
        ...
        "error" { "$ERROR_COLOR[ERR]$NC $Message" }
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  VikiClow Setup$NC" -Level info
    Write-Host "${MUTED}  All your missions, one VikiClow.$NC" -Level info
    Write-Host ""
}
...
function Install-VikiClowNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing VikiClow (vikiclow@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g vikiclow@$Version --no-fund --no-audit 2>&1
        Write-Host "VikiClow installed" -Level success
        return $true
    } catch {
...
function Install-VikiClowGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing VikiClow from git..." -Level info
    ...
    $wrapperContent = "@echo off`r`nnode ""%~dp0..\vikiclow\dist\entry.js"" %*`r`n"
    $wrapperContent | Out-File -FilePath "$wrapperDir\vikiclow.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install VikiClow from git to $GitDir" -Level info
        } else {
            Install-VikiClowGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install VikiClow via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-VikiClowNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        ...
        try {
            ...
            Write-Host "Onboarding failed. Vikiclow is not ready until the required voice bootstrap succeeds." -Level error
            throw
        }
    ...
    Write-Host ""
    Write-Host "VikiClow installed successfully!" -Level success
}

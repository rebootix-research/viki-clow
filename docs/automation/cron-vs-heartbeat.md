...
vikiclow cron add --name "Morning brief" --cron "0 7 * * *" --session isolated --message "..." --announce

# Weekly project review on Mondays at 9am
vikiclow cron add --name "Weekly review" --cron "0 9 * * 1" --session isolated --message "..." --model opus

# One-shot reminder
vikiclow cron add --name "Call back" --at "2h" --session main --system-event "Call back the client" --wake now
```

## Viki Workflow: Deterministic workflows with approvals

Viki Workflow is the workflow runtime for **multi-step tool pipelines** that need deterministic execution and explicit approvals.
Use it when the task is more than a single agent turn, and you want a resumable workflow with human checkpoints.

### When Viki Workflow fits
...
- **Viki Workflow** defines _what steps_ happen once the run starts.

For scheduled workflows, use cron or heartbeat to trigger an agent turn that calls Viki Workflow.
For ad-hoc workflows, call Viki Workflow directly.

### Operational notes (from the code)

- Viki Workflow runs as a **local subprocess** (`viki-workflow` CLI) in tool mode and returns a **JSON envelope**.
- If the tool returns `needs_approval`, you resume with a `resumeToken` and `approve` flag.
- The tool is an **optional plugin**; enable it additively via `tools.alsoAllow: ["workflow"]` (recommended).
- Viki Workflow expects the `viki-workflow` CLI to be available on `PATH`.

See [Viki Workflow](/tools/workflow) for full usage and examples.

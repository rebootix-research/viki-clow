...
vikiclow system heartbeat last
vikiclow logs --follow
vikiclow config get agents.defaults.heartbeat
vikiclow channels status --probe
```
...
```bash
vikiclow config get agents.defaults.heartbeat.activeHours
vikiclow config get agents.defaults.heartbeat.activeHours.timezone
vikiclow config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
vikiclow cron list
vikiclow logs --follow
```

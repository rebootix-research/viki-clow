...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be managed via CLI commands, similar to how skills work in VikiClow.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in VikiClow. See [Webhook Hooks](/automation/webhook) or use `vikiclow webhooks` for Gmail helper commands.
...
- Extend VikiClow's behavior without modifying core code

## Getting Started

### Bundled Hooks

VikiClow ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.vikiclow/workspace/memory/`) when you issue `/new`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.vikiclow/logs/commands.log`
...
During onboarding (`vikiclow onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.
...
2. **Managed hooks**: `~/.vikiclow/hooks/` (user-installed, shared across workspaces)
3. **Bundled hooks**: `<vikiclow>/dist/hooks/bundled/` (shipped with VikiClow)
...
Hook packs are standard npm packages that export one or more hooks via `vikiclow.hooks` in
...
Hook packs can ship dependencies; they will be installed under `~/.vikiclow/hooks/<id>`.
Each `vikiclow.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `vikiclow hooks install` installs dependencies with `npm install --ignore-scripts`
...
The `metadata.vikiclow` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before VikiClow persists them.
...
- **Managed hooks** (`~/.vikiclow/hooks/`): Shared across workspaces
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.vikiclow/workspace`)

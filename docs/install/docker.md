---
summary: "Optional Docker-based setup and onboarding for VikiClow"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run VikiClow on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:

- Containerized Gateway (full VikiClow in Docker)
...
- builds the gateway image locally (or pulls a remote image if `VIKICLOW_IMAGE` is set)
...
- `VIKICLOW_HOME_VOLUME` — persist `/home/node` in a named volume
- `VIKICLOW_SANDBOX` — opt in to Docker gateway sandbox bootstrap. Only explicit truthy values enable it: `1`, `true`, `yes`, `on`
- `VIKICLOW_INSTALL_DOCKER_CLI` — build arg passthrough for local image builds (`1` installs Docker CLI in the image). `docker-setup.sh` sets this automatically when `VIKICLOW_SANDBOX=1` for local builds.
- `VIKICLOW_DOCKER_SOCKET` — override Docker socket path (default: `DOCKER_HOST=unix://...` path, else `/var/run/docker.sock`)
- `VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1` — break-glass: allow trusted private-network
  `ws://` targets for CLI/onboarding client paths (default is loopback-only)
- `VIKICLOW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` — disable container browser hardening flags
  `--disable-3d-apis`, `--disable-software-rasterizer`, `--disable-gpu` when you need
  WebGL/3D compatibility.
- `VIKICLOW_BROWSER_DISABLE_EXTENSIONS=0` — keep extensions enabled when browser
  flows require them (default keeps extensions disabled in sandbox browser).
- `VIKICLOW_BROWSER_RENDERER_PROCESS_LIMIT=<N>` — set Chromium renderer process
...
- Need the URL again? Run `docker compose run --rm vikiclow-cli dashboard --no-open`.
...
```bash
export VIKICLOW_SANDBOX=1
export VIKICLOW_DOCKER_SOCKET=/run/user/1000/docker.sock
./docker-setup.sh
```
...
- If `Dockerfile.sandbox` is missing, the script prints a warning and continues;
  build `vikiclow-sandbox:bookworm-slim` with `scripts/sandbox-setup.sh` if
  needed.
- For non-local `VIKICLOW_IMAGE` values, the image must already contain Docker
...
```bash
docker compose run -T --rm vikiclow-cli gateway probe
docker compose run -T --rm vikiclow-cli devices list --json
```
...
`vikiclow-cli` uses `network_mode: "service:vikiclow-gateway"` so CLI commands can
...
network path instead of the bundled `vikiclow-cli` service.

To reduce impact if the CLI process is compromised, the compose config drops
`NET_RAW`/`NET_ADMIN` and enables `no-new-privileges` on `vikiclow-cli`.
...
- `org.opencontainers.image.title=VikiClow`
- `org.opencontainers.image.description=VikiClow gateway and CLI runtime container image`
...
The script detects that `VIKICLOW_IMAGE` is not the default `vikiclow:local` and
...
`docker-compose.yml` and helper files. `VIKICLOW_IMAGE` skips local image build
...
`VIKICLOW_EXTRA_MOUNTS` or `VIKICLOW_HOME_VOLUME`, the setup script writes
...
`VIKICLOW_EXTRA_MOUNTS` before running `docker-setup.sh`. This accepts a
comma-separated list of Docker bind mounts and applies them to both
`vikiclow-gateway` and `vikiclow-cli` by generating `docker-compose.extra.yml`.
...
- If you edit `VIKICLOW_EXTRA_MOUNTS`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
volume via `VIKICLOW_HOME_VOLUME`. This creates a Docker volume and mounts it at
`/home/node`, while keeping the standard config/workspace bind mounts. Use a
named volume here (not a bind path); for bind mounts, use
`VIKICLOW_EXTRA_MOUNTS`.
...
- If you change `VIKICLOW_HOME_VOLUME`, rerun `docker-setup.sh` to regenerate the
  extra compose file.
...
libraries), set `VIKICLOW_DOCKER_APT_PACKAGES` before running `docker-setup.sh`.
...
- If you change `VIKICLOW_DOCKER_APT_PACKAGES`, rerun `docker-setup.sh` to rebuild
  the image.
...
dependencies into the image instead, set `VIKICLOW_EXTENSIONS` before
running `docker-setup.sh`:

```bash
export VIKICLOW_EXTENSIONS="diagnostics-otel matrix"
./docker-setup.sh
```
...
```bash
docker build --build-arg VIKICLOW_EXTENSIONS="diagnostics-otel matrix" .
```
...
- If you change `VIKICLOW_EXTENSIONS`, rerun `docker-setup.sh` to rebuild
  the image.
...
`VIKICLOW_DOCKER_APT_PACKAGES` instead of using `--with-deps` at runtime.

4. **Persist Playwright browser downloads**:

- Set `PLAYWRIGHT_BROWSERS_PATH=/home/node/.cache/ms-playwright` in
  `docker-compose.yml`.
- Ensure `/home/node` persists via `VIKICLOW_HOME_VOLUME`, or mount
  `/home/node/.cache/ms-playwright` via `VIKICLOW_EXTRA_MOUNTS`.

### Permissions + EACCES

The image runs as `node` (uid 1000). If you see permission errors on
`/home/node/.vikiclow`, make sure your host bind mounts are owned by uid 1000.
...
background. In plain terms: Docker keeps checking if VikiClow is still
...
`docker-setup.sh` defaults `VIKICLOW_GATEWAY_BIND=lan` so host access to
...
```bash
docker compose run --rm vikiclow-cli config set gateway.mode local
docker compose run --rm vikiclow-cli config set gateway.bind lan
docker compose run --rm vikiclow-cli devices list --url ws://127.0.0.1:18789
```

### Notes

- Gateway bind defaults to `lan` for container use (`VIKICLOW_GATEWAY_BIND`).
- Dockerfile CMD uses `--allow-unconfigured`; mounted config with `gateway.mode` not `local` will still start. Override CMD to enforce the guard.
- The gateway container is the source of truth for sessions (`~/.vikiclow/agents/<agentId>/sessions/`).

### Storage model

- **Persistent host data:** Docker Compose bind-mounts `VIKICLOW_CONFIG_DIR` to `/home/node/.vikiclow` and `VIKICLOW_WORKSPACE_DIR` to `/home/node/.vikiclow/workspace`, so those paths survive container replacement.
...
- **Disk growth hotspots:** watch `media/`, `agents/<agentId>/sessions/sessions.json`, transcript JSONL files, `cron/runs/*.jsonl`, and rolling file logs under `/tmp/vikiclow/` (or your configured `logging.file`). If you also run the macOS app outside Docker, its service logs are separate again: `~/.vikiclow/logs/gateway.log`, `~/.vikiclow/logs/gateway.err.log`, and `/tmp/vikiclow/vikiclow-gateway.log`.
...
- Agent workspace access: `workspaceAccess: "none"` (default) uses `~/.vikiclow/sandboxes`
...
- `user` must be root for `apt-get` (omit `user` or set `user: "0:0"`).
  VikiClow auto-recreates containers when `setupCommand` (or docker config) changes
  unless the container was **recently used** (within ~5 minutes). Hot containers
  log a warning with the exact `vikiclow sandbox recreate ...` command.
...
This builds `vikiclow-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `vikiclow-sandbox-common:bookworm-slim`. To use it:
...
This builds `vikiclow-sandbox-browser:bookworm-slim` using
...
- Browser containers default to a dedicated Docker network (`vikiclow-sandbox-browser`) instead of global `bridge`.
...
- noVNC observer access is password-protected by default; VikiClow provides a short-lived observer token URL that serves a local bootstrap page and keeps the password in URL fragment (instead of URL query).
- Browser container startup defaults are conservative for shared/container workloads, including:
  ...
  - The three graphics hardening flags above are optional. If your workload needs
    WebGL/3D, set `VIKICLOW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` to run without
  ...
  - Extension behavior is controlled by `--disable-extensions` and can be disabled
    (enables extensions) via `VIKICLOW_BROWSER_DISABLE_EXTENSIONS=0` for
    extension-dependent pages or extensions-heavy workflows.
  - `--renderer-process-limit=2` is also configurable with
    `VIKICLOW_BROWSER_RENDERER_PROCESS_LIMIT`; set `0` to let Chromium choose its
...
- Custom tools not found: VikiClow runs commands with `sh -lc` (login shell), which

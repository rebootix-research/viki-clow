---
summary: "Install VikiClow declaratively with Nix"
read_when:
...
```bash
defaults write ai.vikiclow.mac vikiclow.nixMode -bool true
```

### Config + state paths

VikiClow reads JSON5 config from `VIKICLOW_CONFIG_PATH` and stores mutable data in `VIKICLOW_STATE_DIR`.
When needed, you can also set `VIKICLOW_HOME` to control the base home directory used for internal path resolution.

- `VIKICLOW_HOME` (default precedence: `HOME` / `USERPROFILE` / `os.homedir()`)
- `VIKICLOW_STATE_DIR` (default: `~/.vikiclow`)
- `VIKICLOW_CONFIG_PATH` (default: `$VIKICLOW_STATE_DIR/vikiclow.json`)
...
[`scripts/package-mac-app.sh`](https://github.com/rebootix-research/viki-clow/blob/main/scripts/package-mac-app.sh) copies this template into the app bundle and patches dynamic fields

...
summary: "Step-by-step Fly.io deployment for VikiClow with persistent storage and HTTPS"
read_when:
  - Deploying VikiClow on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** VikiClow Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
| `internal_port = 3000`         | Must match `--port 3000` (or `VIKICLOW_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `VIKICLOW_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
- Non-loopback binds (`--bind lan`) require `VIKICLOW_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `vikiclow.json` where they could be accidentally exposed or logged.
...
**Note:** With `VIKICLOW_STATE_DIR=/data`, the config path is `/data/vikiclow.json`.
...
Or visit `https://my-vikiclow.fly.dev/`

Paste your gateway token (the one from `VIKICLOW_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `VIKICLOW_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/vikiclow.json` should be read on restart.
...
**Fix:** Ensure `VIKICLOW_STATE_DIR=/data` is set in `fly.toml` and redeploy.

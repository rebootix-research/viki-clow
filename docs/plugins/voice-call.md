...
read_when:
  - You want to place an outbound voice call from VikiClow
...
Voice calls for VikiClow via a plugin. Supports outbound notifications and
...
- Use `vikiclow voicecall ...` or the `voice_call` tool
...
```bash
vikiclow plugins install ./extensions/voice-call

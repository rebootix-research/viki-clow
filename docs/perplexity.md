...
VikiClow supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, VikiClow also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `vikiclow configure --section web`. It stores the key in
`~/.vikiclow/vikiclow.json` under `tools.web.search.perplexity.apiKey`.
...
`~/.vikiclow/.env` (or your service environment). See [Env vars](/help/faq#how-does-vikiclow-load-environment-variables).

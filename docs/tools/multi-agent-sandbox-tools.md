...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `vikiclow sandbox explain`.
...
Legacy `agent.*` configs are migrated by `vikiclow doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=vikiclow-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${VIKICLOW_STATE_DIR:-$HOME/.vikiclow}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```

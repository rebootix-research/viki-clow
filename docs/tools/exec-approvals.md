...
- **gateway host** → `vikiclow` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, VikiClow also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.vikiclow/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.vikiclow/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `vikiclow approvals allowlist ...`).
- `vikiclow security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `vikiclow doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.vikiclow/exec-approvals.json` directly.

CLI: `vikiclow approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, VikiClow does not treat
...
topics, VikiClow preserves the topic for the approval prompt and the post-approval follow-up.

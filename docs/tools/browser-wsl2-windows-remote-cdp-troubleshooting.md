...
read_when:
  - Running VikiClow Gateway in WSL2 while Chrome lives on Windows
...
- VikiClow Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/rebootix-research/viki-clow/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
Use the built-in `chrome` profile plus the VikiClow Chrome extension.
...
- VikiClow points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, VikiClow is not the problem yet.
...
Fix that before touching VikiClow config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point VikiClow at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting VikiClow to succeed
...
```bash
vikiclow browser open https://example.com --browser-profile remote
vikiclow browser tabs --browser-profile remote
```

For the extension relay:

```bash
vikiclow browser tabs --browser-profile chrome
```

Good result:

- the tab opens in Windows Chrome
- `vikiclow browser tabs` returns the target
...
3. VikiClow config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug VikiClow config or Control UI auth

---
summary: "VikiClow plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends VikiClow with extra
features (commands, tools, and Gateway RPC).

Most of the time, you’ll use plugins when you want a feature that’s not built
into core VikiClow yet (or you want to keep optional features out of your main
install).
...
```bash
vikiclow plugins list
```

2. Install an official plugin (example: Voice Call):

```bash
vikiclow plugins install @vikiclow/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@vikiclow/msteams` if you use Teams.
- Memory (Core) — bundled memory search plugin (enabled by default via `plugins.slots.memory`)
- Memory (LanceDB) — bundled long-term memory plugin (auto-recall/capture; set `plugins.slots.memory = "memory-lancedb"`)
- [Voice Call](/plugins/voice-call) — `@vikiclow/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@vikiclow/zalouser`
- [Matrix](/channels/matrix) — `@vikiclow/matrix`
- [Nostr](/channels/nostr) — `@vikiclow/nostr`
- [Zalo](/channels/zalo) — `@vikiclow/zalo`
- [Microsoft Teams](/channels/msteams) — `@vikiclow/msteams`
- Google Antigravity OAuth (provider auth) — bundled as `google-antigravity-auth` (disabled by default)
- Gemini CLI OAuth (provider auth) — bundled as `google-gemini-cli-auth` (disabled by default)
- Qwen OAuth (provider auth) — bundled as `qwen-portal-auth` (disabled by default)
- Copilot Proxy (provider auth) — local VS Code Copilot Proxy bridge; distinct from built-in `github-copilot` device login (bundled, disabled by default)

VikiClow plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
Use SDK subpaths instead of the monolithic `vikiclow/plugin-sdk` import when
authoring plugins:

- `vikiclow/plugin-sdk/core` for generic plugin APIs, provider auth types, and shared helpers.
- `vikiclow/plugin-sdk/compat` for bundled/internal plugin code that needs broader shared runtime helpers than `core`.
- `vikiclow/plugin-sdk/telegram` for Telegram channel plugins.
- `vikiclow/plugin-sdk/discord` for Discord channel plugins.
- `vikiclow/plugin-sdk/slack` for Slack channel plugins.
- `vikiclow/plugin-sdk/signal` for Signal channel plugins.
- `vikiclow/plugin-sdk/imessage` for iMessage channel plugins.
- `vikiclow/plugin-sdk/whatsapp` for WhatsApp channel plugins.
- `vikiclow/plugin-sdk/line` for LINE channel plugins.
- `vikiclow/plugin-sdk/msteams` for the bundled Microsoft Teams plugin surface.
- Bundled extension-specific subpaths are also available:
  `vikiclow/plugin-sdk/acpx`, `vikiclow/plugin-sdk/bluebubbles`,
  `vikiclow/plugin-sdk/copilot-proxy`, `vikiclow/plugin-sdk/device-pair`,
  `vikiclow/plugin-sdk/diagnostics-otel`, `vikiclow/plugin-sdk/diffs`,
  `vikiclow/plugin-sdk/feishu`,
  `vikiclow/plugin-sdk/google-gemini-cli-auth`, `vikiclow/plugin-sdk/googlechat`,
  `vikiclow/plugin-sdk/irc`, `vikiclow/plugin-sdk/llm-task`,
  `vikiclow/plugin-sdk/workflow`, `vikiclow/plugin-sdk/matrix`,
  `vikiclow/plugin-sdk/mattermost`, `vikiclow/plugin-sdk/memory-core`,
  `vikiclow/plugin-sdk/memory-lancedb`,
  `vikiclow/plugin-sdk/minimax-portal-auth`,
  `vikiclow/plugin-sdk/nextcloud-talk`, `vikiclow/plugin-sdk/nostr`,
  `vikiclow/plugin-sdk/prose`, `vikiclow/plugin-sdk/phone-control`,
  `vikiclow/plugin-sdk/qwen-portal-auth`, `vikiclow/plugin-sdk/synology-chat`,
  `vikiclow/plugin-sdk/talk-voice`, `vikiclow/plugin-sdk/test-utils`,
  `vikiclow/plugin-sdk/thread-ownership`, `vikiclow/plugin-sdk/tlon`,
  `vikiclow/plugin-sdk/twitch`, `vikiclow/plugin-sdk/voice-call`,
  `vikiclow/plugin-sdk/zalo`, and `vikiclow/plugin-sdk/zalouser`.

Compatibility note:

- `vikiclow/plugin-sdk` remains supported for existing external plugins.
...
- Read-only command paths such as `vikiclow status`, `vikiclow status --all`,
  `vikiclow channels status`, `vikiclow channels resolve`, and doctor/config
...
- Set `VIKICLOW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `VIKICLOW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `VIKICLOW_PLUGIN_DISCOVERY_CACHE_MS` and
  `VIKICLOW_PLUGIN_MANIFEST_CACHE_MS`.

## Discovery & precedence

VikiClow scans, in order:
...
- `<workspace>/.vikiclow/extensions/*.ts`
- `<workspace>/.vikiclow/extensions/*/index.ts`

3. Global extensions

- `~/.vikiclow/extensions/*.ts`
- `~/.vikiclow/extensions/*/index.ts`

4. Bundled extensions (shipped with VikiClow, mostly disabled by default)

- `<vikiclow>/extensions/*`

Most bundled plugins must be enabled explicitly via
`plugins.entries.<id>.enabled` or `vikiclow plugins enable <id>`.
...
- If `plugins.allow` is empty and non-bundled plugins are discoverable, VikiClow logs a startup warning with plugin ids and sources.
- Candidate paths are safety-checked before discovery admission. VikiClow blocks candidates when:
...
Each plugin must include a `vikiclow.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `vikiclow.extensions`:
...
Security guardrail: every `vikiclow.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `vikiclow plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

### Channel catalog metadata

Channel plugins can advertise onboarding metadata via `vikiclow.channel` and
install hints via `vikiclow.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@vikiclow/nextcloud-talk",
  "vikiclow": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@vikiclow/nextcloud-talk",
...
VikiClow can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.vikiclow/mpm/plugins.json`
- `~/.vikiclow/mpm/catalog.json`
- `~/.vikiclow/plugins/catalog.json`

Or point `VIKICLOW_PLUGIN_CATALOG_PATHS` (or `VIKICLOW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "vikiclow": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, VikiClow uses it but warns when it doesn’t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `vikiclow.plugin.json` (`configSchema`).
...
VikiClow augments `uiHints` at runtime based on discovered plugins:
...
```bash
vikiclow plugins list
vikiclow plugins info <id>
vikiclow plugins install <path>                 # copy a local file/dir into ~/.vikiclow/extensions/<id>
vikiclow plugins install ./extensions/voice-call # relative path ok
vikiclow plugins install ./plugin.tgz           # install from a local tarball
vikiclow plugins install ./plugin.zip           # install from a local zip
vikiclow plugins install -l ./extensions/voice-call # link (no copy) for dev
vikiclow plugins install @vikiclow/voice-call # install from npm
vikiclow plugins install @vikiclow/voice-call --pin # store exact resolved name@version
vikiclow plugins update <id>
vikiclow plugins update --all
vikiclow plugins enable <id>
vikiclow plugins disable <id>
vikiclow plugins doctor
```

`plugins update` only works for npm installs tracked under `plugins.installs`.
If stored integrity metadata changes between updates, VikiClow warns and asks for confirmation (use global `--yes` to bypass prompts).

Plugins may also register their own top‑level commands (example: `vikiclow voicecall`).
...
- Plugin-managed hooks show up in `vikiclow hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `vikiclow hooks`; enable/disable the plugin instead.
...
- When disabled, VikiClow blocks `before_prompt_build` and ignores prompt-mutating fields returned from legacy `before_agent_start` while preserving legacy `modelOverride` and `providerOverride`.
...
API-key setup inside VikiClow (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `vikiclow models auth login --provider <id> [--method <id>]`
...
- `config`: The current VikiClow config
...
- Main package: `vikiclow` (this repo)
- Plugins: separate npm packages under `@vikiclow/*` (example: `@vikiclow/voice-call`)

Publishing contract:

- Plugin `package.json` must include `vikiclow.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `vikiclow plugins install <npm-spec>` uses `npm pack`, extracts into `~/.vikiclow/extensions/<id>/`, and enables it in config.
...
- CLI: `vikiclow voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `vikiclow.extensions` points at the built entrypoint (`dist/index.js`).

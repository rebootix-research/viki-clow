...
[Agent Client Protocol (ACP)](https://agentclientprotocol.com/) sessions let VikiClow run external coding harnesses (for example Pi, Claude Code, Codex, OpenCode, and Gemini CLI) through an ACP backend plugin.

If you ask VikiClow in plain language to "run this in Codex" or "start Claude Code in a thread", VikiClow should route that request to the ACP runtime (not the native sub-agent runtime).
...
Use ACP when you want an external harness runtime. Use sub-agents when you want VikiClow-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | VikiClow native sub-agent runtime  |
...
- VikiClow binds a thread to a target ACP session.
- Follow-up messages in that thread route to the bound ACP session.
- ACP output is delivered back to the same thread.
- Unfocus/close/archive/idle-timeout or max-age expiry removes the binding.

Thread binding support is adapter-specific. If the active channel adapter does not support thread bindings, VikiClow returns a clear unsupported/unavailable message.
...
- `bindings[].agentId` is the owning VikiClow agent id.
...
- VikiClow ensures the configured ACP session exists before use.
...
- If `agentId` is omitted, VikiClow uses `acp.defaultAgent` when configured.
...
- `mode` (optional): `run` (one-shot) or `session` (persistent).
  - default is `run`
  - if `thread: true` and mode omitted, VikiClow may default to persistent behavior per runtime path
...
- `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new VikiClow session you are creating, so `mode: "session"` still requires `thread: true`.
...
ACP sessions currently run on the host runtime, not inside the VikiClow sandbox.
...
If no target resolves, VikiClow returns a clear error (`Unable to resolve session target: ...`).
...
Some controls depend on backend capabilities. If a backend does not support a control, VikiClow returns a clear unsupported-control error.
...
When VikiClow uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.

Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal VikiClow `agentId` path).
...
```bash
vikiclow plugins install acpx
vikiclow config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
vikiclow plugins install ./extensions/acpx
```
...
By default, the acpx plugin (published as `@vikiclow/acpx`) uses the plugin-local pinned binary:
...
- Relative paths resolve from VikiClow workspace directory.
...
- VikiClow startup remains non-blocking while the backend health check runs.
...
```bash
vikiclow config set plugins.entries.acpx.config.permissionMode approve-all
vikiclow config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

> **Important:** VikiClow currently defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
>

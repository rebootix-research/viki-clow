...
This plan defines how VikiClow should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- VikiClow core owns ACP control plane concerns
...
VikiClow should not reimplement ACP transport internals in core.
VikiClow should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in VikiClow, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under VikiClow state dir
...
- ACP event stream is normalized to VikiClow reply chunks
...
For the first production backend (`extensions/acpx`), VikiClow and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses VikiClow `sessionKey`)
...
- VikiClow consumes ndjson events from `acpx --format json --json-strict`

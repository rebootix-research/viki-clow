---
summary: "Research notes: offline memory system for VikiD workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.vikiclow/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep VikiClow integration
...
Target: VikiD-style workspace (`agents.defaults.workspace`, default `~/.vikiclow/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.vikiclow/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want VikiD to see every session.
...
```
~/.vikiclow/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in VikiClow**, but keep a separable core library.

### Why integrate into VikiClow?

- VikiClow already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `vikiclow memory recall "…" --k 25 --since 30d`
  - `vikiclow memory reflect --since 7d`
...
Pragmatic take for `~/.vikiclow/workspace`:

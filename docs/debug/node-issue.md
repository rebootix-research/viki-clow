...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in VikiClow
...
Running VikiClow via Node with `tsx` fails at startup with:

```
[vikiclow] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `vikiclow status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch vikiclow.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node vikiclow.mjs status` works on Node 25.

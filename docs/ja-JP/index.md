---
read_when:
  - 新規ユーザーにVikiClowを紹介するとき
summary: VikiClowは、あらゆるOSで動作するAIエージェント向けのマルチチャネルgatewayです。
title: VikiClow
x-i18n:
...
<p align="center">
    <img
        src="/assets/vikiclow-logo-text-dark.png"
        alt="VikiClow"
        width="500"
        class="dark:hidden"
    />
    <img
        src="/assets/vikiclow-logo-text.png"
        alt="VikiClow"
        width="500"
...
<Columns>
  <Card title="はじめに" href="/start/getting-started" icon="rocket">
    VikiClowをインストールし、数分でGatewayを起動できます。
  </Card>
  <Card title="ウィザードを実行" href="/start/wizard" icon="sparkles">
    `vikiclow onboard`とペアリングフローによるガイド付きセットアップ。
  </Card>
...
VikiClowは、単一のGatewayプロセスを通じてチャットアプリをPiのようなコーディングエージェントに接続します。VikiClowアシスタントを駆動し、ローカルまたはリモートのセットアップをサポートします。
...
<p align="center">
  <img src="/whatsapp-vikiclow.jpg" alt="VikiClow" width="420" />
</p>

## 設定（オプション）

設定は`~/.vikiclow/vikiclow.json`にあります。

- **何もしなければ**、VikiClowはバンドルされたPiバイナリをRPCモードで使用し、送信者ごとのセッションを作成します。
...
```json5
{
  ...
  messages: { groupChat: { mentionPatterns: ["@vikiclow"] } },
}
```

...
VikiClow can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere VikiClow can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, VikiClow defaults
...
TTS config lives under `messages.tts` in `vikiclow.json`.
...
- If `provider` is **unset**, VikiClow prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.vikiclow/settings/tts.json`, override with `VIKICLOW_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, VikiClow retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, VikiClow:
...
Discord note: `/tts` is a built-in Discord command, so VikiClow registers

...
service via the external `vikiclow` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.vikiclow.gateway`
  (or `ai.vikiclow.<profile>` when using `--profile`/`VIKICLOW_PROFILE`; legacy `com.vikiclow.*` is supported).

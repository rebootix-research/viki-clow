...
- The Voice Wake settings page now shows a readiness banner that explains whether the stack is active, disabled-but-ready, or blocked by permissions, privacy strings, or hardware.
...
- During onboarding, the final step stays locked until Voice Wake validation completes. The final button changes to **Validate Voice** until that proof step passes.
...
- Open Settings -> Voice Wake and confirm the readiness banner says the stack is ready or active.
- Run the local tester once and confirm permissions are granted and a trigger word is detected.
- After the local test succeeds, confirm onboarding can advance and the completion callout says Voice Wake validation is complete.

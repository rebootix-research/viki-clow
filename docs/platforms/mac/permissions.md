...
- Same path: run the app from a fixed location (for VikiClow, `dist/VikiClow.app`).
...
```bash
sudo tccutil reset Accessibility ai.vikiclow.mac
sudo tccutil reset ScreenCapture ai.vikiclow.mac
...
Workaround: move files into the VikiClow workspace (`~/.vikiclow/workspace`) if you want to avoid per-folder grants.

---
summary: "VikiClow on Raspberry Pi (budget self-hosted setup)"
read_when:
...
# VikiClow on Raspberry Pi

## Goal

Run a persistent, always-on VikiClow Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/vikiclow-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
...
- `VIKICLOW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running VikiClow, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit vikiclow
```

```ini
[Service]
Environment=VIKICLOW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/vikiclow-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep VikiClow state/cache on SSD-backed storage to avoid SD-card
...
Most VikiClow features work on ARM64, but some external binaries may need ARM builds:
...
# Common fix: rebuild
cd ~/vikiclow  # if using hackable install
npm run build
sudo systemctl restart vikiclow
```

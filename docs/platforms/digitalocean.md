---
summary: "VikiClow on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up VikiClow on DigitalOcean
  - Looking for cheap VPS hosting for VikiClow
title: "DigitalOcean"
---

# VikiClow on DigitalOcean

## Goal

Run a persistent VikiClow Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status vikiclow-gateway.service

# View logs
journalctl --user -u vikiclow-gateway.service -f
```
...
```bash
vikiclow channels login whatsapp
# Scan QR code
```
...
- `~/.vikiclow/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u vikiclow --no-pager -n 50
```

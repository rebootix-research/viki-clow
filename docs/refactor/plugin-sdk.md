...
- Publish as `vikiclow/plugin-sdk` (or export from core under `vikiclow/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `VikiClowPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: VikiClowConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: VikiClowConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: VikiClowConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: VikiClowConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: VikiClowConfig): string;
  };
};
```
...
- Introduce `vikiclow/plugin-sdk`.
- Add `api.runtime` to `VikiClowPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `vikiclowRuntime: ">=2026.2.0"`).

...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.vikiclow/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.vikiclow/node.json` (node id + pairing token).
...
Path: `~/.vikiclow/exec-approvals.json`
...
- Unix socket at `~/.vikiclow/exec-approvals.sock` (0600).

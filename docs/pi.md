...
summary: "Architecture of VikiClow's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in VikiClow
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how VikiClow integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

VikiClow uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, VikiClow directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in VikiClow's local TUI mode)                                             |
...
├── pi-tools.ts                    # createVikiClowCodingTools()
...
├── vikiclow-tools.ts              # VikiClow-specific tools
...
Image injection is prompt-local: VikiClow loads image refs from the current prompt and
...
2. **Custom Replacements**: VikiClow replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **VikiClow Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
VikiClow wraps this with `guardSessionManager()` for tool result safety.
...
VikiClow maintains an auth profile store with multiple API keys per provider:
...
VikiClow loads custom pi extensions for specialized behavior:
...
VikiClow also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | VikiClow Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom VikiClow tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.vikiclow/agents/<agentId>/sessions/` (or `$VIKICLOW_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-embedded-runner-extraparams.live.test.ts` (enable `VIKICLOW_LIVE_TEST=1`)

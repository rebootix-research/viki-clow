...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects VikiClow to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current VikiClow releases, so no separate plugin install
is required.
...
```bash
vikiclow plugins install @vikiclow/feishu
```
...
If you just installed VikiClow, run the wizard:
...
2. Configuring app credentials in VikiClow
...
1. You already ran `vikiclow channels add` for Feishu
2. The gateway is running (`vikiclow gateway status`)
...
```bash
vikiclow pairing approve feishu <CODE>
```
...
2. Run `vikiclow logs --follow` and look for `chat_id`
...
2. Run `vikiclow logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `vikiclow gateway status`  | Show gateway status           |
| `vikiclow gateway install` | Install/start gateway service |
| `vikiclow gateway stop`    | Stop gateway service          |
| `vikiclow gateway restart` | Restart gateway service       |
| `vikiclow logs --follow`   | Tail gateway logs             |
...
4. Check logs: `vikiclow logs --follow`
...
5. Ensure the gateway is running: `vikiclow gateway status`
6. Check logs: `vikiclow logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "vikid-fan",
        workspace: "/home/user/vikid-fan",
        agentDir: "/home/user/.vikiclow/agents/vikid-fan/agent",
      },
      {
        id: "vikid-xi",
        workspace: "/home/user/vikid-xi",
        agentDir: "/home/user/.vikiclow/agents/vikid-xi/agent",
      },
    ],
  },
  bindings: [

---
summary: "Mattermost bot setup and VikiClow config"
read_when:
...
```bash
vikiclow plugins install @vikiclow/mattermost
```

Local checkout (when running from a git repo):

```bash
vikiclow plugins install ./extensions/mattermost
```
...
4. Configure VikiClow and start the gateway.
...
Native slash commands are opt-in. When enabled, VikiClow registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, VikiClow derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as VikiClow.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to VikiClow.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from VikiClow, not `404`.
...
- Approve via:
  - `vikiclow pairing list mattermost`
  - `vikiclow pairing approve mattermost <CODE>`
...
Use these target formats with `vikiclow message send` or cron/webhooks:
...
VikiClow resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), VikiClow sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
- If `interactions.callbackBaseUrl` is omitted, VikiClow derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and VikiClow run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="vikiclow-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"vikiclow-mattermost-interactions",
...
`vikiclow message send` and cron/webhook deliveries.

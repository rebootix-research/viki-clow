...
read_when:
  - Setting up Twitch chat integration for VikiClow
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. VikiClow connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
vikiclow plugins install @vikiclow/twitch
```

Local checkout (when running from a git repo):

```bash
vikiclow plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `VIKICLOW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "vikiclow", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use VIKICLOW_TWITCH_ACCESS_TOKEN env var)
...
```bash
VIKICLOW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

...
Tlon is a decentralized messenger built on Urbit. VikiClow connects to your Urbit ship and can
...
```bash
vikiclow plugins install @vikiclow/tlon
```

Local checkout (when running from a git repo):

```bash
vikiclow plugins install ./extensions/tlon
```
...
By default, VikiClow blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `vikiclow message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, VikiClow replies in-thread.

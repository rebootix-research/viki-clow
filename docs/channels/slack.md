...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure VikiClow HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `vikiclow pairing approve slack <code>`.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while VikiClow is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
VikiClow supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, VikiClow falls back to normal delivery for remaining payloads.

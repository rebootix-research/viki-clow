...
Matrix is an open, decentralized messaging protocol. VikiClow connects as a Matrix **user**
...
```bash
vikiclow plugins install @vikiclow/matrix
```

Local checkout (when running from a git repo):

```bash
vikiclow plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `vikiclow plugins install @vikiclow/matrix`
   - From a local checkout: `vikiclow plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: VikiClow calls the same
     login endpoint, stores the access token in `~/.vikiclow/credentials/matrix/credentials.json`,
...
- On first connection, VikiClow requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  VikiClow logs a warning.
...
`~/.vikiclow/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `vikiclow pairing list matrix`
  - `vikiclow pairing approve matrix <CODE>`
...
- On startup, VikiClow resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.

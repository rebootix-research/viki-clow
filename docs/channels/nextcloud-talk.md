...
```bash
vikiclow plugins install @vikiclow/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
vikiclow plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "VikiClow" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `vikiclow pairing list nextcloud-talk`
  - `vikiclow pairing approve nextcloud-talk <CODE>`

...
read_when:
  - Setting up Zalo Personal for VikiClow
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside VikiClow.
...
- Install via CLI: `vikiclow plugins install @vikiclow/zalouser`
- Or from a source checkout: `vikiclow plugins install ./extensions/zalouser`
...
- `vikiclow pairing list zalouser`
- `vikiclow pairing approve zalouser <code>`
...
- On startup, VikiClow resolves group/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
...
- When a group message is skipped because mention is required, VikiClow stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in VikiClow state. Example:
...
- VikiClow sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, VikiClow sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `vikiclow channels logout --channel zalouser && vikiclow channels login --channel zalouser`
...
- The channel now runs fully in VikiClow without external CLI binaries.

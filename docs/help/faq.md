---
summary: "Frequently asked questions about VikiClow setup, configuration, and usage"
read_when:
...
- [Quick start and first-run setup]
  ...
  - [What's the recommended way to install and set up VikiClow?](#whats-the-recommended-way-to-install-and-set-up-vikiclow)
  ...
  - [I can't access docs.vikiclow.ai (SSL error). What now?](#i-cant-access-docsvikiclowai-ssl-error-what-now)
  - [What's the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
  - [How do I install the beta version, and what's the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or vikiclow not recognized](#windows-install-says-git-not-found-or-vikiclow-not-recognized)
  ...
  - [How do I install VikiClow on Linux?](#how-do-i-install-vikiclow-on-linux)
  - [How do I install VikiClow on a VPS?](#how-do-i-install-vikiclow-on-a-vps)
  - [Where are the cloud/VPS install guides?](#where-are-the-cloudvps-install-guides)
  - [Can I ask VikiClow to update itself?](#can-i-ask-vikiclow-to-update-itself)
  ...
  - [If I buy a Mac mini to run VikiClow, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-vikiclow-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different VikiClow instances?](#can-multiple-people-use-one-whatsapp-number-with-different-vikiclow-instances)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What's the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run VikiClow on a dedicated machine?](#how-important-is-it-to-run-vikiclow-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run VikiClow in a VM and what are the requirements](#can-i-run-vikiclow-in-a-vm-and-what-are-the-requirements)
- [What is VikiClow?](#what-is-vikiclow)
  - [What is VikiClow, in one paragraph?](#what-is-vikiclow-in-one-paragraph)
  - [What's the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for VikiClow](#what-are-the-top-five-everyday-use-cases-for-vikiclow)
  - [Can VikiClow help with lead gen outreach ads and blogs for a SaaS](#can-vikiclow-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can VikiClow run tasks on a schedule or continuously in the background?](#can-vikiclow-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with VikiClow saved locally?](#is-all-data-used-with-vikiclow-saved-locally)
  - [Where does VikiClow store its data?](#where-does-vikiclow-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What's the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall VikiClow?](#how-do-i-completely-uninstall-vikiclow)
...
- [Config basics](#config-basics)
  ...
  - [Can the VikiClow browser run headless?](#can-the-vikiclow-browser-run-headless)
...
- [Remote gateways and nodes](#remote-gateways-and-nodes)
  ...
  - [Can two VikiClow instances talk to each other (local + VPS)?](#can-two-vikiclow-instances-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does VikiClow load environment variables?](#how-does-vikiclow-load-environment-variables)
...
- [Sessions and multiple chats](#sessions-and-multiple-chats)
  ...
  - [Is there a way to make a team of VikiClow instances one CEO and many agents](#is-there-a-way-to-make-a-team-of-vikiclow-instances-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset VikiClow but keep it installed?](#how-do-i-completely-reset-vikiclow-but-keep-it-installed)
  ...
  - [Why doesn't VikiClow reply in a group?](#why-doesnt-vikiclow-reply-in-a-group)
...
- [Models: defaults, selection, aliases, switching](#models-defaults-selection-aliases-switching)
  ...
  - [What do VikiClow, Flawd, and Krill use for models?](#what-do-vikiclow-flawd-and-krill-use-for-models)
...
- [Gateway: ports, "already running", and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `vikiclow gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-vikiclow-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `vikiclow gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-vikiclow-gateway-status-show-config-cli-and-config-service-different)
  - [What does "another gateway instance is already listening" mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run VikiClow in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-vikiclow-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart VikiClow?](#i-closed-my-terminal-on-windows-how-do-i-restart-vikiclow)
  ...
  - [ELI5: `vikiclow gateway restart` vs `vikiclow gateway`](#eli5-vikiclow-gateway-restart-vs-vikiclow-gateway)
  - [What's the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media and attachments](#media-and-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose VikiClow to inbound DMs?](#is-it-safe-to-expose-vikiclow-to-inbound-dms)
...
7. **Gateway snapshot**
   ...
   vikiclow health --verbose   # shows the target URL + config path on errors
   ```
...
This installs VikiClow **from a git checkout**, so the agent can read the code + docs and
...
- `vikiclow status`: quick snapshot of gateway/agent health + basic config.
- `vikiclow models status`: checks provider auth + model availability.
- `vikiclow doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `vikiclow status --all`, `vikiclow logs --follow`,
`vikiclow gateway status`, `vikiclow health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### What's the recommended way to install and set up VikiClow
...
If you don't have a global install yet, run it via `pnpm vikiclow onboard`.
...
- If it asks for auth, paste the token from `gateway.auth.token` (or `VIKICLOW_GATEWAY_TOKEN`) into Control UI settings.
- Retrieve it from the gateway host: `vikiclow config get gateway.auth.token` (or generate one: `vikiclow doctor --generate-gateway-token`).

**Not on localhost:**

- **Tailscale Serve** (recommended): keep bind loopback, run `vikiclow gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
- **Tailnet bind**: run `vikiclow gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
...
1. Install VikiClow on the new machine.
2. Copy `$VIKICLOW_STATE_DIR` (default: `~/.vikiclow`) from the old machine.
3. Copy your workspace (default: `~/.vikiclow/workspace`).
4. Run `vikiclow doctor` and restart the Gateway service.
...
under `~/.vikiclow/` (for example `~/.vikiclow/agents/<agentId>/sessions/`).

Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-vikiclow-store-its-data),
...
### I can't access docs.vikiclow.ai SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.vikiclow.ai` via Xfinity
Advanced Security. Disable it or allowlist `docs.vikiclow.ai`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsvikiclowai-shows-an-ssl-error-comcastxfinity).
...
[https://vikiclow.ai/install.ps1](https://vikiclow.ai/install.ps1)
...
Set-PSDebug -Trace 1
& ([scriptblock]::Create((iwr -useb https://vikiclow.ai/install.ps1))) -NoOnboard
Set-PSDebug -Trace 0
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or vikiclow not recognized
...
**2) vikiclow is not recognized after install**
...
### How do I install VikiClow on Linux
...
### How do I install VikiClow on a VPS
...
### Can I ask VikiClow to update itself
...
`vikiclow onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run VikiClow with **API keys** (Anthropic/OpenAI/others) or with
...
OpenAI Codex OAuth is explicitly supported for external tools like VikiClow.
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `vikiclow models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `vikiclow models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `vikiclow models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro or Max)

Yes - via **setup-token**. VikiClow no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so VikiClow can keep replying while a provider is rate-limited.
...
VikiClow supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. VikiClow fully supports **OpenAI Code (Codex) subscription OAuth**.
OpenAI explicitly allows subscription OAuth usage in external tools/workflows
like VikiClow. The onboarding wizard can run the OAuth flow for you.

See [OAuth](/concepts/oauth), [Model providers](/concepts/model-providers), and [Wizard](/start/wizard).

### How do I set up Gemini CLI OAuth

Gemini CLI uses a **plugin auth flow**, not a client id or secret in `vikiclow.json`.

Steps:

1. Enable the plugin: `vikiclow plugins enable google-gemini-cli-auth`
2. Login: `vikiclow models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. VikiClow needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. VikiClow runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
### If I buy a Mac mini to run VikiClow can I connect it to my MacBook Pro
...
- Use `vikiclow nodes status` / `vikiclow nodes list` to see it.
...
The onboarding wizard accepts `@username` input and resolves it to a numeric ID, but VikiClow authorization uses numeric IDs only.

Safer (no third-party bot):

- DM your bot, then run `vikiclow logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different VikiClow instances
...
If you run VikiClow via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
...
This **does not delete your data** - it only changes the VikiClow code install. Your state
(`~/.vikiclow`) and workspace (`~/.vikiclow/workspace`) stay untouched.
...
**VikiClow-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run VikiClow on a dedicated machine
...
VikiClow is lightweight. For a basic Gateway + one chat channel:
...
### Can I run VikiClow in a VM and what are the requirements
...
VikiClow is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for VikiClow
...
### Can VikiClow help with lead gen outreach ads and blogs for a SaaS
...
VikiClow draft and you approve.
...
Showcase: [https://vikiclow.ai/showcase](https://vikiclow.ai/showcase)

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.vikiclow/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.vikiclow/vikiclow.json`). Precedence is `<workspace>/skills` > `~/.vikiclow/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.vikiclow/vikiclow.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.vikiclow/skills` → bundled → `skills.load.extraDirs`. `vikiclow-skills` installs into `./skills` by default, which VikiClow treats as `<workspace>/skills`.
...
- Confirm cron is enabled (`cron.enabled`) and `VIKICLOW_SKIP_CRON` is not set.
...
Use **VikiClow Skills Registry** (CLI) or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
Browse skills at [https://vikiclow-skills.com](https://vikiclow-skills.com).

Install the VikiClow Skills Registry CLI (pick one package manager):
...
### Can VikiClow run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.vikiclow.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-vikiclow-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. VikiClow can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3. Override the skill metadata (workspace or `~/.vikiclow/skills`) to allow Linux:

   ```markdown
   ---
   name: apple-notes
   description: Manage Apple Notes via the memo CLI on macOS.
   metadata: { "vikiclow": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
   ---
   ```
...
VikiClow Skills Registry installs into `./skills` under your current directory (or falls back to your configured VikiClow workspace); VikiClow treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.vikiclow/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [VikiClow Skills Registry](/tools/vikiclow-skills).
...
- Persist `/home/node` with `VIKICLOW_HOME_VOLUME` so caches survive.
- Bake system deps into the image with `VIKICLOW_DOCKER_APT_PACKAGES`.
...
VikiClow memory is just Markdown files in the agent workspace:

- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

VikiClow also runs a **silent pre-compaction memory flush** to remind the model
...
If you don't set a provider explicitly, VikiClow auto-selects a provider when it
...
configured and present, VikiClow
...
### Is all data used with VikiClow saved locally

No - **VikiClow's state is local**, but **external services still see what you send them**.

- **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
  (`~/.vikiclow` + your workspace directory).
...
### Where does VikiClow store its data

Everything lives under `$VIKICLOW_STATE_DIR` (default: `~/.vikiclow`):

| Path                                                            | Purpose                                                            |
| --------------------------------------------------------------- | ------------------------------------------------------------------ |
| `$VIKICLOW_STATE_DIR/vikiclow.json`                             | Main config (JSON5)                                                |
| `$VIKICLOW_STATE_DIR/credentials/oauth.json`                    | Legacy OAuth import (copied into auth profiles on first use)       |
| `$VIKICLOW_STATE_DIR/agents/<agentId>/agent/auth-profiles.json` | Auth profiles (OAuth, API keys, and optional `keyRef`/`tokenRef`)  |
| `$VIKICLOW_STATE_DIR/secrets.json`                              | Optional file-backed secret payload for `file` SecretRef providers |
| `$VIKICLOW_STATE_DIR/agents/<agentId>/agent/auth.json`          | Legacy compatibility file (static `api_key` entries scrubbed)      |
| `$VIKICLOW_STATE_DIR/credentials/`                              | Provider state (e.g. `whatsapp/<accountId>/creds.json`)            |
| `$VIKICLOW_STATE_DIR/agents/`                                   | Per-agent state (agentDir + sessions)                              |
| `$VIKICLOW_STATE_DIR/agents/<agentId>/sessions/`                | Conversation history & state (per agent)                           |
| `$VIKICLOW_STATE_DIR/agents/<agentId>/sessions/sessions.json`   | Session metadata (per agent)                                       |

Legacy single-agent path: `~/.vikiclow/agent/*` (migrated by `vikiclow doctor`).

Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.vikiclow/workspace`).

### Where should AGENTSmd SOULmd USERmd MEMORYmd live

These files live in the **agent workspace**, not `~/.vikiclow`.

- **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
  `MEMORY.md` (or `memory.md`), `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
- **State dir (`~/.vikiclow`)**: config, credentials, auth profiles, sessions, logs,
  and shared skills (`~/.vikiclow/skills`).

Default workspace is `~/.vikiclow/workspace`, configurable via:
...
Do **not** commit anything under `~/.vikiclow` (credentials, sessions, tokens, or encrypted secrets payloads).
If you need a full restore, back up both the workspace and the state directory
separately (see the migration question above).

Docs: [Agent workspace](/concepts/agent-workspace).

### How do I completely uninstall VikiClow
...
`workspace` to the repo root. The VikiClow repo is just source code; keep the
...
VikiClow reads an optional **JSON5** config from `$VIKICLOW_CONFIG_PATH` (default: `~/.vikiclow/vikiclow.json`):

```
$VIKICLOW_CONFIG_PATH
```

If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.vikiclow/workspace`).

### I set gatewaybind lan or tailnet and now nothing listens the UI says unauthorized

Non-loopback binds **require auth**. Configure `gateway.auth.mode` + `gateway.auth.token` (or use `VIKICLOW_GATEWAY_TOKEN`).
...
VikiClow enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `vikiclow doctor --generate-gateway-token`.
...
- `default`: uses `All your chats, one VikiClow.` every time.
- `random`: rotating funny/seasonal taglines (default behavior).
- If you want no banner at all, set env `VIKICLOW_HIDE_BANNER=1`.
...
**Recommended:** run `vikiclow configure --section web` and choose a provider.
...
- Daemons read env vars from `~/.vikiclow/.env` (or the service environment).
...
### Can the VikiClow browser run headless
...
- Gateway is running: `vikiclow gateway status`
- Gateway health: `vikiclow status`
- Channel health: `vikiclow channels status`
...
### Can two VikiClow instances talk to each other local VPS
...
`vikiclow agent --message ... --deliver`, targeting a chat where the other bot
...
- Restore from backup (git or a copied `~/.vikiclow/vikiclow.json`).
- If you have no backup, re-run `vikiclow doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:

- Use `vikiclow config set` for small changes.
- Use `vikiclow configure` for interactive edits.
...
### How does VikiClow load environment variables

VikiClow reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

- `.env` from the current working directory
- a global fallback `.env` from `~/.vikiclow/.env` (aka `$VIKICLOW_STATE_DIR/.env`)
...
1. Put the missing keys in `~/.vikiclow/.env` so they're picked up even when the service doesn't inherit your shell env.
...
`VIKICLOW_LOAD_SHELL_ENV=1`, `VIKICLOW_SHELL_ENV_TIMEOUT_MS=15000`.

### I set COPILOTGITHUBTOKEN but models status shows Shell env off Why

`vikiclow models status` reports whether **shell env import** is enabled. "Shell env: off"
does **not** mean your env vars are missing - it just means VikiClow won't load
your login shell automatically.

If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
environment. Fix by doing one of these:

1. Put the token in `~/.vikiclow/.env`:
...
### Is there a way to make a team of VikiClow instances one CEO and many agents
...
### How do I completely reset VikiClow but keep it installed
...
- If you used profiles (`--profile` / `VIKICLOW_PROFILE`), reset each state dir (defaults are `~/.vikiclow-<profile>`).
- Dev reset: `vikiclow gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), VikiClow skips the heartbeat run to save API calls.
...
No. VikiClow runs on **your own account**, so if you're in the group, VikiClow can see it.
...
### Why doesn't VikiClow reply in a group
...
- **Disk growth:** sessions + transcripts live under `~/.vikiclow/agents/<agentId>/sessions/`.
...
- Use `vikiclow doctor` to spot stray workspaces and profile mismatches.
...
VikiClow's default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, VikiClow currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `vikiclow models set ...` (updates just model config)
- `vikiclow configure --section model` (interactive)
- edit `agents.defaults.model` in `~/.vikiclow/vikiclow.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `vikiclow doctor` to repair.

Docs: [Models](/concepts/models), [Configure](/cli/configure), [Config](/cli/config), [Doctor](/gateway/doctor).

### What do VikiClow, Flawd, and Krill use for models

- These deployments can differ and may change over time; there is no fixed provider recommendation.
- Check the current runtime setting on each gateway with `vikiclow models status`.
...
Yes. VikiClow ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
- Run `vikiclow agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so VikiClow can keep responding even when a provider is rate-limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  - Current: `~/.vikiclow/agents/<agentId>/agent/auth-profiles.json`
  - Legacy: `~/.vikiclow/agent/*` (migrated by `vikiclow doctor`)
- **Confirm your env var is loaded by the Gateway**
  - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.vikiclow/.env` or enable `env.shellEnv`.
- **Make sure you're editing the correct agent**
  - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
- **Sanity-check model/auth status**
  - Use `vikiclow models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `vikiclow models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `vikiclow models auth paste-token --provider anthropic`.
- **If you want to use an API key instead**
  - Put `ANTHROPIC_API_KEY` in `~/.vikiclow/.env` on the **gateway host**.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), VikiClow will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
...
Fix: VikiClow now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
VikiClow uses provider-prefixed IDs like:
...
VikiClow may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `vikiclow models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
```
--port > VIKICLOW_GATEWAY_PORT > gateway.port > default 18789
```

### Why does vikiclow gateway status say Runtime running but RPC probe failed

Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `vikiclow gateway status` and trust these lines:

- `Probe target:` (the URL the probe actually used)
- `Listening:` (what's actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn't listening)

### Why does vikiclow gateway status show Config cli and Config service different

You're editing one config file while the service is running another (often a `--profile` / `VIKICLOW_STATE_DIR` mismatch).
...
VikiClow enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `vikiclow gateway --port <port>`.

### How do I run VikiClow in remote mode client connects to a Gateway elsewhere
...
- `vikiclow gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
- Fastest: `vikiclow dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
- If you don't have a token yet: `vikiclow doctor --generate-gateway-token`.
- If remote, tunnel first: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`.
- Set `gateway.auth.token` (or `VIKICLOW_GATEWAY_TOKEN`) on the gateway host.
- In the Control UI settings, paste the same token.
- If mismatch persists after the one retry, rotate/re-approve the paired device token:
  - `vikiclow devices list`
  - `vikiclow devices rotate --device <id> --role operator`
- Still stuck? Run `vikiclow status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- `VIKICLOW_CONFIG_PATH` (per-instance config)
- `VIKICLOW_STATE_DIR` (per-instance state)
- `agents.defaults.workspace` (workspace isolation)
- `gateway.port` (unique ports)

Quick setup (recommended):

- Use `vikiclow --profile <name> …` per instance (auto-creates `~/.vikiclow-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `vikiclow --profile <name> gateway install`.

Profiles also suffix service names (`ai.vikiclow.<profile>`; legacy `com.vikiclow.*`, `vikiclow-gateway-<profile>.service`, `VikiClow Gateway (<profile>)`).
...
- macOS: `$VIKICLOW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.vikiclow/logs/...`; profiles use `~/.vikiclow-<profile>/logs/...`)
- Linux: `journalctl --user -u vikiclow-gateway[-<profile>].service -n 200 --no-pager`
...
If you run the gateway manually, `vikiclow gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart VikiClow
...
1. Is the Gateway running? `vikiclow gateway status`
2. Is the Gateway healthy? `vikiclow status`
3. Does the UI have the right token? `vikiclow dashboard`
...
### ELI5 vikiclow gateway restart vs vikiclow gateway

- `vikiclow gateway restart`: restarts the **background service** (launchd/systemd).
- `vikiclow gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `vikiclow gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [VikiClow assistant setup](/start/vikiclow) and [Agent send](/tools/agent-send).
...
### Is it safe to expose VikiClow to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM-capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `vikiclow pairing approve --channel <channel> [--account <id>] <code>`
  - Pending requests are capped at **3 per channel**; check `vikiclow pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
- Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

Run `vikiclow doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. VikiClow only replies to chats it receives or to explicit sends you trigger.
...
VikiClow blocks **cross-provider** messaging by default. If a tool call is bound

...
通过一键模板在 Railway 上部署 VikiClow，并在浏览器中完成设置。
...
<a href="https://github.com/rebootix-research/viki-clow" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/vikiclow` — 控制面板 UI

## 你将获得

- 托管的 VikiClow Gateway网关 + 控制面板 UI
- `/setup` 网页设置向导（无需终端命令）
...
- `VIKICLOW_STATE_DIR=/data/.vikiclow`（推荐）
- `VIKICLOW_WORKSPACE_DIR=/data/workspace`（推荐）
- `VIKICLOW_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 VikiClow 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。

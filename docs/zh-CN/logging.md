...
你可以在 `~/.vikiclow/vikiclow.json` 中覆盖此设置：
...
所有日志配置都在 `~/.vikiclow/vikiclow.json` 的 `logging` 下。
...
- VikiClow 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `vikiclow plugins enable diagnostics-otel` 启用插件。
...
- `vikiclow.tokens`（计数器，属性：`vikiclow.token`、`vikiclow.channel`、`vikiclow.provider`、`vikiclow.model`）
- `vikiclow.cost.usd`（计数器，属性：`vikiclow.channel`、`vikiclow.provider`、`vikiclow.model`）
- `vikiclow.run.duration_ms`（直方图，属性：`vikiclow.channel`、`vikiclow.provider`、`vikiclow.model`）
- `vikiclow.context.tokens`（直方图，属性：`vikiclow.context`、`vikiclow.channel`、`vikiclow.provider`、`vikiclow.model`）

消息流：

- `vikiclow.webhook.received`（计数器，属性：`vikiclow.channel`、`vikiclow.webhook`）
- `vikiclow.webhook.error`（计数器，属性：`vikiclow.channel`、`vikiclow.webhook`）
- `vikiclow.webhook.duration_ms`（直方图，属性：`vikiclow.channel`、`vikiclow.webhook`）
- `vikiclow.message.queued`（计数器，属性：`vikiclow.channel`、`vikiclow.source`）
- `vikiclow.message.processed`（计数器，属性：`vikiclow.channel`、`vikiclow.outcome`）
- `vikiclow.message.duration_ms`（直方图，属性：`vikiclow.channel`、`vikiclow.outcome`）

队列 + 会话：

- `vikiclow.queue.lane.enqueue`（计数器，属性：`vikiclow.lane`）
- `vikiclow.queue.lane.dequeue`（计数器，属性：`vikiclow.lane`）
- `vikiclow.queue.depth`（直方图，属性：`vikiclow.lane` 或 `vikiclow.channel=heartbeat`）
- `vikiclow.queue.wait_ms`（直方图，属性：`vikiclow.lane`）
- `vikiclow.session.state`（计数器，属性：`vikiclow.state`、`vikiclow.reason`）
- `vikiclow.session.stuck`（计数器，属性：`vikiclow.state`）
- `vikiclow.session.stuck_age_ms`（直方图，属性：`vikiclow.state`）
- `vikiclow.run.attempt`（计数器，属性：`vikiclow.attempt`）
...
- `vikiclow.webhook.error`
  - `vikiclow.channel`、`vikiclow.webhook`、`vikiclow.chatId`、`vikiclow.error`
- `vikiclow.message.processed`
  - `vikiclow.channel`、`vikiclow.outcome`、`vikiclow.chatId`、`vikiclow.messageId`、`vikiclow.sessionKey`、`vikiclow.sessionId`、`vikiclow.reason`
- `vikiclow.session.stuck`
  - `vikiclow.state`、`vikiclow.ageMs`、`vikiclow.queueDepth`、`vikiclow.sessionKey`、`vikiclow.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `vikiclow doctor`。

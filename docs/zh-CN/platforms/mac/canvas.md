...
- `~/Library/Application Support/VikiClow/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `vikiclow-canvas://<session>/<path>`

示例：

- `vikiclow-canvas://main/` → `<canvasRoot>/main/index.html`
- `vikiclow-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `vikiclow-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
vikiclow nodes canvas present --node <id>
vikiclow nodes canvas navigate --node <id> --url "/"
vikiclow nodes canvas eval --node <id> --js "document.title"
vikiclow nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__vikiclow__/a2ui/
```
...
vikiclow nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
vikiclow nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "vikiclow://agent?message=Review%20this%20design";
```

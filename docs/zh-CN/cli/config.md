...
read_when:
  - 你想以非交互方式读取或编辑配置
summary: "`vikiclow config` 的 CLI 参考（获取/设置/取消设置配置值）"
title: config
x-i18n:
...
vikiclow config set gateway.port 19001 --json
vikiclow config set channels.whatsapp.groups '["*"]' --json
```

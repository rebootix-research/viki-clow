...
当前版本的 VikiClow 已内置 Feishu 插件，因此通常不需要单独安装。
...
如果您刚安装完 VikiClow，可以直接运行向导，根据提示添加飞书：
...
- `vikiclow gateway status` - 查看网关运行状态
- `vikiclow logs --follow` - 查看实时日志
...
- `vikiclow gateway status` - 查看网关运行状态
- `vikiclow gateway restart` - 重启网关以应用新配置
- `vikiclow logs --follow` - 查看实时日志
...
1. 运行 `vikiclow channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `vikiclow gateway status` 检查状态）
...
```bash
vikiclow pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  vikiclow pairing list feishu      # 查看待审批列表
  vikiclow pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `vikiclow logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `vikiclow logs --follow` 查看日志中的 `open_id`
...
| -------------------------- | ----------------- |
| `vikiclow gateway status`  | 查看网关运行状态  |
| `vikiclow gateway install` | 安装/启动网关服务 |
| `vikiclow gateway stop`    | 停止网关服务      |
| `vikiclow gateway restart` | 重启网关服务      |
| `vikiclow logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`vikiclow gateway status`
6. 查看实时日志：`vikiclow logs --follow`

...
VikiClow supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `VIKICLOW_GATEWAY_TOKEN` (or `VIKICLOWBOT_GATEWAY_TOKEN`) is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, VikiClow runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). VikiClow validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/VikiClow QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/vikiclow"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `vikiclow.json` refs)
...
When reload-time activation fails after a healthy state, VikiClow enters degraded secrets state.
...
- Strict command paths (for example `vikiclow memory` remote-memory paths and `vikiclow qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `vikiclow status`, `vikiclow status --all`, `vikiclow channels status`, `vikiclow channels resolve`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `vikiclow secrets reload`.
...
```bash
vikiclow secrets audit --check
vikiclow secrets configure
vikiclow secrets audit --check
```
...
- plaintext values at rest (`vikiclow.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `vikiclow.json` refs)
...
- lets you select supported secret-bearing fields in `vikiclow.json` plus `auth-profiles.json` for one agent scope
...
- `vikiclow secrets configure --providers-only`
- `vikiclow secrets configure --skip-provider-setup`
- `vikiclow secrets configure --agent <id>`
...
```bash
vikiclow secrets apply --from /tmp/vikiclow-secrets-plan.json
vikiclow secrets apply --from /tmp/vikiclow-secrets-plan.json --dry-run
```
...
VikiClow intentionally does not write rollback backups containing historical plaintext secret values.

...
- `vikiclow gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `vikiclow doctor` reports no blocking config/service issues.
- `vikiclow channels status --probe` shows connected/ready channels.
...
vikiclow config get agents.defaults.models
```
...
vikiclow status
vikiclow channels status --probe
vikiclow pairing list --channel <channel> [--account <id>]
vikiclow config get channels
vikiclow logs --follow
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `vikiclow config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `vikiclow devices list` then `vikiclow devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `vikiclow configure`). If you are running VikiClow via Podman using the dedicated `vikiclow` user, the config lives at `~vikiclow/.vikiclow/vikiclow.json`.
...
```bash
vikiclow cron status
vikiclow cron list
vikiclow cron runs --id <jobId> --limit 20
vikiclow system heartbeat last
vikiclow logs --follow
```
...
vikiclow approvals get --node <idOrNameOrIp>
vikiclow logs --follow
vikiclow status
```
...
vikiclow browser start --browser-profile vikiclow
...
```bash
vikiclow gateway status
vikiclow config get gateway.mode
vikiclow config get gateway.remote.url
vikiclow config get gateway.auth.mode
```
...
```bash
vikiclow config get gateway.bind
vikiclow config get gateway.auth.token
vikiclow gateway status
vikiclow logs --follow
```

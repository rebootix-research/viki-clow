...
- During heartbeat runs, VikiClow treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `vikiclow sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, VikiClow skips the heartbeat run entirely (no model call).
...
```bash
vikiclow system event --text "Check for urgent follow-ups" --mode now
```

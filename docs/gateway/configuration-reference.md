...
description: "Complete field-by-field reference for ~/.vikiclow/vikiclow.json"
summary: "Complete reference for every VikiClow config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.vikiclow/vikiclow.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — VikiClow uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.vikiclow/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `vikiclow doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `vikiclow doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["vikiclow-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-vikiclow",
          requireMention: false,
...
- VikiClow additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `vikiclow plugins install @vikiclow/mattermost`.
...
- `commands.callbackUrl` must resolve to the VikiClow gateway endpoint and be reachable from the Mattermost server.
...
VikiClow spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `vikiclow channels add` (or channel onboarding) while still on a single-account top-level channel config, VikiClow moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `vikiclow doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@vikiclow", "vikiclow"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@vikiclow"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `vikiclow.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, VikiClow auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/vikiclow" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, VikiClow assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        browser: {
          enabled: false,
          image: "vikiclow-sandbox-browser:bookworm-slim",
          network: "vikiclow-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.vikiclow/sandboxes`
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `vikiclow.json`.
noVNC observer access uses VNC auth by default and VikiClow emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `vikiclow-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from VIKICLOW_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `VIKICLOW_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `VIKICLOW_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `VIKICLOW_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        agentDir: "~/.vikiclow/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@vikiclow"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.vikiclow/workspace-home" },
      { id: "work", workspace: "~/.vikiclow/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, VikiClow resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.vikiclow/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.vikiclow/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.vikiclow/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.vikiclow/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, VikiClow starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.vikiclow/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, VikiClow treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:vikiclow`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.vikiclow/attachments/<uuid>/` with a `.manifest.json`.
...
VikiClow uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.vikiclow/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `VIKICLOW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), VikiClow forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `vikiclow onboard --auth-choice opencode-zen` or `vikiclow onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `vikiclow onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `vikiclow onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `vikiclow onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `vikiclow onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `vikiclow onboard --auth-choice minimax-api`.
...
- Loaded from `~/.vikiclow/extensions`, `<workspace>/.vikiclow/extensions`, plus `plugins.load.paths`.
...
- `plugins.installs`: CLI-managed install metadata used by `vikiclow plugins update`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or VIKICLOW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `VIKICLOW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
Convenience flags: `--dev` (uses `~/.vikiclow-dev` + port `19001`), `--profile <name>` (uses `~/.vikiclow-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.vikiclow/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-vikiclow-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `VIKICLOW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.vikiclow/workspace/canvas",
    liveReload: true,
    // enabled: false, // or VIKICLOW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__vikiclow__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__vikiclow__/a2ui/`
...
- Also serves A2UI at `/__vikiclow__/a2ui/`.
...
- Hostname defaults to `vikiclow`. Override with `VIKICLOW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.vikiclow/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `vikiclow dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.vikiclow/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${VIKICLOW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `vikiclow.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.vikiclow/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/vikiclow-vault-resolver",
...
- Legacy OAuth imports from `~/.vikiclow/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/vikiclow/vikiclow.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/vikiclow/vikiclow-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one VikiClow.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `VIKICLOW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `vikiclow doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `vikiclow.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.

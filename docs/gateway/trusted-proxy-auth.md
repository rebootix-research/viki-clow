...
read_when:
  - Running VikiClow behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of VikiClow
...
- You run VikiClow behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. VikiClow checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. VikiClow extracts the user identity from the configured header
...
- VikiClow can stay on loopback HTTP behind the proxy.
...
If VikiClow itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://vikiclow.example.com
    to: http://vikiclow-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
vikiclow.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy vikiclow:18789 {
...
location / {
    ...
    proxy_pass http://vikiclow:18789;
...
`vikiclow security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update VikiClow config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `vikiclow security audit` and review findings

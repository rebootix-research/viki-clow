...
VikiClow integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- VikiClow owns lifecycle when `channels.signal.autoStart=true`.
...
- VikiClow spawns `imsg rpc` as a child process (legacy iMessage integration).

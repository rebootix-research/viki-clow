---
summary: "How VikiClow builds prompt context and reports token usage + costs"
read_when:
...
VikiClow tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

VikiClow assembles its own system prompt on every run. It includes:
...
For images, VikiClow downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from VikiClow session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `vikiclow status --usage` and `vikiclow channels list` show
...
`cacheWrite`. If pricing is missing, VikiClow shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. VikiClow can
...
Anthropic's 1M context window is currently beta-gated. VikiClow can inject the
...
VikiClow skips the `context-1m-*` beta header because Anthropic currently

...
VikiClow features that can generate provider usage or paid API calls.
...
- `vikiclow status --usage` and `vikiclow channels list` show provider **usage windows**
...
VikiClow can pick up credentials from:
...
- `vikiclow status --usage`
- `vikiclow models status --json`
...
`vikiclow models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

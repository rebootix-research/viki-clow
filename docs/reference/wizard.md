...
This is the full reference for the `vikiclow onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.vikiclow/credentials/oauth.json`; auth profiles live in `~/.vikiclow/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.vikiclow/credentials/oauth.json` (or `$VIKICLOW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.vikiclow/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `vikiclow pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `vikiclow configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `vikiclow health`.
    - Tip: `vikiclow status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export VIKICLOW_GATEWAY_TOKEN="your-token"
vikiclow onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env VIKICLOW_GATEWAY_TOKEN
```
...
- Stores it under `~/.vikiclow/tools/signal-cli/<version>/`.
...
Typical fields in `~/.vikiclow/vikiclow.json`:
...
`vikiclow agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.vikiclow/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.vikiclow/agents/<agentId>/sessions/`.

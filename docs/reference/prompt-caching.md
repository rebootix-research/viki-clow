...
- With Anthropic API-key auth profiles, VikiClow seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, VikiClow injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
VikiClow exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.vikiclow/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$VIKICLOW_STATE_DIR/logs/cache-trace.jsonl`
...
- `VIKICLOW_CACHE_TRACE=1` enables cache tracing.
- `VIKICLOW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `VIKICLOW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `VIKICLOW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `VIKICLOW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.

...
This is a living document maintained by the VikiClow community. See [CONTRIBUTING-THREAT-MODEL.md](/security/CONTRIBUTING-THREAT-MODEL) for guidelines on contributing:
...
This threat model documents adversarial threats to the VikiClow AI agent platform and VikiClow Skills Registry skill marketplace, using the MITRE ATLAS framework designed specifically for AI/ML systems.
...
| **Description**         | Attacker scans for exposed VikiClow gateway endpoints                |
...
| **Affected Components** | ~/.vikiclow/credentials/, config storage                    |
...
## 4. VikiClow Skills Registry Supply Chain Analysis
...
/(keepcold131\/VikiDAuthenticatorTool|VikiDAuthenticatorTool)/i
...
| ATLAS ID      | Technique Name                 | VikiClow Threats                                                 |
| ------------- | ------------------------------ | ---------------------------------------------------------------- |
...
_This threat model is a living document. Report security issues to security@vikiclow.ai_

...
- `/usage cost` in chats: local cost summary aggregated from VikiClow session logs.
- CLI: `vikiclow status --usage` prints a full per-provider breakdown.
- CLI: `vikiclow channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

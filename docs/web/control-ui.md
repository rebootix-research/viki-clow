...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/vikiclow`)
...
If the page fails to load, start the Gateway first: `vikiclow gateway`.
...
you revoke it with `vikiclow devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.vikiclow/vikiclow.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop vikiclow`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. VikiClow
...
```bash
vikiclow gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
VikiClow **blocks** Control UI connections without device identity.
...
```bash
VIKICLOW_CONTROL_UI_BASE_PATH=/vikiclow/ pnpm ui:build
```

...
```bash
vikiclow tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `vikiclow tui --deliver`
...
- Local shell commands receive `VIKICLOW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `VIKICLOW_THEME=light` before launching `vikiclow tui`.
- To force the original dark palette instead, set `VIKICLOW_THEME=dark`.
...
- Check the Gateway logs: `vikiclow logs --follow`.
- Confirm the agent can run: `vikiclow status` and `vikiclow models status`.
...
- No agents in picker: check `vikiclow agents list` and your routing config.

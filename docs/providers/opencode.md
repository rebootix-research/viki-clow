---
summary: "Use OpenCode Zen and Go catalogs with VikiClow"
read_when:
...
OpenCode exposes two hosted catalogs in VikiClow:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. VikiClow keeps the runtime provider ids
...
vikiclow onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
vikiclow onboard --auth-choice opencode-go
vikiclow onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```

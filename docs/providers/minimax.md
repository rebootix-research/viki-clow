---
summary: "Use MiniMax M2.5 in VikiClow"
read_when:
  - You want MiniMax models in VikiClow
...
```bash
vikiclow plugins enable minimax-portal-auth  # skip if already loaded.
vikiclow gateway restart  # restart if gateway is already running
...
Configure manually via `vikiclow.json`:
...
- Use `vikiclow models list` and `vikiclow models set minimax/MiniMax-M2.5` to switch.
...
- Running `vikiclow configure` and selecting **MiniMax M2.5**, or

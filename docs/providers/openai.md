---
summary: "Use OpenAI via API keys or Codex subscription in VikiClow"
read_when:
  - You want to use OpenAI models in VikiClow
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like VikiClow.
...
```bash
vikiclow onboard --auth-choice openai-api-key
# or non-interactive
vikiclow onboard --openai-api-key "$OPENAI_API_KEY"
```
...
OpenAI API usage. VikiClow forwards both through the `openai/*` Responses path.
...
vikiclow onboard --auth-choice openai-codex

# Or run OAuth directly
vikiclow models auth login --provider openai-codex
```
...
OpenAI's current Codex docs list `gpt-5.4` as the current Codex model. VikiClow
maps that to `openai-codex/gpt-5.4` for ChatGPT/Codex OAuth usage.

### Transport default

VikiClow uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), VikiClow also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. VikiClow enables it by default for
...
VikiClow, set `agents.defaults.models["openai/<model>"].params.serviceTier` to
...
`baseUrl` on `api.openai.com`), VikiClow now auto-enables OpenAI server-side

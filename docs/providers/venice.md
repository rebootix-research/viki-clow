---
summary: "Use Venice AI privacy-focused models in VikiClow"
read_when:
  - You want privacy-focused inference in VikiClow
...
```bash
vikiclow onboard --auth-choice venice-api-key
```
...
```bash
vikiclow agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, VikiClow shows all available Venice models. Pick based on your needs:
...
```bash
vikiclow models set venice/kimi-k2-5
vikiclow models set venice/claude-opus-4-6
```

List all available models:

```bash
vikiclow models list | grep venice
```

## Configure via `vikiclow configure`

1. Run `vikiclow configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
VikiClow automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
vikiclow agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
vikiclow agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
vikiclow agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
vikiclow agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
vikiclow agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `vikiclow models list` to see currently available models. Some models may be temporarily offline.

...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a VikiClow Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`vikiclow acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the VikiClow gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
vikiclow acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
vikiclow acp --url wss://gateway-host:18789 --token-file ~/.vikiclow/gateway.token

# Attach to an existing session key
vikiclow acp --session agent:main:main

# Attach by label (must already exist)
vikiclow acp --session-label "support inbox"

# Reset the session key before the first prompt
vikiclow acp --session agent:main:main --reset-session
```
...
vikiclow acp client --server-args --url wss://gateway-host:18789 --token-file ~/.vikiclow/gateway.token

# Override the server command (default: vikiclow)
vikiclow acp client --server "node" --server-args vikiclow.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive a VikiClow Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `vikiclow acp` over stdio.

Example config (persisted):

```bash
vikiclow config set gateway.remote.url wss://gateway-host:18789
vikiclow config set gateway.remote.token <token>
```
...
```bash
vikiclow acp --session agent:main:main
vikiclow acp --session agent:design:main
vikiclow acp --session agent:qa:bug-123
```
...
VikiClow bot over ACP, use `acpx` with its built-in `vikiclow` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx vikiclow` at `vikiclow acp`.
3. Target the VikiClow session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default VikiClow ACP session
acpx vikiclow exec "Summarize the active VikiClow session state."

# Persistent named session for follow-up turns
acpx vikiclow sessions ensure --name codex-bridge
acpx vikiclow -s codex-bridge --cwd /path/to/repo \
  "Ask my VikiClow work agent for recent context relevant to this repo."
```

If you want `acpx vikiclow` to target a specific Gateway and session key every
time, override the `vikiclow` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "vikiclow": {
      "command": "env VIKICLOW_HIDE_BANNER=1 VIKICLOW_SUPPRESS_NOTES=1 vikiclow acp --url ws://127.0.0.1:18789 --token-file ~/.vikiclow/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local VikiClow checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env VIKICLOW_HIDE_BANNER=1 VIKICLOW_SUPPRESS_NOTES=1 node vikiclow.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an VikiClow agent without scraping a terminal.
...
In Zed, open the Agent panel and select “VikiClow ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`VIKICLOW_GATEWAY_TOKEN`, `VIKICLOW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`VIKICLOW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `VIKICLOW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `vikiclow acp client` sets `VIKICLOW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `vikiclow`).

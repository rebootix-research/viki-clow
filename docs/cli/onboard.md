---
summary: "CLI reference for `vikiclow onboard` (interactive onboarding wizard)"
read_when:
...
```bash
vikiclow onboard
vikiclow onboard --flow quickstart
vikiclow onboard --flow manual
vikiclow onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `vikiclow dashboard` (Control UI, no channel setup).

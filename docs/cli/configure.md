---
summary: "CLI reference for `vikiclow configure` (interactive configuration prompts)"
read_when:
...
Tip: `vikiclow config` without a subcommand opens the same wizard. Use
`vikiclow config get|set|unset` for non-interactive edits.
...
```bash
vikiclow configure
vikiclow configure --section model --section channels
```

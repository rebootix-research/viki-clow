...
The onboarding wizard is the **recommended** way to set up VikiClow on macOS,
...
`vikiclow dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`vikiclow configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.vikiclow/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `vikiclow doctor` first.
</Note>
...
Use `vikiclow agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.vikiclow/workspace-<agentId>`.
...
- CLI command reference: [`vikiclow onboard`](/cli/onboard)

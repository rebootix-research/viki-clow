...
def test_write_gallery_normal_output():
    with tempfile.TemporaryDirectory() as tmpdir:
        out = Path(tmpdir)
        items = [
            {"prompt": "a VikiClow field agent, golden hour", "file": "001-viki-agent.png"},
            {"prompt": "a cozy reading nook", "file": "002-nook.png"},
        ]
        write_gallery(out, items)
        html = (out / "index.html").read_text()
        assert "a VikiClow field agent, golden hour" in html
        assert 'src="001-viki-agent.png"' in html

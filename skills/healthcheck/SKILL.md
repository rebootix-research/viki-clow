---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for VikiClow deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, VikiClow cron scheduling for periodic checks, or version status checks on a machine running VikiClow (laptop, workstation, Pi, VPS).
---

# VikiClow Host Hardening

## Overview

Assess and harden the host running VikiClow, then align it to a user-defined risk tolerance without breaking access. Use VikiClow security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim VikiClow changes the host firewall, SSH, or OS updates; it does not.
...
5. VikiClow gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if VikiClow can access sensitive data.
...
### 2) Run VikiClow security audits (read-only)

As part of the default read-only checks, run `vikiclow security audit --deep`. Only offer alternatives if the user requests them:

1. `vikiclow security audit` (faster, non-probing)
2. `vikiclow security audit --json` (structured output)

Offer to apply VikiClow safe defaults (numbered):

1. `vikiclow security audit --fix`

Be explicit that `--fix` only tightens VikiClow defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check VikiClow version/update status (read-only)

As part of the default read-only checks, run `vikiclow update status`.
...
- Credential hygiene notes (location of VikiClow creds, prefer disk encryption)
...
- VikiClow security audit (re-run)
...
After VikiClow install or first hardening pass, run at least one baseline audit and version check:

- `vikiclow security audit`
- `vikiclow security audit --deep`
- `vikiclow update status`

Ongoing monitoring is recommended. Use the VikiClow cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `vikiclow cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `vikiclow update status`
...
Before creating, `vikiclow cron list` and match on exact `name`. If found, `vikiclow cron edit <id> ...`.
If not found, `vikiclow cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `vikiclow update status` (preferred for source checkouts and channels)
2. `npm view vikiclow version` (published npm version)

## VikiClow command accuracy

Use only supported commands and flags:

- `vikiclow security audit [--deep] [--fix] [--json]`
- `vikiclow status` / `vikiclow status --deep`
- `vikiclow health --json`
- `vikiclow update status`
- `vikiclow cron add|list|runs|run`

Do not invent CLI flags or imply VikiClow enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by VikiClow compaction:

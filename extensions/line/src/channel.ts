...
} from "vikiclow/plugin-sdk/compat";
import {
...
const lineConfigBase = createScopedChannelConfigBase<ResolvedLineAccount, VikiClowConfig>({
  sectionKey: "line",
...
const resolveLineDmPolicy = createScopedDmSecurityResolver<ResolvedLineAccount>({
  ...
  approveHint: "vikiclow pairing approve line <code>",
...
export const linePlugin: ChannelPlugin<ResolvedLineAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      ...
      await line.pushMessageLine(id, "VikiClow: your access has been approved.", {
  ...
  gateway: {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      const nextCfg = { ...cfg } as VikiClowConfig;
      const lineConfig = (cfg.channels?.line ?? {}) as LineConfig;

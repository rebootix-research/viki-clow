...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["vikiclowPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["vikiclow", "pollId"]) ??
    readNestedString(value, ["vikiclow", "poll", "id"]) ??
    readNestedString(value, ["data", "vikiclowPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "vikiclow", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          vikiclowPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "vikiclow poll vote",
          displayText: "Vote recorded",
          value: { vikiclowPollId: pollId, pollId },
        },
      },
    ],
  };

...
function resolveVikiClowRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.VIKICLOW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set VIKICLOW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveVikiClowRoot(), "dist", "extensionAPI.js");

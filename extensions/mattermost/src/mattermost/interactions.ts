...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__vikiclow_channel_id";
...
type InteractionCallbackConfig = Pick<VikiClowConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "vikiclow-mattermost-interactions").update(botToken).digest("hex");
}

...
import type { VikiClowPluginApi, VikiClowPluginService } from "vikiclow/plugin-sdk/phone-control";
...
function normalizeDenyList(cfg: VikiClowPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: VikiClowPluginApi["config"]): string[] {
...
export default function register(api: VikiClowPluginApi) {
  ...
  const timerService: VikiClowPluginService = {

...
import type { VikiClowConfig, PluginRuntime } from "vikiclow/plugin-sdk/bluebubbles";
...
function createConfig(overrides?: Record<string, unknown>): VikiClowConfig {
  return {
    channels: {
      bluebubbles: {
        ...overrides,
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-bb-media-"));
  tempDirs.push(dir);
  return dir;
}

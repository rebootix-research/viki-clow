...
vi.mock("vikiclow/plugin-sdk/diagnostics-otel", async () => {
  const actual = await vi.importActual<typeof import("vikiclow/plugin-sdk/diagnostics-otel")>(
    "vikiclow/plugin-sdk/diagnostics-otel",
  );
  return {
    ...actual,
...
import type { VikiClowPluginServiceContext } from "vikiclow/plugin-sdk/diagnostics-otel";
import { emitDiagnosticEvent } from "vikiclow/plugin-sdk/diagnostics-otel";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/vikiclow-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): VikiClowPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("vikiclow.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("vikiclow.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("vikiclow.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("vikiclow.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("vikiclow.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("vikiclow.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("vikiclow.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("vikiclow.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("vikiclow.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("vikiclow.webhook.processed");
    expect(spanNames).toContain("vikiclow.message.processed");
    expect(spanNames).toContain("vikiclow.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["vikiclow.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("vikiclow.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "vikiclow.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["vikiclow.reason"]).toBe("string");
    expect(String(attrs?.["vikiclow.reason"])).not.toContain(

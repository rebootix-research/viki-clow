# Google Gemini CLI Auth (VikiClow plugin)
...
```bash
vikiclow plugins enable google-gemini-cli-auth
```
...
```bash
vikiclow models auth login --provider google-gemini-cli --set-default
```
...
- `VIKICLOW_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `VIKICLOW_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`

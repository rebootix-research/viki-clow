...
function resolveGatewayPort(cfg: VikiClowPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.VIKICLOW_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.VIKICLOWBOT_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
function resolveAuth(cfg: VikiClowPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: VikiClowPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the VikiClow iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the VikiClow iOS app:",
            "",
            "```",
            qrAscii,
            "```",

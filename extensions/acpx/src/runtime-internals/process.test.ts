...
async function createTempDir(): Promise<string> {
  const dir = await mkdtemp(path.join(tmpdir(), "vikiclow-acpx-process-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("spawnAndCollect", () => {
  ...
  it("strips shared provider auth env vars from spawned acpx children", async () => {
    ...
    vi.stubEnv("VIKICLOW_API_KEY", "keep-me");

    const result = await spawnAndCollect({
      command: process.execPath,
      args: [
        "-e",
        "process.stdout.write(JSON.stringify({openai:process.env.OPENAI_API_KEY,github:process.env.GITHUB_TOKEN,hf:process.env.HF_TOKEN,vikiclow:process.env.VIKICLOW_API_KEY,shell:process.env.VIKICLOW_SHELL}))",
      ],
      cwd: process.cwd(),
    ...
    expect(parsed.vikiclow).toBe("keep-me");
  ...
  it("strips provider auth env vars case-insensitively", async () => {
    ...
    const result = await spawnAndCollect({
      ...
      args: [
        "-e",
        "process.stdout.write(JSON.stringify({openai:process.env.OpenAI_Api_Key,github:process.env.Github_Token,vikiclow:process.env.VIKICLOW_API_KEY,shell:process.env.VIKICLOW_SHELL}))",
      ],
      cwd: process.cwd(),
      stripProviderAuthEnvVars: true,
    });

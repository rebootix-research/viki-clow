...
Read-only diff viewer plugin for **VikiClow** agents.
...
Set plugin-wide defaults in `~/.vikiclow/vikiclow.json`:
...
Before:
VikiClow supports plugins.

After:
VikiClow supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/vikiclow-diffs`).

...
import type { VikiClowPluginApi } from "vikiclow/plugin-sdk/diffs";
...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("vikiclow-diffs-tool-"));
  });
...
function createApi(): VikiClowPluginApi {
  return {
    id: "diffs",
    name: "Diffs",
    description: "Diffs",
    ...
    runtime: {} as VikiClowPluginApi["runtime"],
    logger: {
      info() {},
      warn() {},
      error() {},

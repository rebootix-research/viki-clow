/**
 * Twitch channel plugin for VikiClow.
 *
...
import { buildChannelConfigSchema } from "vikiclow/plugin-sdk/twitch";
 ...
 * for VikiClow. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: VikiClowConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: VikiClowConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: VikiClowConfig): boolean => {

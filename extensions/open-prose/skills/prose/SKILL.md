...
metadata: { "vikiclow": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == VikiClow `sessions_spawn`
- **File I/O** == VikiClow `read`/`write`
- **Remote fetch** == VikiClow `web_fetch` (or `exec` with curl when POST is required)

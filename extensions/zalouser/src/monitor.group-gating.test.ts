import type { VikiClowConfig, PluginRuntime, RuntimeEnv } from "vikiclow/plugin-sdk/zalouser";
...
function createConfig(): VikiClowConfig {
  return {
    channels: {
      zalouser: {
        enabled: true,
    ...
    channel: {
      ...
      groups: {
        resolveRequireMention: vi.fn((input) => {
          const cfg = input.cfg as VikiClowConfig;

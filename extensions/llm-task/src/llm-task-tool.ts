...
import Ajv from "ajv";
import { resolvePreferredVikiClowTmpDir } from "vikiclow/plugin-sdk/llm-task";
// NOTE: This extension is intended to be bundled with VikiClow.
// When running from source (tests/dev), VikiClow internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { VikiClowPluginApi } from "vikiclow/plugin-sdk/llm-task";
...
export function createLlmTaskTool(api: VikiClowPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Viki Workflow pipelines via vikiclow.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredVikiClowTmpDir(), "vikiclow-llm-task-"),
        );

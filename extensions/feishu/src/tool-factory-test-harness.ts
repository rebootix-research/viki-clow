import type { AnyAgentTool, VikiClowPluginApi } from "vikiclow/plugin-sdk/feishu";
...
export function createToolFactoryHarness(cfg: VikiClowPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<VikiClowPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},

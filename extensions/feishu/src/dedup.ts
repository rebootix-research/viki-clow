...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  const stateOverride = env.VIKICLOW_STATE_DIR?.trim() || env.VIKICLOWBOT_STATE_DIR?.trim();
  if (stateOverride) {
    return stateOverride;
  }
  if (env.VITEST || env.NODE_ENV === "test") {
    return path.join(os.tmpdir(), ["vikiclow-vitest", String(process.pid)].join("-"));
  }
  return path.join(os.homedir(), ".vikiclow");
}

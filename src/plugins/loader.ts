...
import { resolveVikiClowPackageRootSync } from "../infra/vikiclow-root.js";
...
import { discoverVikiClowPlugins } from "./discovery.js";
...
function listPluginSdkExportedSubpaths(params: { modulePath?: string } = {}): string[] {
  const modulePath = params.modulePath ?? fileURLToPath(import.meta.url);
  const packageRoot = resolveVikiClowPackageRootSync({
...
const resolvePluginSdkScopedAliasMap = (): Record<string, string> => {
  ...
  for (const subpath of listPluginSdkExportedSubpaths()) {
    ...
    if (resolved) {
      aliasMap[`vikiclow/plugin-sdk/${subpath}`] = resolved;
    }
  }
  return aliasMap;
};
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: VikiClowPluginDefinition;
  register?: VikiClowPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as VikiClowPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as VikiClowPluginDefinition;
...
export function loadVikiClowPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverVikiClowPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    const aliasMap = {
      ...(pluginSdkAlias ? { "vikiclow/plugin-sdk": pluginSdkAlias } : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: VikiClowPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as VikiClowPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,

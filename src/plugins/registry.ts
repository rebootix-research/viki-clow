...
import type {
  VikiClowPluginApi,
  VikiClowPluginChannelRegistration,
  VikiClowPluginCliRegistrar,
  VikiClowPluginCommandDefinition,
  VikiClowPluginHttpRouteAuth,
  VikiClowPluginHttpRouteMatch,
  VikiClowPluginHttpRouteHandler,
  VikiClowPluginHttpRouteParams,
  VikiClowPluginHookOptions,
  ProviderPlugin,
...
export type PluginToolRegistration = {
  pluginId: string;
  factory: VikiClowPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
};
...
export type PluginCliRegistration = {
  pluginId: string;
  register: VikiClowPluginCliRegistrar;
  commands: string[];
  source: string;
};
...
export type PluginServiceRegistration = {
  pluginId: string;
  service: VikiClowPluginService;
  source: string;
};

export type PluginCommandRegistration = {
  pluginId: string;
  command: VikiClowPluginCommandDefinition;
  source: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: VikiClowPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: VikiClowPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: VikiClowPluginHookOptions | undefined,
    config: VikiClowPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: VikiClowPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: VikiClowPluginChannelRegistration | ChannelPlugin,
  ) => {
    const normalized =
      typeof (registration as VikiClowPluginChannelRegistration).plugin === "object"
        ? (registration as VikiClowPluginChannelRegistration)
  ...
  const registerCli = (
    record: PluginRecord,
    registrar: VikiClowPluginCliRegistrar,
  ...
  const registerService = (record: PluginRecord, service: VikiClowPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: VikiClowPluginCommandDefinition) => {

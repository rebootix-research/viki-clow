...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing vikiclow.extensions; update the plugin package to include vikiclow.extensions (for example ["./dist/index.js"]). See https://docs.vikiclow.ai/help/troubleshooting#plugin-install-fails-with-missing-vikiclow-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_VIKICLOW_EXTENSIONS: "missing_vikiclow_extensions",
  EMPTY_VIKICLOW_EXTENSIONS: "empty_vikiclow_extensions",
...
function ensureVikiClowExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_VIKICLOW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json vikiclow.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_VIKICLOW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureVikiClowExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from vikiclow.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-vikiclow"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "vikiclow security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "vikiclow security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "vikiclow-npm-pack-",
    spec,
    timeoutMs,

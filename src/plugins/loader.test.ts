...
const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.VIKICLOW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "vikiclow.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadVikiClowPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          vikiclow: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadVikiClowPlugins>) {
...
function useNoBundledPlugins() {
  process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadVikiClowPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.VIKICLOW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadVikiClowPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadVikiClowPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadVikiClowPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadVikiClowPlugins(options);
  ...
  it("skips importing bundled memory plugins that are disabled by memory slot", () => {
    ...
    fs.writeFileSync(
      path.join(memoryADir, "vikiclow.plugin.json"),
      JSON.stringify(
        {
          id: "memory-a",
          kind: "memory",
    ...
    fs.writeFileSync(
      path.join(memoryBDir, "vikiclow.plugin.json"),
      JSON.stringify(
        {
          id: "memory-b",
          kind: "memory",
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ VIKICLOW_STATE_DIR: stateDir, VIKICLOWBOT_STATE_DIR: undefined }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("vikiclow/plugin-sdk").emptyPluginConfigSchema)(),
  register() {},
};`,
    });
    ...
    const script = `
      import { loadVikiClowPlugins } from ${JSON.stringify(loaderModuleUrl)};
      const registry = loadVikiClowPlugins({
        cache: false,
    ...
    execFileSync(process.execPath, ["--import", "tsx", "--input-type=module", "-e", script], {
      ...
      env: {
        ...process.env,
        VIKICLOW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
      },
      encoding: "utf-8",
      stdio: "pipe",
    });

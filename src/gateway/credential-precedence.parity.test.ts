...
const gatewayEnv = {
  VIKICLOW_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  VIKICLOW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): VikiClowConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  const keys = [
    ...
    "VIKICLOW_SERVICE_KIND",
    "VIKICLOWBOT_GATEWAY_TOKEN",
    "VIKICLOWBOT_GATEWAY_PASSWORD",
  ] as const;
...
describe("gateway credential precedence parity", () => {
  const cases: TestCase[] = [
    ...
    },
    {
      ...
      env: {
        VIKICLOWBOT_GATEWAY_TOKEN: "legacy-token", // pragma: allowlist secret

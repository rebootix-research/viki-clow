...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-vikiclow-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-vikiclow-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-vikiclow-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-vikiclow-thread-id")?.trim() || undefined;
  ...
  const allTools = createVikiClowTools({

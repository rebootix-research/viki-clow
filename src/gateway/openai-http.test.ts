    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "vikiclow", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-vikiclow-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-vikiclow-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "vikiclow", messages: [{ role: "user", content: "hi" }] },
          {
            "x-vikiclow-agent-id": "beta",
            "x-vikiclow-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from VikiClow.");
      }

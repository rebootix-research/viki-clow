...
describe("resolveGatewayConnectionAuth", () => {
  const cases: ConnectionAuthCase[] = [
    ...
    {
      name: "includeLegacyEnv controls VIKICLOWBOT fallback",
      cfg: cfg({
        gateway: {
          mode: "local",
          auth: {},
  ...
  it("resolves config-first token SecretRef even when VIKICLOW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when VIKICLOW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {

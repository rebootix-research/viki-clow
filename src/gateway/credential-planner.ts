...
type GatewaySecretDefaults = NonNullable<VikiClowConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${VIKICLOW_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(
  ...
  return readGatewayEnv(
    env,
    ["VIKICLOW_GATEWAY_TOKEN", "VIKICLOWBOT_GATEWAY_TOKEN"],
    includeLegacyEnv,
  );
}
...
export function readGatewayPasswordEnv(
  ...
  return readGatewayEnv(
    env,
    ["VIKICLOW_GATEWAY_PASSWORD", "VIKICLOWBOT_GATEWAY_PASSWORD"],
    includeLegacyEnv,
  );
}

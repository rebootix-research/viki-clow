...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.VIKICLOW_GATEWAY_URL) ??
      trimToUndefined(process.env.VIKICLOWBOT_GATEWAY_URL));
  ...
  const allowPrivateWs = process.env.VIKICLOW_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: vikiclow doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__VIKICLOW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;

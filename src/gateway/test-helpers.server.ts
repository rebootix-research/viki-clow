...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "VIKICLOW_BUNDLED_PLUGINS_DIR",
  "VIKICLOW_SKIP_CHANNELS",
  "VIKICLOW_SKIP_PROVIDERS",
  "VIKICLOW_SKIP_CRON",
  "VIKICLOW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.VIKICLOW_STATE_DIR = path.join(tempHome, ".vikiclow");
...
function applyGatewaySkipEnv() {
  ...
  process.env.VIKICLOW_SKIP_PROVIDERS = "1";
  process.env.VIKICLOW_SKIP_CRON = "1";
  process.env.VIKICLOW_TEST_MINIMAL_GATEWAY = "1";
  process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "vikiclow-test-no-bundled-extensions")
    : "vikiclow-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".vikiclow-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".vikiclow-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__vikiclowTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__vikiclowTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["VIKICLOW_GATEWAY_TOKEN"]);
  const prev = process.env.VIKICLOW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.VIKICLOW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.VIKICLOW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

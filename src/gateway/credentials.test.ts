...
function cfg(input: Partial<VikiClowConfig>): VikiClowConfig {
  return input as VikiClowConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<VikiClowConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  VIKICLOW_GATEWAY_TOKEN: "env-token",
  VIKICLOW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy VIKICLOWBOT env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        ...
        VIKICLOWBOT_GATEWAY_PASSWORD: "legacy-password", // pragma: allowlist secret
      } as NodeJS.ProcessEnv,
      includeLegacyEnv: false,
    });
...
describe("resolveGatewayCredentialsFromValues", () => {
  ...
  it("rejects unresolved env var placeholders in config credentials", () => {
    const resolved = resolveGatewayCredentialsFromValues({
      configToken: "${VIKICLOW_GATEWAY_TOKEN}",
      configPassword: "${VIKICLOW_GATEWAY_PASSWORD}",
      env: {} as NodeJS.ProcessEnv,
      tokenPrecedence: "config-first",
      passwordPrecedence: "config-first", // pragma: allowlist secret
    });

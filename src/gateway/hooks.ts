...
export function resolveHooksConfig(cfg: VikiClowConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: VikiClowConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-vikiclow-token"] === "string"
      ? req.headers["x-vikiclow-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }

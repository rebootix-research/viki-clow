...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("vikiclow doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with VIKICLOW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.VIKICLOW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://vikiclow-gateway.ai:18789",
      onConnectError,
    });

...
const { createVikiClowTools } = await import("../agents/vikiclow-tools.js");
...
type SessionSendTool = ReturnType<typeof createVikiClowTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createVikiClowTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["VIKICLOW_GATEWAY_PORT", "VIKICLOW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.VIKICLOW_GATEWAY_PORT = String(gatewayPort);
  process.env.VIKICLOW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("VIKICLOW_CONFIG_PATH missing in gateway test environment");
      }

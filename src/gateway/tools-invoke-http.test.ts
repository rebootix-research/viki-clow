...
let lastCreateVikiClowToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/vikiclow-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateVikiClowToolsContext?.agentTo,
          agentThreadId: lastCreateVikiClowToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createVikiClowTools: (ctx: Record<string, unknown>) => {
      lastCreateVikiClowToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateVikiClowToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateVikiClowToolsContext?.allowMediaInvokeCommands).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-vikiclow-message-to": "channel:24514",
        "x-vikiclow-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });

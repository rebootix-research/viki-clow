...
function resolveGatewayTokenFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return env.VIKICLOW_GATEWAY_TOKEN?.trim() || env.VIKICLOWBOT_GATEWAY_TOKEN?.trim() || undefined;
}

function resolveGatewayPasswordFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    env.VIKICLOW_GATEWAY_PASSWORD?.trim() || env.VIKICLOWBOT_GATEWAY_PASSWORD?.trim() || undefined
  );
}

function resolveAuth(cfg: VikiClowConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
...
async function resolveGatewayTokenSecretRef(
  ...
  if (mode !== "token") {
    const hasPasswordEnvCandidate = Boolean(
      env.VIKICLOW_GATEWAY_PASSWORD?.trim() || env.VIKICLOWBOT_GATEWAY_PASSWORD?.trim(),
    );

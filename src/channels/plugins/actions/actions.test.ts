...
function telegramCfg(): VikiClowConfig {
  return { channels: { telegram: { botToken: "tok" } } } as VikiClowConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: VikiClowConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as VikiClowConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as VikiClowConfig;
...
function createSignalAccountOverrideCfg(): VikiClowConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as VikiClowConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as VikiClowConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as VikiClowConfig,
        accountId: undefined,
        params: {

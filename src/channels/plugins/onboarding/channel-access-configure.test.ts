...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: VikiClowConfig, policy: ChannelAccessPolicy) => VikiClowConfig;
  resolveAllowlist: (params: { cfg: VikiClowConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: VikiClowConfig; resolved: TResolved }) => VikiClowConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: VikiClowConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: VikiClowConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: VikiClowConfig, policy: ChannelAccessPolicy): VikiClowConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: VikiClowConfig, policy: ChannelAccessPolicy): VikiClowConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: VikiClowConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: VikiClowConfig; resolved: string[] }) => {
      calls.push("apply");

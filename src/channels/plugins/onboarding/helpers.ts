...
export function setTopLevelChannelDmPolicyWithAllowFrom(params: {
  ...
  channel: string;
  dmPolicy: DmPolicy;
  getAllowFrom?: (cfg: VikiClowConfig) => Array<string | number> | undefined;
}): VikiClowConfig {
  const channelConfig =
    (params.cfg.channels?.[params.channel] as Record<string, unknown> | undefined) ?? {};
...
export async function runSingleChannelSecretStep(params: {
  ...
  applyUseEnv?: (cfg: VikiClowConfig) => VikiClowConfig | Promise<VikiClowConfig>;
  applySet?: (
    cfg: VikiClowConfig,
    value: SecretInput,
    resolvedValue: string,
  ) => VikiClowConfig | Promise<VikiClowConfig>;
}): Promise<{
  cfg: VikiClowConfig;
...
export async function promptSingleChannelSecretInput(params: {
  ...
  const selectedMode = await resolveSecretInputModeForEnvSelection({
    ...
    copy: {
      ...
      plaintextHint: "Stores the credential directly in VikiClow config",
  ...
  const resolved = await promptSecretRefForOnboarding({
    ...
    copy: {
      ...
      envVarPlaceholder: params.preferredEnvVar ?? "VIKICLOW_SECRET",
      envVarFormatError:
        'Use an env var name like "VIKICLOW_SECRET" (uppercase letters, numbers, underscores).',
      noProvidersMessage:

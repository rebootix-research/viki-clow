...
export function resolveVikiClowUserDataDir(profileName = DEFAULT_VIKICLOW_BROWSER_PROFILE_NAME) {
...
export async function launchVikiClowChrome(
  ...
  const userDataDir = resolveVikiClowUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_VIKICLOW_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateVikiClowProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`VikiClow browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`vikiclow browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`vikiclow browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `VikiClow browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopVikiClowChrome(

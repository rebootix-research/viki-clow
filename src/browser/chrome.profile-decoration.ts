...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".vikiclow-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + signature Viki teal). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateVikiClowProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_VIKICLOW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_VIKICLOW_BROWSER_COLOR).toUpperCase();

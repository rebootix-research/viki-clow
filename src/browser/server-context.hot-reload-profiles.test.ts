...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only vikiclow profile
    ...
    // 2. Simulate adding a new profile to config (like user editing vikiclow.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles.vikiclow = { cdpPort: 19999, color: "#11847E" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles.vikiclow?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", async () => {
    const cfg = loadConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const vikiclowProfile = resolveProfile(resolved, "vikiclow");
    expect(vikiclowProfile).toBeTruthy();
    ...
    const runtime = state.profiles.get("vikiclow");

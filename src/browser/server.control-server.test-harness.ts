...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.VIKICLOW_GATEWAY_PORT;
    return;
  }
  process.env.VIKICLOW_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/vikiclow" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          vikiclow: { cdpPort: state.testPort + 1, color: "#11847E" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchVikiClowChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveVikiClowUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopVikiClowChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.VIKICLOW_GATEWAY_PORT;
  process.env.VIKICLOW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.VIKICLOW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.VIKICLOW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.VIKICLOW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.VIKICLOW_GATEWAY_PASSWORD;
  } else {
    process.env.VIKICLOW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}

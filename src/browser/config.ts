import type { BrowserConfig, BrowserProfileConfig, VikiClowConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_VIKICLOW_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_VIKICLOW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "vikiclow" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_VIKICLOW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_VIKICLOW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an VikiClow browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for vikiclow/vikiclow CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: VikiClowConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_VIKICLOW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_VIKICLOW_BROWSER_PROFILE_NAME]
        ? DEFAULT_VIKICLOW_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "vikiclow";

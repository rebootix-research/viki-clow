...
import {
  probeAuthenticatedVikiClowRelay,
...
async function probeRelay(baseUrl: string, relayAuthToken: string): Promise<boolean> {
  return await probeAuthenticatedVikiClowRelay({
    baseUrl,
    relayAuthHeader: "x-vikiclow-relay-token",
    relayAuthToken,
  });
}
...
describe("extension-relay-auth", () => {
  ...
  it("accepts authenticated vikiclow relay probe responses", async () => {

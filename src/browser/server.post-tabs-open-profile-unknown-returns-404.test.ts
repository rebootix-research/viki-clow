...
describe("Viki Browser control server", () => {
  installBrowserControlServerHooks();
...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    await startBrowserControlServerFromConfig();
    const base = getBrowserControlServerBaseUrl();
    const remoteName = `browser-remote-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const badRemoteName = `browser-bad-remote-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const badExtensionName = `browser-bad-extension-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "vikiclow" }),
    });
    ...
    const createRemote = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: remoteName, cdpUrl: "http://10.0.0.42:9222" }),
    });
    ...
    expect(createRemoteBody.profile).toBe(remoteName);
    ...
    const createBadRemote = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: badRemoteName, cdpUrl: "ftp://bad" }),
    });
    ...
    const deleteRemote = await realFetch(`${base}/profiles/${encodeURIComponent(remoteName)}`, {
      method: "DELETE",
    });
    expect(deleteRemote.status).toBe(200);
    const deleteRemoteBody = (await deleteRemote.json()) as {
      ok: boolean;
      profile?: string;
      deleted?: boolean;
    };
    expect(deleteRemoteBody.ok).toBe(true);
    expect(deleteRemoteBody.profile).toBe(remoteName);

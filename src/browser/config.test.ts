...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and the signature coral color", () => {
    const resolved = resolveBrowserConfig(undefined);
    expect(resolved.enabled).toBe(true);
    expect(resolved.controlPort).toBe(18791);
    expect(resolved.color).toBe("#11847E");
    expect(shouldStartLocalBrowserServer(resolved)).toBe(true);
    expect(resolved.cdpHost).toBe("127.0.0.1");
    expect(resolved.cdpProtocol).toBe("http");
    const profile = resolveProfile(resolved, resolved.defaultProfile);
    expect(profile?.name).toBe("vikiclow");
    expect(profile?.driver).toBe("vikiclow");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const vikiclow = resolveProfile(resolved, "vikiclow");
    expect(vikiclow?.driver).toBe("vikiclow");
    expect(vikiclow?.cdpPort).toBe(18800);
    expect(vikiclow?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from VIKICLOW_GATEWAY_PORT when unset", () => {
    withEnv({ VIKICLOW_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const vikiclow = resolveProfile(resolved, "vikiclow");
      expect(vikiclow?.cdpPort).toBe(19012);
      expect(vikiclow?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ VIKICLOW_GATEWAY_PORT: undefined }, () => {
      ...
      const vikiclow = resolveProfile(resolved, "vikiclow");
      expect(vikiclow?.cdpPort).toBe(19022);
      expect(vikiclow?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(vikiclow?.cdpPort).toBe(19000);
    expect(vikiclow?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "vikiclow");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        vikiclow: { cdpPort: 18792, color: "#11847E" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("vikiclow");
  });
  ...
  describe("default profile preference", () => {
    it("defaults to vikiclow profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
    ...
    it("keeps vikiclow default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("vikiclow");
    ...
    it("keeps vikiclow default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("vikiclow");
    ...
    it("keeps vikiclow default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });

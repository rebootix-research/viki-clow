...
vi.mock("./chrome.js", () => ({
  resolveVikiClowUserDataDir: vi.fn(() => "/tmp/vikiclow-test/vikiclow/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveVikiClowUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "vikiclow-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveVikiClowUserDataDir).mockReturnValue(userDataDir);

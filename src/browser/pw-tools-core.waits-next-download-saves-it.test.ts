...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredVikiClowTmpDir: vi.fn(() => "/tmp/vikiclow"),
}));
vi.mock("../infra/tmp-vikiclow-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredVikiClowTmpDir.mockReturnValue("/tmp/vikiclow");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".vikiclow-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredVikiClowTmpDir.mockReturnValue("/tmp/vikiclow-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "vikiclow-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "vikiclow-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredVikiClowTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "vikiclow-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "vikiclow-preferred", "downloads")}${path.sep}`),
    );
  });

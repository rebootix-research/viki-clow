...
describe("extension-relay-auth SecretRef handling", () => {
  const ENV_KEYS = ["VIKICLOW_GATEWAY_TOKEN", "VIKICLOWBOT_GATEWAY_TOKEN", "CUSTOM_GATEWAY_TOKEN"];
  ...
  it("resolves file-backed gateway.auth.token SecretRef", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-relay-file-secret-"));

...
const { stopVikiClowChromeMock, stopChromeExtensionRelayServerMock } = vi.hoisted(() => ({
  stopVikiClowChromeMock: vi.fn(async () => {}),
...
vi.mock("./chrome.js", () => ({
  stopVikiClowChrome: stopVikiClowChromeMock,
}));
...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("starts relay only for extension profiles", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "vikiclow", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  beforeEach(() => {
    ...
    stopVikiClowChromeMock.mockClear();
    stopChromeExtensionRelayServerMock.mockClear();
  });

  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["vikiclow", "chrome"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      vikiclow: vi.fn(async () => {}),
      chrome: vi.fn(async () => {
    ...
    expect(stopMap.vikiclow).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopVikiClowChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("vikiclow browser stop failed: Error: oops");
  });
});

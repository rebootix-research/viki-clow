...
const RELAY_AUTH_HEADER = "x-vikiclow-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/VikiClow-Extension-Relay",
            revision: "0",
            userAgent: "VikiClow-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "VikiClow/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedVikiClowRelay({

  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          vikiclow: { cdpPort: testPort + 1, color: "#11847E" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.VIKICLOW_GATEWAY_PORT;
    process.env.VIKICLOW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.VIKICLOW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.VIKICLOW_GATEWAY_PASSWORD;

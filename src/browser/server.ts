...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.error(`vikiclow browser server failed to bind 127.0.0.1:${port}: ${String(err)}`);
    return null;
  });
  ...
  logServer.info(`Viki Browser control listening on http://127.0.0.1:${port}/ (auth=${authMode})`);
  return state;
}

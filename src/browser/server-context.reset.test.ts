...
function localVikiClowProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "vikiclow",
...
function createLocalVikiClowResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localVikiClowProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveVikiClowUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-reset-"));
    const profileDir = path.join(tempRoot, "vikiclow");
    ...
    const ops = createLocalVikiClowResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveVikiClowUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-reset-no-own-"));
    const profileDir = path.join(tempRoot, "vikiclow");
    fs.mkdirSync(profileDir, { recursive: true });

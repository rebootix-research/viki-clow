...
function withFakeCli(versionOutput: string): { root: string; cliPath: string } {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-install-sh-"));
  const cliPath = path.join(root, "vikiclow");
...
VIKICLOW_BIN="$FAKE_VIKICLOW_BIN"
resolve_vikiclow_version`,
    ],
    {
      ...
      env: {
        ...process.env,
        FAKE_VIKICLOW_BIN: cliPath,
        VIKICLOW_INSTALL_SH_NO_RUN: "1",
      },
    },
  );
  return output.trim();
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("VikiClow 2026.3.9 (abcdef0)");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const fixture = withFakeCli("VikiClow dev's build");
      tempRoots.push(fixture.root);

      expect(resolveVersionFromInstaller(fixture.cliPath)).toBe("VikiClow dev's build");
    },
  );
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      ...
      const hostileCwd = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-install-stdin-"));

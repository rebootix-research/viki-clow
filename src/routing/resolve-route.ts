...
function listAgents(cfg: VikiClowConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: VikiClowConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<VikiClowConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: VikiClowConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: VikiClowConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: VikiClowConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<VikiClowConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  VikiClowConfig,
  {
    bindingsRef: VikiClowConfig["bindings"];
    agentsRef: VikiClowConfig["agents"];
    sessionRef: VikiClowConfig["session"];
...
function resolveRouteCacheForConfig(cfg: VikiClowConfig): Map<string, ResolvedAgentRoute> {

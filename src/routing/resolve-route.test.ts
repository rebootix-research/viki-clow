...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: VikiClowConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: VikiClowConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/vikiclow-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<VikiClowConfig["bindings"]>[number];

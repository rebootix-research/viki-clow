...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".vikiclow");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.VIKICLOW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$VIKICLOW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via VIKICLOW_STATE_DIR.

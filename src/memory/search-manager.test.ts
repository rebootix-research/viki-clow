...
import { readLatestMemoryPersistenceProof } from "./persistence-proof.js";
...
function createQmdCfg(agentId: string): VikiClowConfig {
  return {
...
describe("getMemorySearchManager caching", () => {
  ...
  it("bootstraps a durable memory proof on manager acquisition and sync", async () => {
    await withTempHome(async () => {
      const cfg = createQmdCfg("proof-agent");
      const result = await getMemorySearchManager({ cfg, agentId: "proof-agent" });
      const manager = requireManager(result);

      const bootstrapProof = await readLatestMemoryPersistenceProof({ agentId: "proof-agent" });
      expect(bootstrapProof?.result).toBe("ready");
      expect(bootstrapProof?.proofPath).toContain("proof-agent");

      await manager.sync?.({ reason: "manual" });

      const syncedProof = await readLatestMemoryPersistenceProof({ agentId: "proof-agent" });
      expect(syncedProof?.result).toBe("completed");
      expect(syncedProof?.syncReason).toBe("manual");
    });
  });
});

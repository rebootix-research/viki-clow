...
export type ResolvedMemoryBackendConfig = {
  ...
  graphiti?: ResolvedGraphitiConfig;
};
...
const DEFAULT_GRAPHITI_TIMEOUT_MS = 5_000;
const DEFAULT_QMD_MCPORTER: ResolvedQmdMcporterConfig = {
  enabled: false,
  serverName: "qmd",
  startDaemon: true,
};
const GRAPHITI_ENV_VARS = [
  "VIKICLOW_GRAPHITI_NEO4J_URI",
  "VIKICLOW_GRAPHITI_NEO4J_USER",
  "VIKICLOW_GRAPHITI_NEO4J_PASSWORD",
  "VIKICLOW_GRAPHITI_NEO4J_DATABASE",
  "VIKICLOW_GRAPHITI_LOCAL_STORE",
  "VIKICLOW_GRAPHITI_TIMEOUT_MS",
] as const;
...
function resolveGraphitiConfig(
  raw: MemoryGraphitiConfig | undefined,
  workspaceDir: string,
  ...
  return {
    enabled: env.VIKICLOW_MEMORY_BACKEND?.trim() === "graphiti" ? true : raw?.enabled !== false,
    neo4jUri: env.VIKICLOW_GRAPHITI_NEO4J_URI?.trim() || raw?.neo4jUri?.trim() || undefined,
    neo4jUser: env.VIKICLOW_GRAPHITI_NEO4J_USER?.trim() || raw?.neo4jUser?.trim() || undefined,
    neo4jPassword:
      env.VIKICLOW_GRAPHITI_NEO4J_PASSWORD?.trim() || raw?.neo4jPassword?.trim() || undefined,
    neo4jDatabase:
      env.VIKICLOW_GRAPHITI_NEO4J_DATABASE?.trim() || raw?.neo4jDatabase?.trim() || undefined,
    localStore: localStoreRaw
      ? resolvePath(localStoreRaw, workspaceDir)
      : path.join(workspaceDir, "graphiti"),
    connectionTimeoutMs,
  };
}

function hasGraphitiEnvOverride(env: NodeJS.ProcessEnv): boolean {
  return GRAPHITI_ENV_VARS.some((key) => {
    const value = env[key];
...
export function resolveMemoryBackendConfig(params: {
  ...
  const envBackendRaw = env.VIKICLOW_MEMORY_BACKEND?.trim();
  const envBackend =
    envBackendRaw === "builtin" || envBackendRaw === "qmd" || envBackendRaw === "graphiti"
      ? envBackendRaw
      : undefined;
  const backend =
    envBackend ??
    (hasGraphitiEnvOverride(env) ? "graphiti" : (params.cfg.memory?.backend ?? DEFAULT_BACKEND));
  const citationsRaw = env.VIKICLOW_MEMORY_CITATIONS?.trim();
  const citations =
    citationsRaw === "auto" || citationsRaw === "on" || citationsRaw === "off"
      ? citationsRaw
      : (params.cfg.memory?.citations ?? DEFAULT_CITATIONS);
  if (backend !== "qmd") {
    if (backend === "graphiti") {
      ...
      return {
        backend: "graphiti",
        citations,
        graphiti: resolveGraphitiConfig(params.cfg.memory?.graphiti, workspaceDir, env),
      };
    }

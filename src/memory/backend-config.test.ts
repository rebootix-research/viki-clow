...
describe("resolveMemoryBackendConfig", () => {
  it("defaults to builtin backend when config missing", () => {
    const cfg = { agents: { defaults: { workspace: "/tmp/memory-test" } } } as VikiClowConfig;
  ...
  it("promotes graphiti when graph env overrides are present", () => {
    const cfg = {
      agents: { defaults: { workspace: "/tmp/memory-test" } },
    } as VikiClowConfig;
    const resolved = resolveMemoryBackendConfig({
      ...
      env: {
        ...process.env,
        VIKICLOW_GRAPHITI_NEO4J_URI: "bolt://127.0.0.1:7687",
        VIKICLOW_GRAPHITI_NEO4J_USER: "neo4j",
        VIKICLOW_GRAPHITI_NEO4J_PASSWORD: "vikiclow-dev-password",
      },
    });
    expect(resolved.backend).toBe("graphiti");
    expect(resolved.graphiti?.neo4jUri).toBe("bolt://127.0.0.1:7687");
    expect(resolved.graphiti?.neo4jUser).toBe("neo4j");
  });

  it("lets env override the configured graphiti store and timeout", () => {
    const cfg = {
      agents: { defaults: { workspace: "/tmp/memory-test" } },
      memory: {
        backend: "graphiti",
        graphiti: {
          localStore: "/tmp/memory-test/graphiti",
    ...
    const resolved = resolveMemoryBackendConfig({
      ...
      env: {
        ...
        VIKICLOW_GRAPHITI_LOCAL_STORE: "/tmp/custom-graphiti-store",

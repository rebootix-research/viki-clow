...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.vikiclowCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.vikiclowCanvasA2UIAction &&
        typeof window.vikiclowCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.vikiclowSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("vikiclow:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android VikiClow node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.vikiclowSendUserAction === "function"
        ? window.vikiclowSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.VIKICLOW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.VIKICLOW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>VikiClow Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

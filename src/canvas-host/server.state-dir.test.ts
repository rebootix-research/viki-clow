...
describe("canvas host state dir defaults", () => {
  it("uses VIKICLOW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("vikiclow-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("VikiClow Canvas");
      } finally {
        await handler.close();
      }
    });

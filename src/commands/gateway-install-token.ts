...
import { readConfigFileSnapshot, writeConfigFile, type VikiClowConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("vikiclow config set gateway.auth.mode token")} or ${formatCliCommand("vikiclow config set gateway.auth.mode password")}.`,
  ].join(" ");
}

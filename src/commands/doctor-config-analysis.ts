...
import { VikiClowSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: VikiClowConfig): {
  config: VikiClowConfig;
  removed: string[];
} {
  const parsed = VikiClowSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: VikiClowConfig): void {

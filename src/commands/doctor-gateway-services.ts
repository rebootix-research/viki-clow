...
import { writeConfigFile, type VikiClowConfig } from "../config/config.js";
...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (tokenRefConfigured && serviceToken) {
    audit.issues.push({
      ...
      message:
        "Gateway service VIKICLOW_GATEWAY_TOKEN should be unset when gateway.auth.token is SecretRef-managed",
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "Remove legacy pre-VikiClow gateway services now?",
      initialValue: true,
    });
    if (shouldRemove) {
      const removed: string[] = [];
      ...
      if (removed.length > 0) {
        runtime.log("Legacy gateway services removed. Installing VikiClow gateway next.");
      }
    }
  }

...
export function requireVikiClowAgentDir(): string {
  const agentDir = process.env.VIKICLOW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("VIKICLOW_AGENT_DIR not set");
  }
  return agentDir;
}

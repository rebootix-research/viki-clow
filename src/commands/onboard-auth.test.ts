...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under VIKICLOW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("vikiclow-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-oauth-sync-"));
    ...
    process.env.VIKICLOW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-oauth-nosync-"));
    process.env.VIKICLOW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside VIKICLOW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-oauth-external-"));
    process.env.VIKICLOW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* VIKICLOW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to VIKICLOW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("vikiclow-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);

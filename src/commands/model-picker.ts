...
type PromptDefaultModelResult = { model?: string; config?: VikiClowConfig };
...
function resolveConfiguredModelRaw(cfg: VikiClowConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: VikiClowConfig): string[] {
...
export function applyPrimaryModel(cfg: VikiClowConfig, model: string): VikiClowConfig {
...
export function applyModelAllowlist(cfg: VikiClowConfig, models: string[]): VikiClowConfig {
  const defaults = cfg.agents?.defaults;

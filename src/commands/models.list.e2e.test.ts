...
const ensureVikiClowModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveVikiClowAgentDir = vi.fn().mockReturnValue("/tmp/vikiclow-agent");
...
vi.mock("../agents/models-config.js", () => ({
  ensureVikiClowModelsJson,
}));

vi.mock("../agents/agent-paths.js", () => ({
  resolveVikiClowAgentDir,
}));
...
beforeEach(() => {
  ...
  ensureVikiClowModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureVikiClowModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureVikiClowModelsJson).toHaveBeenCalled();
    expect(ensureVikiClowModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });

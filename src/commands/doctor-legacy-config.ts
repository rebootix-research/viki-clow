...
export function normalizeCompatibilityConfigValues(cfg: VikiClowConfig): {
  config: VikiClowConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: VikiClowConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as VikiClowConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as VikiClowConfig["browser"],
    };
    changes.push(

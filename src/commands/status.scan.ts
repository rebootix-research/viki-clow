...
function resolveMemoryPluginStatus(cfg: VikiClowConfig): MemoryPluginStatus {
  ...
  return await withProgress(
    ...
    async (progress) => {
      ...
      const channels = await buildChannelsTable(cfg, {
        ...
        // Set `VIKICLOWBOT_SHOW_SECRETS=0` to force redaction.
        showSecrets: process.env.VIKICLOWBOT_SHOW_SECRETS?.trim() !== "0",
        sourceConfig: loadedRaw,
      });
      progress.tick();

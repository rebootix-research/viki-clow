...
const SEARCH_TOOL = "https://docs.vikiclow.ai/mcp.SearchVikiClow";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.vikiclow.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('vikiclow docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.vikiclow.ai/");
      runtime.log(`Search: ${formatCliCommand('vikiclow docs "your query"')}`);
    }
    return;
  }

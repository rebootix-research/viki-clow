...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByVikiClow ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByVikiClow ? "installed · " : "";
  ...
  runtime.log(theme.heading("VikiClow status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`vikiclow devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("vikiclow devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("vikiclow devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("vikiclow security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("vikiclow security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.vikiclow.ai/faq");
  runtime.log("Troubleshooting: https://docs.vikiclow.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("vikiclow status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("vikiclow logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("vikiclow status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("vikiclow gateway probe")}`);
  }
}

...
function resolveMode(cfg: VikiClowConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("VikiClow doctor");

  const root = await resolveVikiClowPackageRoot({
  ...
  let cfg: VikiClowConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("vikiclow configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("vikiclow config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("vikiclow setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("vikiclow config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("vikiclow config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("vikiclow doctor --fix")}" to apply changes.`);
  }

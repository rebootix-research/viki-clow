...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".vikiclow", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: VikiClowConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const deprecatedLaunchctlEntries = [
    ["VIKICLOWBOT_GATEWAY_TOKEN", await getenv("VIKICLOWBOT_GATEWAY_TOKEN")],
    ["VIKICLOWBOT_GATEWAY_PASSWORD", await getenv("VIKICLOWBOT_GATEWAY_PASSWORD")],
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`VIKICLOW_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["VIKICLOW_GATEWAY_TOKEN", await getenv("VIKICLOW_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["VIKICLOW_GATEWAY_PASSWORD", await getenv("VIKICLOW_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "VIKICLOW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const entries = Object.entries(env)
    .filter(([key, value]) => key.startsWith("VIKICLOWBOT_") && value?.trim())
    .map(([key]) => key);
  if (entries.length === 0) {
    return;
  }
  ...
  const lines = [
    ...
    "- Use VIKICLOW_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> VIKICLOW_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.VIKICLOW_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- VIKICLOW_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }
  ...
  const suggestions = [
    ...
    "  export NODE_COMPILE_CACHE=/var/tmp/vikiclow-compile-cache",
    "  mkdir -p /var/tmp/vikiclow-compile-cache",
    "  export VIKICLOW_NO_RESPAWN=1",

...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: VikiClowConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: VikiClowConfig) => { changed: boolean; next: VikiClowConfig };
  defaultModel: string;
  overrideConfig: VikiClowConfig;
  alreadyDefaultConfig: VikiClowConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: VikiClowConfig) => config,
      applyDefaultConfig: (config: VikiClowConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: VikiClowConfig = { agents: { defaults: {} } };

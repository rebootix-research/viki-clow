...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "VIKICLOW_GATEWAY_TOKEN",
          placeholder: "VIKICLOW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like VIKICLOW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. VikiClow will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: VikiClow trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.vikiclow.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );

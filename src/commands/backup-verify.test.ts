...
describe("backupVerifyCommand", () => {
  ...
  beforeEach(async () => {
    tempHome = await createTempHomeEnv("vikiclow-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".vikiclow");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "vikiclow.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-no-manifest-"));
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-vikiclow-backup";
      ...
      const manifest = {
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.vikiclow",
            archivePath: `${rootName}/payload/posix/tmp/.vikiclow`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-traversal-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
  ...
  it("fails when archive paths contain backslashes", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-backslash-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    const payloadPath = path.join(tempDir, "payload.txt");
    try {
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".vikiclow");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-workspace-"));
    const configPath = path.join(tempHome.home, "custom-config.json");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-verify-out-"));
    try {
      process.env.VIKICLOW_CONFIG_PATH = configPath;
      await fs.writeFile(
        configPath,
        JSON.stringify({
          agents: {
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-duplicate-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    const payloadPath = path.join(tempDir, "payload.txt");
    try {
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.vikiclow",
            archivePath: `${rootName}/payload/posix/tmp/.vikiclow/payload.txt`,
          },
        ],
      };
      await fs.writeFile(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`, "utf8");
      ...
      await tar.c(
        {
          ...
          onWriteEntry: (entry) => {
            ...
            if (entry.path === payloadPath) {
              entry.path = `${rootName}/payload/posix/tmp/.vikiclow/payload.txt`;
            }
          },
        },
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-backup-duplicate-payload-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    const manifestPath = path.join(tempDir, "manifest.json");
    ...
    try {
      const rootName = "2026-03-09T00-00-00.000Z-vikiclow-backup";
      const payloadArchivePath = `${rootName}/payload/posix/tmp/.vikiclow/payload.txt`;
      const manifest = {
        schemaVersion: 1,
        createdAt: "2026-03-09T00:00:00.000Z",
        archiveRoot: rootName,

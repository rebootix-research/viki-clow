...
function prependInternalEventContext(
  ...
  if (body.includes("VikiClow runtime context (internal):")) {
    return body;
  }
...
function formatCapabilityPlanPrompt(
  plan: Awaited<ReturnType<typeof ensureCapabilitiesForObjective>>,
): string | undefined {
  const lines = ["Capability Preflight"];
...
async function deliverMissionTerminalNotice(params: {
  ...
  if (params.opts.deliver !== true) {
    return;
  }
  const payload = normalizeReplyPayload({ text: params.text });
  if (!payload) {
    return;
  }
  try {
    ...
    log.warn(
      `Mission terminal notice delivery failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
async function prepareAgentCommandExecution(
  ...
  if (agentIdOverride) {
    ...
    if (!knownAgents.includes(agentIdOverride)) {
      throw new Error(
        `Unknown agent id "${agentIdOverrideRaw}". Use "${formatCliCommand("vikiclow agents list")}" to see configured agents.`,
      );
    }
  }
...
async function agentCommandInternal(
  ...
  const missionId = opts.missionId?.trim() || runId;
  const missionTracker = await beginMissionRun({
    missionId,
    objective: body,
    runId,
    agentId: sessionAgentId,
    ...
    channel: opts.replyChannel ?? opts.channel,
    to: opts.replyTo ?? opts.to,
    accountId: opts.replyAccountId ?? opts.accountId,
    threadId: opts.threadId,
    replayRequest: createMissionReplayRequest(opts, missionId),
  });
  opts.missionId = missionTracker.missionId;
  const capabilityPlan = await ensureCapabilitiesForObjective({
    objective: body,
    workspaceDir,
    autoInstall: true,
  });
  await missionTracker.recordCapabilityPlan(capabilityPlan);
  opts.extraSystemPrompt = combineExtraSystemPrompts(
    missionTracker.buildExecutionPrompt(),
    formatCapabilityPlanPrompt(capabilityPlan),
    opts.extraSystemPrompt,
  );
  const unsubscribeMissionEvents = onAgentEvent((evt) => {
    if (evt.runId !== runId) {
      return;
    }
    void missionTracker.onEvent(evt).catch((error) => {
      log.warn(
        `Mission tracker update failed for ${missionTracker.missionId}: ${error instanceof Error ? error.message : String(error)}`,
      );
    });
  });
  try {
    ...
    if (acpResolution?.kind === "ready" && sessionKey) {
      ...
      registerAgentRunContext(runId, {
        sessionKey,
        missionId: missionTracker.missionId,
      });
      emitAgentEvent({
        runId,
        stream: "lifecycle",
      ...
      let payloads = normalizedFinalPayload ? [normalizedFinalPayload] : [];
      if (payloads.length === 0) {
        const noticeStatus =
          missionTracker.status === "needs_approval"
            ? "needs_approval"
            : missionTracker.status === "blocked"
              ? "blocked"
              : "completed";
      ...
      await missionTracker.finalizeCompleted({
        replyText: finalText,
    ...
    const payloadEvidenceText = payloads.length > 0 ? JSON.stringify(payloads, null, 2) : undefined;
    await missionTracker.finalizeCompleted({
      replyText: payloadEvidenceText,
      sessionFile,
      aborted: result.meta.aborted ?? false,
    });
    return deliveryResult;
    ...
    const missionRecord = await missionTracker.finalizeFailed(err);
    await deliverMissionTerminalNotice({
      cfg,
      deps,
      runtime,
      opts,
      outboundSession,
      sessionEntry,
      text: missionRecord.terminalMessage ?? errorText,
    });
    throw err;
  } finally {

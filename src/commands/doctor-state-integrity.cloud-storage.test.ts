...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "VikiClow", ".vikiclow");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/vikiclow-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".vikiclow");
  ...
  it("anchors cloud detection to OS homedir when VIKICLOW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".vikiclow");
    const originalVikiClowHome = process.env.VIKICLOW_HOME;
    process.env.VIKICLOW_HOME = "/tmp/vikiclow-home-override";
    ...
    try {
      ...
      if (originalVikiClowHome === undefined) {
        delete process.env.VIKICLOW_HOME;
      } else {
        process.env.VIKICLOW_HOME = originalVikiClowHome;
      }
    }
  });

...
function resolveAgentName(cfg: VikiClowConfig, agentId: string) {
...
function resolveAgentModel(cfg: VikiClowConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: VikiClowConfig): AgentSummary[] {

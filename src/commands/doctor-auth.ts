...
function pruneAuthProfiles(
  cfg: VikiClowConfig,
  profileIds: Set<string>,
): { next: VikiClowConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("vikiclow models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  ...
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("vikiclow models auth setup-token")} or ${formatCliCommand(
      "vikiclow configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "vikiclow models auth login --provider openai-codex",
    )} or ${formatCliCommand("vikiclow configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("vikiclow configure")}\` or \`${formatCliCommand("vikiclow onboard")}\`.`;
}

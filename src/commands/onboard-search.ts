...
function rawKeyValue(config: VikiClowConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: VikiClowConfig, provider: SearchProvider): boolean {
...
function applyProviderOnly(config: VikiClowConfig, provider: SearchProvider): VikiClowConfig {
  return {
    ...config,
    tools: {
      ...config.tools,
...
function preserveDisabledState(original: VikiClowConfig, result: VikiClowConfig): VikiClowConfig {
...
export async function setupSearch(
  ...
  const choice = await prompter.select<PickerValue>({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with vikiclow configure --section web",
      },
    ],
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — VikiClow will store a reference instead of the API key.",

...
async function runGatewayHealthCheck(params: {
  ...
  const token =
    process.env.VIKICLOW_GATEWAY_TOKEN ?? process.env.VIKICLOWBOT_GATEWAY_TOKEN ?? configuredToken;
  const password =
    process.env.VIKICLOW_GATEWAY_PASSWORD ??
    process.env.VIKICLOWBOT_GATEWAY_PASSWORD ??
    configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from vikiclow.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "VikiClow update wizard" : "VikiClow configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: VikiClowConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("vikiclow doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      ...
      token:
        ...
        process.env.VIKICLOWBOT_GATEWAY_TOKEN ??
        baseLocalProbeToken,
      password:
        process.env.VIKICLOW_GATEWAY_PASSWORD ??
        process.env.VIKICLOWBOT_GATEWAY_PASSWORD ??

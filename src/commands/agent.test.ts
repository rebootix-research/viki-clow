...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "vikiclow-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<VikiClowConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<VikiClowConfig["channels"]>["telegram"]>>,
  ...
  configSpy.mockReturnValue({
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "vikiclow"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as VikiClowConfig },
    writeOptions: {},
...
describe("agentCommand", () => {
  ...
  it("persists a durable mission record for agent runs", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);

      await agentCommand({ message: "Ship the browser flow", to: "+1555" }, runtime);

      const missions = await listMissionRecords();
      expect(missions.length).toBeGreaterThan(0);
      expect(missions[0]?.objective).toContain("Ship the browser flow");

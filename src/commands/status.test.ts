...
beforeAll(() => {
  envSnapshot = captureEnv(["VIKICLOW_PROFILE"]);
  process.env.VIKICLOW_PROFILE = "isolated";
});
...
vi.mock("../infra/vikiclow-root.js", () => ({
  resolveVikiClowPackageRoot: vi.fn().mockResolvedValue("/tmp/vikiclow"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/vikiclow/pnpm-lock.yaml",
      markerPath: "/tmp/vikiclow/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.vikiclow.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.vikiclow.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("vikiclow status --all") ||
          line.includes("vikiclow --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("VIKICLOW_GATEWAY_TOKEN", "abcd1234", async () => {

...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveVikiClowAgentDir: vi.fn().mockReturnValue("/tmp/vikiclow-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/vikiclow-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/vikiclow-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveVikiClowAgentDir: mocks.resolveVikiClowAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/vikiclow-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveVikiClowAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/vikiclow-dev/vikiclow.json");
    expect(payload.auth.storePath).toBe("/tmp/vikiclow-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/vikiclow-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/vikiclow-agent-custom");

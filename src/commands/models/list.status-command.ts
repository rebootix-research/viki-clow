...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveVikiClowAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("vikiclow models auth setup-token")}\` or \`${formatCliCommand("vikiclow configure")}\`.`
          : `Run \`${formatCliCommand("vikiclow configure")}\` or set an API key env var.`;

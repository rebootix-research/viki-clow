...
export async function loadValidConfigOrThrow(): Promise<VikiClowConfig> {
...
export async function updateConfig(
  mutator: (cfg: VikiClowConfig) => VikiClowConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: VikiClowConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: VikiClowConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("vikiclow agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

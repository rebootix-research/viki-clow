...
describe("readServiceStatusSummary", () => {
  it("marks VikiClow-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["vikiclow", "gateway", "run"] })),
    ...
    expect(summary.managedByVikiClow).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByVikiClow).toBe(false);

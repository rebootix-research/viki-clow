...
vi.mock("../daemon/service-audit.js", () => ({
  ...
  readEmbeddedGatewayToken: (
    ...
    command?.environmentValueSources?.VIKICLOW_GATEWAY_TOKEN === "file"
      ? undefined
      : command?.environment?.VIKICLOW_GATEWAY_TOKEN?.trim() || undefined,
  SERVICE_AUDIT_CODES: {
...
function setupGatewayTokenRepairScenario() {
  mocks.readCommand.mockResolvedValue({
    ...
    environment: {
      VIKICLOW_GATEWAY_TOKEN: "stale-token",
    },
  });
    ...
    issues: [
      {
        ...
        message: "Gateway service VIKICLOW_GATEWAY_TOKEN does not match gateway.auth.token",
        level: "recommended",
      },
    ],
  });
...
describe("maybeRepairGatewayServiceConfig", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.resolveGatewayAuthTokenForService.mockImplementation(async (cfg: VikiClowConfig, env) => {
      const configToken =
        typeof cfg.gateway?.auth?.token === "string" ? cfg.gateway.auth.token.trim() : undefined;
      const envToken = env.VIKICLOW_GATEWAY_TOKEN?.trim() || undefined;
  ...
  it("uses VIKICLOW_GATEWAY_TOKEN when config token is missing", async () => {
    await withEnvAsync({ VIKICLOW_GATEWAY_TOKEN: "env-token" }, async () => {
      setupGatewayTokenRepairScenario();
    ...
    await withEnvAsync(
      {
        ...
        VIKICLOWBOT_GATEWAY_TOKEN: undefined,
      },
      async () => {
        setupGatewayTokenRepairScenario();
    ...
    await withEnvAsync(
      ...
      async () => {
        mocks.readCommand.mockResolvedValue({
          programArguments: gatewayProgramArguments,
          environment: {
            VIKICLOW_GATEWAY_TOKEN: "env-file-token",
          },
...
describe("maybeScanExtraGatewayServices", () => {
  ...
  it("removes legacy Linux user systemd services", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Legacy gateway services removed. Installing VikiClow gateway next.",
    );
  });
});

...
import type { VikiClowConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: VikiClowConfig): string[] {
...
export async function noteSecurityWarnings(cfg: VikiClowConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("vikiclow security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.vikiclow/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("vikiclow approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.vikiclow.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("vikiclow configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("vikiclow config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("vikiclow doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("vikiclow config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('vikiclow config set session.dmScope "per-channel-peer"') +

...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update VikiClow and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["vikiclow update", "Update a source checkout (git)"],
        ["vikiclow update --channel beta", "Switch to beta channel (git + npm)"],
        ["vikiclow update --channel dev", "Switch to dev channel (git + npm)"],
        ["vikiclow update --tag beta", "One-off update to a dist-tag or version"],
        ["vikiclow update --dry-run", "Preview actions without changing anything"],
        ["vikiclow update --no-restart", "Update without restarting the service"],
        ["vikiclow update --json", "Output result as JSON"],
        ["vikiclow update --yes", "Non-interactive (accept downgrade prompts)"],
        ["vikiclow update wizard", "Interactive update wizard"],
        ["vikiclow --update", "Shorthand for vikiclow update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run vikiclow update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.vikiclow.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vikiclow update status", "Show channel + version status."],
          ["vikiclow update status --json", "JSON output."],
          ["vikiclow update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.vikiclow.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

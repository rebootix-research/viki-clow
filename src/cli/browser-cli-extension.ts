...
export function registerBrowserExtensionCommands(
  ...
  const ext = browser.command("extension").description("Viki Browser Relay extension helpers");

  ext
    .command("install")
    .description("Install the Viki Browser Relay extension to a stable local path")
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin Viki Browser Relay, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.vikiclow.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
  ...
  ext
    .command("path")
    .description("Print the path to the installed Viki Browser Relay extension (load unpacked)")
    .action(async (_opts, cmd) => {
      const parent = parentOpts(cmd);
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Viki Browser Relay extension is not installed. Run: "${formatCliCommand("vikiclow browser extension install")}"`,
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.vikiclow.ai/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);

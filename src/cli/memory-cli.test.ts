...
const readLatestMemoryPersistenceProof = vi.fn<() => Promise<unknown>>(async () => null);
const readLatestMemoryWritebackSummary = vi.fn<() => Promise<unknown>>(async () => null);
...
vi.mock("../memory/persistence-proof.js", () => ({
  readLatestMemoryPersistenceProof,
  readLatestMemoryWritebackSummary,
  summarizeMemoryPersistenceProof: (value: unknown) => value,
  summarizeMemoryWritebackSummary: (value: unknown) => value,
}));
...
describe("memory cli", () => {
  ...
  function makeMemoryProof(overrides: Record<string, unknown> = {}) {
    return {
      version: 1 as const,
      result: "completed" as const,
      recordedAt: "2026-03-24T00:00:00.000Z",
      lineageId: "lineage-123",
      proofPath: "/tmp/.vikiclow/memory/proof/main/manifest.json",
      eventsPath: "/tmp/.vikiclow/memory/proof/main/events.jsonl",
      agentId: "main",
      backend: "builtin",
      provider: "none",
      status: {
  ...
  function makeMemoryWriteback(overrides: Record<string, unknown> = {}) {
    return {
  ...
  function setupMemoryStatusWithInactiveSecretDiagnostics(close: ReturnType<typeof vi.fn>) {
    ...
    mockManager({
      ...
      status: () => makeMemoryStatus({ workspaceDir: "/workspace" }),
      close,
    });
  }
  ...
  it("prints persistence proof status when available", async () => {
    const close = vi.fn(async () => {});
    readLatestMemoryPersistenceProof.mockResolvedValueOnce(makeMemoryProof());
    readLatestMemoryWritebackSummary.mockResolvedValueOnce(makeMemoryWriteback());
    mockManager({
      probeVectorAvailability: vi.fn(async () => true),
      status: () => makeMemoryStatus({ workspaceDir: "/workspace", dbPath: "/tmp/memory.sqlite" }),
      close,
    });
    ...
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Persistence: completed"));
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Lineage: lineage-123"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Latest writeback: /workspace/memory/2026-03-24.md"),
    );
    expect(close).toHaveBeenCalled();
  });
  ...
  it("documents memory help examples", () => {
    ...
    expect(helpText).toContain("vikiclow memory status --deep");
    expect(helpText).toContain("Probe embedding provider readiness.");
    expect(helpText).toContain("vikiclow memory proof");
    expect(helpText).toContain("Show the durable persistence proof trail.");
    expect(helpText).toContain("vikiclow memory graphiti status");
    expect(helpText).toContain("Inspect the Graphiti/Neo4j backbone proof.");
    expect(helpText).toContain("vikiclow memory graphiti search mission");
    expect(helpText).toContain("Search the graph backbone ledger.");
    expect(helpText).toContain('vikiclow memory search "meeting notes"');
    expect(helpText).toContain("Quick search using positional query.");
    expect(helpText).toContain('vikiclow memory search --query "deployment" --max-results 20');
  ...
  it("prints durable memory proof details", async () => {
    const close = vi.fn(async () => {});
    readLatestMemoryPersistenceProof.mockResolvedValueOnce(makeMemoryProof());
    readLatestMemoryWritebackSummary.mockResolvedValueOnce(makeMemoryWriteback());
    mockManager({
    ...
    const log = spyRuntimeLogs();
    await runMemoryCli(["proof"]);

    expect(log).toHaveBeenCalledWith(expect.stringContaining("Memory Persistence"));
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Status: completed"));
    expect(log).toHaveBeenCalledWith(expect.stringContaining("Events:"));
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Latest writeback: /workspace/memory/2026-03-24.md"),
    );
    expect(close).toHaveBeenCalled();
  ...
  it("prints status json output when requested", async () => {
    ...
    expect((payload[0] as Record<string, unknown>)?.latestWriteback).toBeDefined();
    expect(close).toHaveBeenCalled();
  });

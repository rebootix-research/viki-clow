...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("VIKICLOW_GATEWAY_TOKEN", "");
    vi.stubEnv("VIKICLOWBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("VIKICLOW_GATEWAY_PASSWORD", "");
    vi.stubEnv("VIKICLOWBOT_GATEWAY_PASSWORD", "");
  });
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("vikiclow devices approve <requestId>");
  });
  ...
  it("uses VIKICLOW_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("VIKICLOW_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),

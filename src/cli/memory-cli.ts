...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  const allResults: Array<{
    ...
    persistenceProof?: Awaited<ReturnType<typeof readLatestMemoryPersistenceProof>>;
    latestWriteback?: Awaited<ReturnType<typeof readLatestMemoryWritebackSummary>>;
  }> = [];
  ...
  for (const result of allResults) {
    ...
    const proof = summarizeMemoryPersistenceProof(result.persistenceProof ?? null);
    ...
    if (proof) {
      lines.push(
        `${label("Persistence")} ${success(proof.result)} ${muted(
          `(${shortenHomePath(proof.proofPath)})`,
        )}`,
      );
    ...
    const writeback = summarizeMemoryWritebackSummary(result.latestWriteback ?? null);
    if (writeback) {
      lines.push(
        `${label("Latest writeback")} ${info(shortenHomePath(writeback.path))} ${muted(
          `(${writeback.size} bytes)`,
        )}`,
      );
      lines.push(`${label("Writeback recorded")} ${info(writeback.recordedAt)}`);
    }
    if (status.batch) {
...
export async function runMemoryProof(opts: MemoryCommandOptions) {
  setVerbose(Boolean(opts.verbose));
  const { config: cfg, diagnostics } = await loadMemoryCommandConfig("memory proof");
  emitMemorySecretResolveDiagnostics(diagnostics, { json: Boolean(opts.json) });
  const agentIds = resolveAgentIds(cfg, opts.agent);
  ...
  for (const agentId of agentIds) {
    await withMemoryManagerForAgent({
      ...
      run: async (manager) => {
        ...
        const proof = summarizeMemoryPersistenceProof(persistenceProof);
        const writeback = summarizeMemoryWritebackSummary(latestWriteback);
        const rich = isRich();
        const heading = (text: string) => colorize(rich, theme.heading, text);
        const muted = (text: string) => colorize(rich, theme.muted, text);
        const info = (text: string) => colorize(rich, theme.info, text);
        ...
        const warn = (text: string) => colorize(rich, theme.warn, text);
        const label = (text: string) => muted(`${text}:`);
        const lines = [`${heading("Memory Persistence")} ${muted(`(${agentId})`)}`];
        if (!proof) {
          lines.push(`${label("Status")} ${warn("no durable proof yet")}`);
        } else {
          lines.push(
            `${label("Status")} ${success(proof.result)} ${muted(
              `(${shortenHomePath(proof.proofPath)})`,
            )}`,
          );
          lines.push(`${label("Lineage")} ${info(proof.lineageId)}`);
          ...
          lines.push(`${label("Events")} ${info(shortenHomePath(proof.eventsPath))}`);
        }
        if (writeback) {
          lines.push(
            `${label("Latest writeback")} ${info(shortenHomePath(writeback.path))} ${muted(
...
async function withGraphitiManagerForAgent(params: {
  cfg: ReturnType<typeof loadConfig>;
  agentId: string;
  run: (manager: GraphitiBackboneManager) => Promise<void>;
}): Promise<void> {
  const resolved = resolveMemoryBackendConfig({
    cfg: params.cfg,
    agentId: params.agentId,
  });
  const delegate = await getMemorySearchManager({
    cfg: params.cfg,
    agentId: params.agentId,
    purpose: "status",
  });
  ...
  await withManager<GraphitiBackboneManager>({
    getManager: async () => ({ manager }),
    onMissing: (error) => defaultRuntime.log(error ?? "Graphiti memory disabled."),
    onCloseError: (err) =>
      defaultRuntime.error(`Graphiti manager close failed: ${formatErrorMessage(err)}`),
    close: async (graphitiManager) => {
      await graphitiManager.close?.();
    },
    run: params.run,
  });
}

export async function runGraphitiBackboneStatus(opts: MemoryCommandOptions) {
  setVerbose(Boolean(opts.verbose));
  const { config: cfg, diagnostics } = await loadMemoryCommandConfig("memory graphiti status");
  emitMemorySecretResolveDiagnostics(diagnostics, { json: Boolean(opts.json) });
  const agentIds = resolveAgentIds(cfg, opts.agent);

  for (const agentId of agentIds) {
    await withGraphitiManagerForAgent({
      cfg,
      agentId,
      run: async (manager) => {
        const status = manager.status();
        const proof = await readLatestGraphitiBackboneProof({
          agentId,
          config: resolveMemoryBackendConfig({ cfg, agentId }).graphiti,
        });
        if (opts.json) {
          defaultRuntime.log(JSON.stringify({ agentId, status, proof }, null, 2));
          return;
        }
        const graphitiProof = summarizeGraphitiBackboneProof(proof ?? null);
        const rich = isRich();
        const heading = (text: string) => colorize(rich, theme.heading, text);
        const muted = (text: string) => colorize(rich, theme.muted, text);
        const info = (text: string) => colorize(rich, theme.info, text);
        ...
        if (graphitiProof) {
          ...
          if (graphitiProof.latestMissionId) {
            lines.push(`${label("Latest mission")} ${info(graphitiProof.latestMissionId)}`);
          }
          if (graphitiProof.latestWritebackRelativePath) {
            lines.push(
              `${label("Latest writeback")} ${info(graphitiProof.latestWritebackRelativePath)}`,
            );
          }
          ...
          lines.push(`${label("Proof")} ${warn("none yet")}`);
        }
        defaultRuntime.log(lines.join("\n"));
        defaultRuntime.log("");
      },
...
export async function runGraphitiBackboneSearch(
  ...
  if (!query) {
    defaultRuntime.error("Missing search query. Provide --query <text>.");
    process.exitCode = 1;
    return;
  }
  for (const agentId of agentIds) {
    await withGraphitiManagerForAgent({
      ...
      run: async (manager) => {
        ...
        if (opts.json) {
          defaultRuntime.log(JSON.stringify({ agentId, results }, null, 2));
          return;
        }
        if (results.length === 0) {
          defaultRuntime.log("No Graphiti matches.");
          return;
        }
        const rich = isRich();
        const lines: string[] = [];
...
export async function runGraphitiBackboneSync(opts: MemoryCommandOptions) {
  setVerbose(Boolean(opts.verbose));
  const { config: cfg, diagnostics } = await loadMemoryCommandConfig("memory graphiti sync");
  emitMemorySecretResolveDiagnostics(diagnostics, { json: Boolean(opts.json) });
  const agentIds = resolveAgentIds(cfg, opts.agent);
  for (const agentId of agentIds) {
    await withGraphitiManagerForAgent({
      ...
      run: async (manager) => {
        ...
        if (opts.json) {
          defaultRuntime.log(JSON.stringify({ agentId, proof }, null, 2));
          return;
        }
        defaultRuntime.log(`Graphiti backbone synced (${agentId}).`);
        if (proof) {
          defaultRuntime.log(
            `Graphiti proof: ${shortenHomePath(proof.paths.proofPath)} (${proof.ledgerEntries} entries)`,
          );
        }
      },
    });
...
export function registerMemoryCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vikiclow memory status", "Show index and provider status."],
          ["vikiclow memory status --deep", "Probe embedding provider readiness."],
          ["vikiclow memory proof", "Show the durable persistence proof trail."],
          ["vikiclow memory graphiti status", "Inspect the Graphiti/Neo4j backbone proof."],
          ["vikiclow memory graphiti search mission", "Search the graph backbone ledger."],
          ["vikiclow memory index --force", "Force a full reindex."],
          ['vikiclow memory search "meeting notes"', "Quick search using positional query."],
          [
            'vikiclow memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["vikiclow memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.vikiclow.ai/cli/memory")}\n`,
    );
  ...
  memory
    .command("proof")
    .description("Show durable memory persistence proof")
    .option("--agent <id>", "Agent id (default: default agent)")
    .option("--json", "Print JSON")
    .option("--verbose", "Verbose logging", false)
    .action(async (opts: MemoryCommandOptions) => {
      await runMemoryProof(opts);
    });

  const graphiti = memory
    .command("graphiti")
    .description("Inspect the Graphiti/Neo4j memory backbone");

  graphiti
    .command("status")
    .description("Show the Graphiti/Neo4j backbone status")
    .option("--agent <id>", "Agent id (default: default agent)")
    .option("--json", "Print JSON")
    .option("--verbose", "Verbose logging", false)
    .action(async (opts: MemoryCommandOptions) => {
      await runGraphitiBackboneStatus(opts);
    });

  graphiti
    .command("proof")
    .description("Show the Graphiti/Neo4j backbone proof")
    .option("--agent <id>", "Agent id (default: default agent)")
    .option("--json", "Print JSON")
    .option("--verbose", "Verbose logging", false)
    .action(async (opts: MemoryCommandOptions) => {
  ...
  graphiti
    .command("search")
    .description("Search the Graphiti ledger")
    ...
    .action(
      async (
        ...
        await runGraphitiBackboneSearch({ ...opts, query: opts.query ?? queryArg });
      },
    );

  graphiti
    .command("sync")
    .description("Rescan workspace memory writebacks into the Graphiti ledger")
    .option("--agent <id>", "Agent id (default: default agent)")
    .option("--force", "Force sync", false)
    .option("--json", "Print JSON")
    .option("--verbose", "Verbose logging", false)
    .action(async (opts: MemoryCommandOptions) => {
      await runGraphitiBackboneSync(opts);
    });

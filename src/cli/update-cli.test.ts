...
import type { VikiClowConfig, ConfigFileSnapshot } from "../config/types.vikiclow.js";
...
vi.mock("../infra/vikiclow-root.js", () => ({
  resolveVikiClowPackageRoot: vi.fn(),
}));
...
const { resolveVikiClowPackageRoot } = await import("../infra/vikiclow-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/vikiclow-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as VikiClowConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/vikiclow-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveVikiClowPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("vikiclow-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveVikiClowPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("vikiclow");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/vikiclow-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "vikiclow-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("VikiClow update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as VikiClowConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveVikiClowPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("vikiclow-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ VIKICLOW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.VIKICLOW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("vikiclow-update-wizard");
    await withEnvAsync({ VIKICLOW_GIT_DIR: tempDir }, async () => {
      setTty(true);

...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-bundled-skills-test-"));
    process.env.VIKICLOW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

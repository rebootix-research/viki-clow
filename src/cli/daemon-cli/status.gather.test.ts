...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    VIKICLOW_STATE_DIR: "/tmp/vikiclow-daemon",
    VIKICLOW_CONFIG_PATH: "/tmp/vikiclow-daemon/vikiclow.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.VIKICLOW_STATE_DIR ?? "/tmp/vikiclow-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.VIKICLOW_CONFIG_PATH ?? `${stateDir}/vikiclow.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/vikiclow-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.VIKICLOW_STATE_DIR = "/tmp/vikiclow-cli";
    process.env.VIKICLOW_CONFIG_PATH = "/tmp/vikiclow-cli/vikiclow.json";
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.VIKICLOW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret

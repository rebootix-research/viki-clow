...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("vikiclow-daemon-install-int-");
    configPath = path.join(tempHome, "vikiclow.json");
    process.env.HOME = tempHome;
    process.env.VIKICLOW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.VIKICLOW_GATEWAY_TOKEN).toBeUndefined();
  });
});

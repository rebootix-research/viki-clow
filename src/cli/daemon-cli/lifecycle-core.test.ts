...
describe("runServiceRestart token drift", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      environment: { VIKICLOW_GATEWAY_TOKEN: "service-token" },
    });
    service.restart.mockResolvedValue(undefined);
    vi.unstubAllEnvs();
    vi.stubEnv("VIKICLOW_GATEWAY_TOKEN", "");
    vi.stubEnv("VIKICLOWBOT_GATEWAY_TOKEN", "");
    vi.stubEnv("VIKICLOW_GATEWAY_URL", "");
    vi.stubEnv("VIKICLOWBOT_GATEWAY_URL", "");
  });
  ...
  it("compares restart drift against config token even when caller env is set", async () => {
    ...
    service.readCommand.mockResolvedValue({
      environment: { VIKICLOW_GATEWAY_TOKEN: "env-token" },
    });
    vi.stubEnv("VIKICLOW_GATEWAY_TOKEN", "env-token");

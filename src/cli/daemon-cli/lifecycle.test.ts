...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["vikiclow", "gateway", "--port", "18789"],
      environment: {},
    });
    ...
    mockReadFileSync.mockImplementation((path: string) => {
      ...
      if ([4200, 4300].includes(pid)) {
        return ["vikiclow", "gateway", "--port", "18789", ""].join("\0");
      }
    ...
    mockSpawnSync.mockReturnValue({
      error: null,
      status: 0,
      stdout: "vikiclow gateway --port 18789",
      stderr: "",
    });
  });
  ...
  it("fails restart when gateway remains unhealthy", async () => {
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toMatchObject({
      ...
      hints: ["vikiclow gateway status --deep", "vikiclow doctor"],
    });
  ...
  it("signals an unmanaged gateway process on stop", async () => {
    ...
    mockSpawnSync.mockReturnValue({
      ...
      status: 0,
      stdout:
        'CommandLine="C:\\\\Program Files\\\\VikiClow\\\\vikiclow.exe" gateway --port 18789\r\n',
      stderr: "",
    });

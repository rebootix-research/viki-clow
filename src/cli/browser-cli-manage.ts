...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "vikiclow";
  defaultRuntime.log(info(`Viki Browser [${name}] running: ${status.running}`));
}
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `nodeLauncher: ${paths.nodeLauncherPath}`,
            `windowsCmd: ${paths.windowsCmdPath}`,
            `windowsPs1: ${paths.windowsPs1Path}`,
            `unixLauncher: ${paths.unixLauncherPath}`,
          ].join("\n"),
        );
      });
    });
  ...
  browser
    ...
    .description("Write and print a native Viki Browser verification proof")
    .action(async (_opts, cmd) => {
      const parent = parentOpts(cmd);
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            ...
            `browserExecutable: ${firstExisting ?? proof.configuredExecutable ?? "not found"}`,
            `proof: ${jsonPath}`,
          ].join("\n"),
        );
      });
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "vikiclow"}`,
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        if (!result.moved) {
          defaultRuntime.log(info("Viki Browser profile already missing."));
          return;
        }
        const dest = result.to ?? result.from;
        defaultRuntime.log(info(`Viki Browser profile moved to Trash (${dest})`));
      });
    });
  ...
  browser
    .command("daemon")
    .description("Inspect the persisted Viki Browser sibling-process manifest")
    .action(async (_opts, cmd) => {
      const parent = parentOpts(cmd);
      await runBrowserCommand(async () => {
        const result = await callBrowserRequest<BrowserdManifest>(
          parent,
          {
            method: "GET",
            path: "/browserd",
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (vikiclow|extension). Default: vikiclow")
    .action(
      async (opts: { name: string; color?: string; cdpUrl?: string; driver?: string }, cmd) => {
        ...
        await runBrowserCommand(async () => {
          ...
          defaultRuntime.log(
            info(
              `Created Viki Browser profile "${result.profile}"\n${loc}\n  color: ${result.color}${
    ...
    .action(async (opts: { name: string }, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        const msg = result.deleted
          ? `Deleted Viki Browser profile "${result.profile}" (user data removed)`
          : `Deleted Viki Browser profile "${result.profile}" (no user data found)`;

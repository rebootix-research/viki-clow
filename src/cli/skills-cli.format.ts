...
function appendSkillsHint(output: string, json?: boolean): string {
  ...
  return `${output}\n\nTip: use \`vikiclow skills list\` and \`vikiclow skills check\` to inspect installed skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("vikiclow skills list")}\` to see all skills.`
      : "No skills found.";
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendSkillsHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("vikiclow skills list")}\` to see available skills.`,
      opts.json,
    );
  }

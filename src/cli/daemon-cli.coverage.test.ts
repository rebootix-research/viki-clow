...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      VIKICLOW_GATEWAY_PORT: String(params.port),
      ...(params.token ? { VIKICLOW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.VIKICLOW_STATE_DIR = "/tmp/vikiclow-cli-state";
    process.env.VIKICLOW_CONFIG_PATH = "/tmp/vikiclow-cli-state/vikiclow.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        VIKICLOW_STATE_DIR: "/tmp/vikiclow-daemon-state",
        VIKICLOW_CONFIG_PATH: "/tmp/vikiclow-daemon-state/vikiclow.json",
        VIKICLOW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.vikiclow.gateway.plist",
    });

    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "vikiclow node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["vikiclow node status", "Check node host service status."],
          ["vikiclow node install", "Install the node host service."],
          ["vikiclow node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.vikiclow.ai/cli/node")}\n`,
    );

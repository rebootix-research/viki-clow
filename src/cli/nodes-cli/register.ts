    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vikiclow nodes status", "List known nodes with live status."],
          ["vikiclow nodes pairing pending", "Show pending node pairing requests."],
          ['vikiclow nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["vikiclow nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.vikiclow.ai/cli/nodes")}\n`,
    );

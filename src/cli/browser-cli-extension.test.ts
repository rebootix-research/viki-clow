...
describe("bundled extension resolver (fs-mocked)", () => {
  it("walks up to find the assets directory", () => {
    const root = abs("/tmp/vikiclow-ext-root");
  ...
  it("prefers the nearest assets directory", () => {
    const root = abs("/tmp/vikiclow-ext-root-nearest");
    const here = path.join(root, "dist", "cli");
...
describe("browser extension install (fs-mocked)", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = abs("/tmp/vikiclow-ext-install");
  ...
  it("copies extension path to clipboard", async () => {
    const tmp = abs("/tmp/vikiclow-ext-path");
    await withEnvAsync({ VIKICLOW_STATE_DIR: tmp }, async () => {
  ...
  it("prints Viki Browser Relay install instructions", async () => {
    const tmp = abs("/tmp/vikiclow-ext-install-output");
    await withEnvAsync({ VIKICLOW_STATE_DIR: tmp }, async () => {
      const bundled = path.join(process.cwd(), "assets", "chrome-extension");
      ...
      await program.parseAsync(["browser", "extension", "install"], { from: "user" });

      const output = runtime.error.mock.calls.map((call) => String(call[0])).join("\n");
      expect(output).toContain("Viki Browser Relay");
      expect(output).not.toContain("VikiClow Browser Relay");

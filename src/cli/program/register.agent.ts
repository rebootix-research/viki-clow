...
${formatHelpExamples([
  ['vikiclow agent --to +15555550123 --message "status update"', "Start a new session."],
  ['vikiclow agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'vikiclow agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'vikiclow agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['vikiclow agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'vikiclow agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.vikiclow.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.vikiclow.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  [
    'vikiclow agents set-identity --agent main --name "VikiClow" --emoji ":sparkles:"',
    "Set name + emoji.",
  ],
  ["vikiclow agents set-identity --agent main --avatar avatars/vikiclow.png", "Set avatar path."],
  [
    "vikiclow agents set-identity --workspace ~/.vikiclow/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "vikiclow agents set-identity --identity-file ~/.vikiclow/workspace/IDENTITY.md --agent main",

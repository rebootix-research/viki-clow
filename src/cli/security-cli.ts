...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vikiclow security audit", "Run a local security audit."],
          ["vikiclow security audit --deep", "Include best-effort live Gateway probe checks."],
          ["vikiclow security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["vikiclow security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.vikiclow.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("VikiClow security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("vikiclow security audit --deep")}`));

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("vikiclow security audit --fix")}`));
        if (!fixResult) {

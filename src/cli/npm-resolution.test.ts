...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@vikiclow/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@vikiclow/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@vikiclow/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@vikiclow/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @vikiclow/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@vikiclow/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@vikiclow/plugin-alpha@1.2.3",
        installPath: "/tmp/vikiclow/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@vikiclow/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @vikiclow/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@vikiclow/plugin-alpha@latest",
      true,
      "/tmp/vikiclow/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@vikiclow/plugin-alpha@latest",
      installPath: "/tmp/vikiclow/extensions/alpha",
      version: "1.2.3",

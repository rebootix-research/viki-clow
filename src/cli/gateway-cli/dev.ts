...
import { applyOnboardingAutonomyDefaults } from "../../commands/onboard-config.js";
...
const resolveDevWorkspaceDir = (env: NodeJS.ProcessEnv = process.env): string => {
  ...
  const profile = env.VIKICLOW_PROFILE?.trim().toLowerCase();
  if (profile === "dev") {
    return baseDir;
  }
...
async function ensureDevWorkspace(dir: string) {
  ...
  const [agents, soul, tools, heartbeat, identity, user] = await Promise.all([
    loadDevTemplate(
      "AGENTS.dev.md",
      `# AGENTS.md - VikiClow Dev Workspace\n\nDefault dev workspace for vikiclow gateway --dev.\n`,
    ),
    loadDevTemplate(
      "SOUL.dev.md",
    ...
    loadDevTemplate(
      "HEARTBEAT.md",
      "# HEARTBEAT.md\n\n# Add short recurring checks here when you want proactive background work.\n",
    ),
    loadDevTemplate(
      "IDENTITY.dev.md",
...
export async function ensureDevGatewayConfig(opts: { reset?: boolean }) {
  ...
  await writeConfigFile({
    ...
    agents: {
      defaults: {
        ...applyOnboardingAutonomyDefaults(undefined),
        workspace,
        skipBootstrap: true,
      },
      list: [

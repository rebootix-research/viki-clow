...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.vikiclow.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + VikiClow hooks")
    ...
    .option("--hook-url <url>", "VikiClow hook URL")
    .option("--hook-token <token>", "VikiClow hook token")

...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when VIKICLOW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".vikiclow", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when VIKICLOW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", VIKICLOW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".vikiclow-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers VIKICLOW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        VIKICLOW_PROFILE: "rescue",
        VIKICLOW_STATE_DIR: "C:\\State\\vikiclow",
      },
      expected: path.join("C:\\State\\vikiclow", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", VIKICLOW_PROFILE: "default" },
      expected: path.join("/home/test", ".vikiclow", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem VikiClow Gateway",
          "cd /d C:\\Projects\\vikiclow",
          "set NODE_ENV=production",
          "set VIKICLOW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\vikiclow",
          environment: {
            NODE_ENV: "production",
            VIKICLOW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\vikiclow\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\vikiclow\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\VikiClow Share\\node.exe" "\\\\fileserver\\VikiClow Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\VikiClow Share\\node.exe",
            "\\\\fileserver\\VikiClow Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from VIKICLOW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ VIKICLOW_STATE_DIR: path.join(tmpDir, "custom-state") }),

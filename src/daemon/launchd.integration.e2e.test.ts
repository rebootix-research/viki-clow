...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `vikiclow-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      VIKICLOW_LAUNCHD_LABEL: `ai.vikiclow.launchd-int-${testId}`,
      VIKICLOW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });

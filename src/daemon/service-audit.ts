...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds VIKICLOW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `vikiclow gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service VIKICLOW_GATEWAY_TOKEN does not match gateway.auth.token in vikiclow.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.VIKICLOW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.VIKICLOW_GATEWAY_TOKEN?.trim() || undefined;
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `vikiclow gateway install --force` to sync the token.",
      level: "recommended",
    };
  }

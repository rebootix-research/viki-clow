...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.VIKICLOW_GATEWAY_PORT).toBe("18789");
    expect(env.VIKICLOW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.VIKICLOW_SERVICE_MARKER).toBe("vikiclow");
    expect(env.VIKICLOW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.VIKICLOW_SERVICE_VERSION).toBe("string");
    expect(env.VIKICLOW_SYSTEMD_UNIT).toBe("vikiclow-gateway.service");
    expect(env.VIKICLOW_WINDOWS_TASK_NAME).toBe("VikiClow Gateway");
    if (process.platform === "darwin") {
      expect(env.VIKICLOW_LAUNCHD_LABEL).toBe("ai.vikiclow.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", VIKICLOW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.VIKICLOW_SYSTEMD_UNIT).toBe("vikiclow-gateway-work.service");
    expect(env.VIKICLOW_WINDOWS_TASK_NAME).toBe("VikiClow Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.VIKICLOW_LAUNCHD_LABEL).toBe("ai.vikiclow.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through VIKICLOW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", VIKICLOW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.VIKICLOW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy VIKICLOWBOT_GATEWAY_TOKEN to VIKICLOW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", VIKICLOWBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.VIKICLOW_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers VIKICLOW_GATEWAY_TOKEN over legacy VIKICLOWBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        VIKICLOW_GATEWAY_TOKEN: "vikiclow-token",
        VIKICLOWBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.VIKICLOW_GATEWAY_TOKEN).toBe("vikiclow-token");
  });

  it("omits VIKICLOW_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        VIKICLOW_GATEWAY_TOKEN: "   ",
        VIKICLOWBOT_GATEWAY_TOKEN: " ",
      },
    });
    expect(env.VIKICLOW_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".vikiclow"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", VIKICLOW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".vikiclow-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", VIKICLOW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".vikiclow"));
  });

  it("uses VIKICLOW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", VIKICLOW_STATE_DIR: "/var/lib/vikiclow" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/vikiclow"));
  });

  it("expands ~ in VIKICLOW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", VIKICLOW_STATE_DIR: "~/vikiclow-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/vikiclow-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { VIKICLOW_STATE_DIR: "C:\\State\\vikiclow" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\vikiclow");
  });
});

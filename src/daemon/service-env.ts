...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.VIKICLOW_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VIKICLOW_PROFILE: profile,
    VIKICLOW_GATEWAY_PORT: String(port),
    VIKICLOW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    VIKICLOW_SYSTEMD_UNIT: systemdUnit,
    VIKICLOW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    VIKICLOW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    VIKICLOW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.VIKICLOW_GATEWAY_TOKEN?.trim() || env.VIKICLOWBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    VIKICLOW_GATEWAY_TOKEN: gatewayToken,
    VIKICLOW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    VIKICLOW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    VIKICLOW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    VIKICLOW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    VIKICLOW_LOG_PREFIX: "node",
    VIKICLOW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    VIKICLOW_SERVICE_KIND: NODE_SERVICE_KIND,
    VIKICLOW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    VIKICLOW_STATE_DIR: sharedEnv.stateDir,
    VIKICLOW_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.VIKICLOW_STATE_DIR;
  const configPath = env.VIKICLOW_CONFIG_PATH;

...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false when systemctl is not present", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/vikiclow-test-home" } });
  ...
  it("calls systemctl is-enabled when systemctl is present", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "vikiclow-gateway.service"]);
      cb(null, "enabled", "");
    });
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/vikiclow-test-home" } });
  ...
  it("returns false for the WSL2 Ubuntu 24.04 wrapper-only is-enabled failure", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, args, _opts, cb) => {
      expect(args).toEqual(["--user", "is-enabled", "vikiclow-gateway.service"]);
      const err = new Error(
        "Command failed: systemctl --user is-enabled vikiclow-gateway.service",
    ...
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/vikiclow-test-home" } }),
    ).rejects.toThrow(
      "systemctl is-enabled unavailable: Command failed: systemctl --user is-enabled vikiclow-gateway.service",
    );
  });
  ...
  it("returns false when is-enabled cannot connect to the user bus without machine fallback", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/vikiclow-test-home", USER: "", LOGNAME: "" },
      }),
  ...
  it("returns false when both direct and machine-scope is-enabled checks report bus unavailability", async () => {
    ...
    await expect(
      isSystemdServiceEnabled({
        env: { HOME: "/tmp/vikiclow-test-home", USER: "debian" },
      }),
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "vikiclow-gateway.service"]);
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled vikiclow-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when VIKICLOW_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/vikiclow-gateway.service",
    },
    {
      name: "uses profile-specific service name when VIKICLOW_PROFILE is set to a custom value",
      env: { HOME: "/home/test", VIKICLOW_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/vikiclow-gateway-jbphoenix.service",
    },
    {
      name: "prefers VIKICLOW_SYSTEMD_UNIT over VIKICLOW_PROFILE",
      env: {
        HOME: "/home/test",
        VIKICLOW_PROFILE: "jbphoenix",
        VIKICLOW_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles VIKICLOW_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        VIKICLOW_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from VIKICLOW_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        VIKICLOW_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/vikiclow gateway start --name "My Bot"')).toEqual([
      "/usr/bin/vikiclow",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('vikiclow --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["vikiclow", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('vikiclow --path "C:\\\\Program Files\\\\VikiClow"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["vikiclow", "--path", "C:\\\\Program Files\\\\VikiClow"]);

    expect(
      splitArgsPreservingQuotes('vikiclow --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["vikiclow", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/vikiclow gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads VIKICLOW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/vikiclow-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/vikiclow gateway run",
          "EnvironmentFile=%h/.vikiclow/.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.vikiclow/.env") {
        return "VIKICLOW_GATEWAY_TOKEN=env-file-token\n";
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });

    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment?.VIKICLOW_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/vikiclow-gateway.service")) {
        return [
          ...
          "ExecStart=/usr/bin/vikiclow gateway run",
          "EnvironmentFile=%h/.vikiclow/.env",
          'Environment="VIKICLOW_GATEWAY_TOKEN=inline-token"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.vikiclow/.env") {
        return "VIKICLOW_GATEWAY_TOKEN=env-file-token\n";
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment?.VIKICLOW_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.VIKICLOW_GATEWAY_TOKEN).toBe("file");
  });
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/vikiclow-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/vikiclow gateway run",
          "EnvironmentFile=-%h/.vikiclow/missing.env",
        ].join("\n");
      }
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.programArguments).toEqual(["/usr/bin/vikiclow", "gateway", "run"]);
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/vikiclow-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/vikiclow gateway run",
          'EnvironmentFile=%h/.vikiclow/first.env "%h/.vikiclow/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.vikiclow/first.env") {
        return "VIKICLOW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.vikiclow/second env.env") {
        return 'VIKICLOW_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      VIKICLOW_GATEWAY_TOKEN: "first-token",
      VIKICLOW_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "VIKICLOW_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "VIKICLOW_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "VIKICLOW_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      VIKICLOW_GATEWAY_TOKEN: "override-token",
      VIKICLOW_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/vikiclow-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/vikiclow gateway run",
          "EnvironmentFile=%h/.vikiclow/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.vikiclow/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'VIKICLOW_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "VIKICLOW_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      VIKICLOW_GATEWAY_TOKEN: "quoted token",
      VIKICLOW_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      VIKICLOW_GATEWAY_TOKEN: "file",
      VIKICLOW_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("systemd service control", () => {
  const assertMachineRestartArgs = (args: string[]) => {
    expect(args).toEqual(["--machine", "debian@", "--user", "restart", "vikiclow-gateway.service"]);
  };
  ...
  it("stops the resolved user unit", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "stop", "vikiclow-gateway.service"]);
        cb(null, "", "");
      });
    const write = vi.fn();
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "vikiclow-gateway-work.service"]);
        cb(null, "", "");
      });
    await assertRestartSuccess({ VIKICLOW_PROFILE: "work" });
  });
  ...
  it("keeps direct --user scope when SUDO_USER is root", async () => {
    execFileMock
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "restart", "vikiclow-gateway.service"]);
        cb(null, "", "");
      });

...
const VIKICLOW_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of VIKICLOW_COMPILED_SECRET_TARGETS) {
...
const VIKICLOW_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: VIKICLOW_COMPILED_SECRET_TARGETS,
    entriesById: VIKICLOW_TARGETS_BY_ID,
  });

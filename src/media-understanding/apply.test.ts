...
const TEMP_MEDIA_PREFIX = "vikiclow-media-";
...
function createGroqAudioConfig(): VikiClowConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): VikiClowConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): VikiClowConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: VikiClowConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredVikiClowTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        VIKICLOW_AGENT_DIR: isolatedAgentDir,

...
export async function ensureMediaHosted(
  ...
  if (needsServerStart && !opts.startServer) {
    ...
    throw new Error(
      `Media hosting requires the webhook/Funnel server. Start \`${formatCliCommand("vikiclow webhook")}\`/\`${formatCliCommand("vikiclow up")}\` or re-run with --serve-media.`,
    );
  }
  if (needsServerStart && opts.startServer) {
    if (!mediaServer) {
      ...
      logInfo(
        `VikiClow temporary media host started on http://localhost:${port}/media/:id (TTL ${TTL_MS / 1000}s)`,
        runtime,
      );
      mediaServer.unref?.();
    }

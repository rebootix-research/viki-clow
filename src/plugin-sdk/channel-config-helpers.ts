...
export function createScopedAccountConfigAccessors<ResolvedAccount>(params: {
  resolveAccount: (params: { cfg: VikiClowConfig; accountId?: string | null }) => ResolvedAccount;
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: VikiClowConfig; accountId?: string | null }) =>
...
export function createScopedChannelConfigBase<
  ResolvedAccount,
  Config extends VikiClowConfig = VikiClowConfig,
>(params: {
  sectionKey: string;

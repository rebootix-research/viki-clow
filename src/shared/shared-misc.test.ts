...
describe("shared/frontmatter", () => {
  ...
  test("resolveVikiClowManifestBlock parses JSON5 metadata and picks vikiclow block", () => {
    const frontmatter = {
      metadata: "{ vikiclow: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveVikiClowManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveVikiClowManifestBlock returns undefined for invalid input", () => {
    expect(resolveVikiClowManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveVikiClowManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveVikiClowManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});

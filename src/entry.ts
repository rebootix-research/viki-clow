...
import { ensureVikiClowExecMarkerOnProcess } from "./infra/vikiclow-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "vikiclow.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "vikiclow.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  process.title = "vikiclow";
  ensureVikiClowExecMarkerOnProcess();
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.VIKICLOW_AUTH_STORE_READONLY = "1";
  }
  ...
  function ensureExperimentalWarningSuppressed(): boolean {
    ...
    if (isTruthyEnvValue(process.env.VIKICLOW_NO_RESPAWN)) {
      return false;
    }
    if (isTruthyEnvValue(process.env.VIKICLOW_NODE_OPTIONS_READY)) {
      return false;
    }
    ...
    process.env.VIKICLOW_NODE_OPTIONS_READY = "1";
    ...
    child.once("error", (error) => {
      console.error(
        "[vikiclow] Failed to respawn CLI:",
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `VikiClow ${VERSION} (${commit})` : `VikiClow ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[vikiclow] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  function tryHandleRootHelpFastPath(argv: string[]): boolean {
    ...
    import("./cli/program.js")
      ...
      .catch((error) => {
        console.error(
          "[vikiclow] Failed to display help:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureExperimentalWarningSuppressed()) {
    ...
    if (!parsed.ok) {
      ...
      console.error(`[vikiclow] ${parsed.error}`);
      process.exit(2);
    }

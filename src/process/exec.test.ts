...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        VIKICLOW_BASE_ENV: "base",
        VIKICLOW_TO_REMOVE: undefined,
      },
      env: {
        VIKICLOW_TEST_ENV: "ok",
      },
    });

    expect(resolved.VIKICLOW_BASE_ENV).toBe("base");
    expect(resolved.VIKICLOW_TEST_ENV).toBe("ok");
    expect(resolved.VIKICLOW_TO_REMOVE).toBeUndefined();
    expect(resolved.VIKICLOW_CLI).toBe(VIKICLOW_CLI_ENV_VALUE);
  });

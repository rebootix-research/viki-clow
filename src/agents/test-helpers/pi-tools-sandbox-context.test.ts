...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("vikiclow-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("vikiclow-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("vikiclow-sbx-");

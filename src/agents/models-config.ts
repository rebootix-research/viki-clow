...
import { planVikiClowModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: VikiClowConfig): VikiClowConfig {
...
export async function ensureVikiClowModelsJson(
  config?: VikiClowConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = resolveModelsConfigInput(config);
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveVikiClowAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planVikiClowModelsJson({
      cfg,
      agentDir,
      env,

...
function findTool(name: string, config: VikiClowConfig) {
  const allTools = createVikiClowTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: VikiClowConfig; env?: NodeJS.ProcessEnv }) {
...
describe("vikiclow tools runtime web metadata wiring", () => {

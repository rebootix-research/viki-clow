...
type CoreToolDefinition = {
  ...
  includeInVikiClowGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const vikiclowTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInVikiClowGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:vikiclow": vikiclowTools,

...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["VIKICLOW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-subagent-"));
  ...
  it("uses isolated temp state when VIKICLOW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "vikiclow-test-state"));
  });
});

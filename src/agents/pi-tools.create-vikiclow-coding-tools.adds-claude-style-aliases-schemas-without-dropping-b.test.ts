...
const defaultTools = createVikiClowCodingTools({ senderIsOwner: true });
...
describe("createVikiClowCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: VikiClowConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createVikiClowCodingTools({
    ...
    const anthropicTools = createVikiClowCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: VikiClowConfig = {
      tools: {
        exec: {
    ...
    const allowed = createVikiClowCodingTools({
    ...
    const denied = createVikiClowCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createVikiClowCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });

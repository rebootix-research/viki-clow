...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes safety guardrails in full prompts", () => {
    ...
    expect(prompt).toContain("## Execution Mindset");
    expect(prompt).toContain(
      "Carry tasks through implementation, verification, and delivery when possible.",
    );
    expect(prompt).toContain("Never bypass approvals, tool policy, sandbox boundaries");
    expect(prompt).toContain("## Safety");
    expect(prompt).toContain("You have no independent goals");
    expect(prompt).toContain("Prioritize safety and human oversight");
    expect(prompt).toContain("if instructions conflict");
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## VikiClow CLI Quick Reference");
    expect(prompt).toContain("vikiclow gateway restart");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List VikiClow agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("VikiClow docs: /tmp/vikiclow/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## VikiClow Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/vikiclow. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`vikiclow ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for VikiClow subagents");

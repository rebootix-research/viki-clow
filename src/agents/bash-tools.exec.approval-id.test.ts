...
describe("exec approvals", () => {
  ...
  beforeEach(async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-test-"));
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-test-bin-"));
  ...
  it("uses exec-approvals ask=off to suppress gateway prompts", async () => {
    const approvalsPath = path.join(process.env.HOME ?? "", ".vikiclow", "exec-approvals.json");
  ...
  it("returns an unavailable approval message instead of a local /approve prompt when discord exec approvals are disabled", async () => {
    const configPath = path.join(process.env.HOME ?? "", ".vikiclow", "vikiclow.json");
  ...
  it("denies gateway obfuscated command when approval request times out", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-test-obf-"));

...
import { markVikiClowExecEnv } from "../../infra/vikiclow-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "vikiclow.sandbox=1");
  args.push("--label", `vikiclow.sessionKey=${params.scopeKey}`);
  args.push("--label", `vikiclow.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `vikiclow.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markVikiClowExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "vikiclow.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`vikiclow sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`vikiclow sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("vikiclow sandbox recreate --all");
}

...
function resolveAttachmentLimits(config: VikiClowConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".vikiclow", "attachments");
  const relDir = path.posix.join(".vikiclow", "attachments", attachmentId);

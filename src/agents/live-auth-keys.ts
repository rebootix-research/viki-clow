...
const GOOGLE_LIVE_SINGLE_KEY = "VIKICLOW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "VIKICLOW_LIVE_ANTHROPIC_KEY",
    listVar: "VIKICLOW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "VIKICLOW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `VIKICLOW_LIVE_${base}_KEY`;

...
describe("createVikiClowCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-read-"));
    try {
      ...
      const contents = "Hello from vikiclow read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createVikiClowCodingTools({ sandbox });

...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createVikiClowTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-test-"));

    try {
      await withEnvAsync(
        { VIKICLOW_STATE_DIR: stateDir, VIKICLOW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: vikiclow --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/vikiclow" } }\n}\n';

...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: VikiClowConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/vikiclow" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: VikiClowConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: VikiClowConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: VikiClowConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: VikiClowConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: VikiClowConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: VikiClowConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: VikiClowConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/vikiclow");
  });

  it("uses VIKICLOW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "vikiclow-home");
    vi.stubEnv("VIKICLOW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as VikiClowConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".vikiclow", "workspace"));
  });

  it("uses VIKICLOW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "vikiclow-home");
    vi.stubEnv("VIKICLOW_HOME", home);
    // Clear state dir so it falls back to VIKICLOW_HOME
    vi.stubEnv("VIKICLOW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as VikiClowConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".vikiclow", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/vikiclow-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/vikiclow-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "vikiclow-agent-scope-"));

...
import { resolveVikiClowDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveVikiClowAgentDir();
  await ensureVikiClowModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createVikiClowCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveVikiClowDocsPath({
      workspaceDir: effectiveWorkspace,

...
const originalBundledDir = process.env.VIKICLOW_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("vikiclow-bundled-");
  const workspaceDir = await createTempDir("vikiclow-workspace-");
...
afterEach(async () => {
  process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = originalBundledDir;
...
describe("resolveEmbeddedRunSkillEntries (integration)", () => {
  it("loads bundled diffs skill when explicitly enabled in config", async () => {
    ...
    process.env.VIKICLOW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
    clearPluginManifestRegistryCache();

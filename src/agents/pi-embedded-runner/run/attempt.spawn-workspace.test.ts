...
vi.mock("../../docs-path.js", () => ({
  resolveVikiClowDocsPath: async () => undefined,
}));
...
describe("runEmbeddedAttempt sessions_spawn workspace inheritance", () => {
  ...
  it("passes the real workspace to sessions_spawn when workspaceAccess is ro", async () => {
    const realWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-real-workspace-"));
    const sandboxWorkspace = await fs.mkdtemp(
      path.join(os.tmpdir(), "vikiclow-sandbox-workspace-"),
    );
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-agent-dir-"));

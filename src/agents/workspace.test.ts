...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses VIKICLOW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      VIKICLOW_HOME: "/srv/vikiclow-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/vikiclow-home"), ".vikiclow", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".vikiclow", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("vikiclow-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-workspace-hardlink-"));
    try {

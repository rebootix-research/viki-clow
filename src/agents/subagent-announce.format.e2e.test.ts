...
describe("subagent announce formatting", () => {
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.VIKICLOW_TEST_FAST;
      return;
    }
    process.env.VIKICLOW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // VIKICLOW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("VikiClow runtime context (internal):");

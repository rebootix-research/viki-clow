...
async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-tools-pdf-"));
  try {
...
describe("createVikiClowTools PDF registration", () => {
  it("includes pdf tool when pdfModel is configured", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const tools = createVikiClowTools({ config: cfg, agentDir });

...
describe("browser tool branding", () => {
  registerBrowserToolAfterEachReset();

  it("describes the managed browser as Viki Browser", () => {
    const tool = createBrowserTool();
    expect(tool.description).toContain("Viki Browser control server");
    expect(tool.description).toContain("Viki Browser Relay");
  });
});

...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    vikiclow: process.env.VIKICLOW_GATEWAY_TOKEN,
    vikiclowbot: process.env.VIKICLOWBOT_GATEWAY_TOKEN,
  };
  ...
  beforeEach(() => {
    ...
    delete process.env.VIKICLOWBOT_GATEWAY_TOKEN;
  });

  afterAll(() => {
    if (envSnapshot.vikiclow === undefined) {
      delete process.env.VIKICLOW_GATEWAY_TOKEN;
    } else {
      process.env.VIKICLOW_GATEWAY_TOKEN = envSnapshot.vikiclow;
    }
    if (envSnapshot.vikiclowbot === undefined) {
      delete process.env.VIKICLOWBOT_GATEWAY_TOKEN;
    } else {
      process.env.VIKICLOWBOT_GATEWAY_TOKEN = envSnapshot.vikiclowbot;
    }
  });
  ...
  it("uses VIKICLOW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.VIKICLOW_GATEWAY_TOKEN = "local-env-token";
    process.env.VIKICLOWBOT_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {

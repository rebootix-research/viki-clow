...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): VikiClowConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as VikiClowConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): VikiClowConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as VikiClowConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: VikiClowConfig, accountId = "media") {
    await handleTelegramAction(

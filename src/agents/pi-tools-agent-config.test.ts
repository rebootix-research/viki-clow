...
import { createVikiClowCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-pi-tools-"));
    ...
    try {
      ...
      const tools = createVikiClowCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: VikiClowConfig) {
    return createVikiClowCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<VikiClowConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<VikiClowConfig["agents"]>["list"]>[number]["tools"];
  }): VikiClowConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createVikiClowCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createVikiClowCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createVikiClowCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createVikiClowCodingTools({
      config: cfg,
    ...
    const bobTools = createVikiClowCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createVikiClowCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createVikiClowCodingTools({
      config: cfg,

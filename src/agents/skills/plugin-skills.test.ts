    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "vikiclow.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "vikiclow.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "vikiclow.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("vikiclow-");
  const acpxRoot = await tempDirs.make("vikiclow-acpx-plugin-");
  const helperRoot = await tempDirs.make("vikiclow-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("vikiclow-");
  const pluginRoot = await tempDirs.make("vikiclow-plugin-");
  const outsideDir = await tempDirs.make("vikiclow-outside-");

...
import {
  applyVikiClowManifestInstallCommonFields,
  getFrontmatterString,
  normalizeStringList,
  parseVikiClowManifestInstallBase,
  parseFrontmatterBool,
  resolveVikiClowManifestBlock,
  resolveVikiClowManifestInstall,
  resolveVikiClowManifestOs,
  resolveVikiClowManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  VikiClowSkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseVikiClowManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyVikiClowManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveVikiClowMetadata(
  frontmatter: ParsedSkillFrontmatter,
): VikiClowSkillMetadata | undefined {
  const metadataObj = resolveVikiClowManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveVikiClowManifestRequires(metadataObj);
  const install = resolveVikiClowManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveVikiClowManifestOs(metadataObj);
  return {

...
function resolveSkillsLimits(config?: VikiClowConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "vikiclow-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "vikiclow-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveVikiClowMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: VikiClowConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`vikiclow skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,

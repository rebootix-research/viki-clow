...
import { resolvePreferredVikiClowTmpDir } from "../infra/tmp-vikiclow-dir.js";
...
async function withOutsideHardlinkInVikiClowTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside vikiclow tmp root",
      media: path.join(os.tmpdir(), "outside-vikiclow", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked VikiClow tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked VikiClow tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInVikiClowTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked VikiClow tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInVikiClowTmp(

...
import { resolveWorkspaceTemplatePath } from "./workspace-templates.js";
...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.VIKICLOW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".vikiclow", `workspace-${profile}`);
  }
  return path.join(home, ".vikiclow", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".vikiclow";
...
async function loadTemplate(name: string): Promise<string> {
  ...
  const pending = (async () => {
    const templatePath = await resolveWorkspaceTemplatePath(name);
    try {

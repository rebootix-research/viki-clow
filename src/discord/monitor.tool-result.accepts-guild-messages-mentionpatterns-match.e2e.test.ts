  ...
  return {
    agents: {
      defaults: {
        ...
        workspace: "/tmp/vikiclow",
      },
    },
    session: { store: "/tmp/vikiclow-sessions.json" },
  ...
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bvikiclow\\b"] },
        },
      });
      ...
      await handler(
        createGuildMessageEvent({ messageId: "m2", content: "vikiclow: hello" }),
        client,
      );
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            author: { id: "bot-id", bot: true, username: "VikiClow" },
          },
        },
        eventPatch: {

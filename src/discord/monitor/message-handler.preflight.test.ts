...
type DiscordConfig = NonNullable<
  import("../../config/config.js").VikiClowConfig["channels"]
>["discord"];
...
} as import("../../config/config.js").VikiClowConfig;
...
function createPreflightArgs(params: {
  cfg: import("../../config/config.js").VikiClowConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createMessage({
      ...
      channelId,
      content: "hi <@vikiclow-bot>",
      mentionedUsers: [{ id: "vikiclow-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey vikiclow");

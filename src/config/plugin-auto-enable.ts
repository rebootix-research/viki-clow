...
function isWhatsAppConfigured(cfg: VikiClowConfig): boolean {
...
function isGenericChannelConfigured(cfg: VikiClowConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: VikiClowConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: VikiClowConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: VikiClowConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: VikiClowConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: VikiClowConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: VikiClowConfig, pluginId: string): VikiClowConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;

...
import type { VikiClowConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareVikiClowVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  vikiclow config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  vikiclow config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: VikiClowConfig,
  pathSegments: string[],
): { changed: boolean; next: VikiClowConfig } {
...
function coerceConfig(value: unknown): VikiClowConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as VikiClowConfig;
}
...
function stampConfigVersion(cfg: VikiClowConfig): VikiClowConfig {
...
function warnIfConfigFromFuture(cfg: VikiClowConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareVikiClowVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer VikiClow (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as VikiClowConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): VikiClowConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as VikiClowConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: VikiClowConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as VikiClowConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.VIKICLOW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.VIKICLOW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.VIKICLOW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.VIKICLOW_WATCH_SESSION === "string" &&
        deps.env.VIKICLOW_WATCH_SESSION.trim().length > 0
          ? deps.env.VIKICLOW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.VIKICLOW_WATCH_COMMAND === "string" &&
        deps.env.VIKICLOW_WATCH_COMMAND.trim().length > 0
          ? deps.env.VIKICLOW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `VIKICLOW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: VikiClowConfig | null = null;
let runtimeConfigSourceSnapshot: VikiClowConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.VIKICLOW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.VIKICLOW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): VikiClowConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): VikiClowConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: VikiClowConfig;
  candidate: VikiClowConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: VikiClowConfig): VikiClowConfig {
...
export function loadConfig(): VikiClowConfig {
...
export async function readBestEffortConfig(): Promise<VikiClowConfig> {
...
export async function writeConfigFile(
  ...
  // edits to vikiclow.json remain visible (no stale snapshot).
}

...
function collectConfigEnvVarsByTarget(cfg?: VikiClowConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: VikiClowConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: VikiClowConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: VikiClowConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.VIKICLOW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth

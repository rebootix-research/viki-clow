...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ VIKICLOW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-vikiclow",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-vikiclow#general");
  });
  ...
  it("derives session transcripts dir from VIKICLOW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { VIKICLOW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when VIKICLOW_STATE_DIR differs from stored paths", () => {

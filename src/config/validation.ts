...
import type { VikiClowConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: VikiClowConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: VikiClowConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: VikiClowConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = VikiClowSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as VikiClowConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as VikiClowConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as VikiClowConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as VikiClowConfig,
  };
}

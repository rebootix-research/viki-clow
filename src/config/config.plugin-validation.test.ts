...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "vikiclow.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    VIKICLOW_STATE_DIR: process.env.VIKICLOW_STATE_DIR,
    VIKICLOW_PLUGIN_MANIFEST_CACHE_MS: process.env.VIKICLOW_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-config-plugin-validation-"));
    ...
    process.env.VIKICLOW_STATE_DIR = path.join(suiteHome, ".vikiclow");
    process.env.VIKICLOW_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.VIKICLOW_STATE_DIR === undefined) {
      delete process.env.VIKICLOW_STATE_DIR;
    } else {
      process.env.VIKICLOW_STATE_DIR = envSnapshot.VIKICLOW_STATE_DIR;
    }
    if (envSnapshot.VIKICLOW_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.VIKICLOW_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.VIKICLOW_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.VIKICLOW_PLUGIN_MANIFEST_CACHE_MS;
    }
  });

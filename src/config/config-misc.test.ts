...
import { buildWebSearchProviderConfig, withTempHome, writeVikiClowConfig } from "./test-helpers.js";
import { VikiClowSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = VikiClowSchema.safeParse({
      $schema: "https://vikiclow.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://vikiclow.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = VikiClowSchema.safeParse({ $schema: 123 });
...
describe("ui.seamColor", () => {
  it("accepts hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "#11847E" } });
    expect(res.ok).toBe(true);
  });

  it("rejects non-hex colors", () => {
    const res = validateConfigObject({ ui: { seamColor: "amber" } });
    expect(res.ok).toBe(false);
  });

  it("rejects invalid hex length", () => {
    const res = validateConfigObject({ ui: { seamColor: "#11847EFF" } });
    expect(res.ok).toBe(false);
  });
});
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = VikiClowSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("preserves resolved-only gateway.bind aliases in config snapshots", async () => {
    await withTempHome(async (home) => {
      ...
      const prev = process.env.VIKICLOW_BIND;
      ...
      try {
        ...
        if (prev === undefined) {
          ...
          process.env.VIKICLOW_BIND = prev;
        }
      }
    });
  });

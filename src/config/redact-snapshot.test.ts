...
import type { ConfigFileSnapshot } from "./types.vikiclow.js";
import { VikiClowSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(VikiClowSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.vikiclow/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/vikiclow/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/vikiclow/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/vikiclow/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/vikiclow/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = VikiClowSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "VikiClowConfig";
    const hints = mainSchemaHints;

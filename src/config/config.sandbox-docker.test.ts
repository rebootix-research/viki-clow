...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("vikiclow-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "vikiclow-sandbox-browser-global" },
      agentBrowser: { network: "vikiclow-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("vikiclow-sandbox-browser-agent");
  });

...
describe("resolveInitialTuiAgentId", () => {
  const cfg: VikiClowConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/vikiclow" },
        { id: "ops", workspace: "/tmp/vikiclow/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/vikiclow/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run vikiclow devices list");
    expect(state.pairingHint).toContain("vikiclow devices list");
  });

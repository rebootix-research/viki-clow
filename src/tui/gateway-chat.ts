...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set VIKICLOW_GATEWAY_TOKEN/VIKICLOW_GATEWAY_PASSWORD, pass --token/--password,",
...
export class GatewayChatClient {
  ...
  constructor(connection: ResolvedGatewayConnection) {
    ...
    this.client = new GatewayClient({
      ...
      clientDisplayName: "vikiclow-tui",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.VIKICLOW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.VIKICLOW_GATEWAY_PASSWORD);

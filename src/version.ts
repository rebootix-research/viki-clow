...
declare const __VIKICLOW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "vikiclow";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["VIKICLOW_VERSION"],
      runtimeVersion,
      env["VIKICLOW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current VikiClow version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __VIKICLOW_VERSION__ === "string" ? __VIKICLOW_VERSION__ : undefined,
  bundledVersion: process.env.VIKICLOW_BUNDLED_VERSION,
});

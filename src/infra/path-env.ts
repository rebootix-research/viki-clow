...
function candidateBinDirs(opts: EnsureVikiClowPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `vikiclow` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "vikiclow");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.VIKICLOW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "vikiclow"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `vikiclow` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureVikiClowCliOnPath(opts: EnsureVikiClowPathOpts = {}) {
  if (isTruthyEnvValue(process.env.VIKICLOW_PATH_BOOTSTRAPPED)) {
    return;
  }

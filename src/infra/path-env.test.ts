...
let ensureVikiClowCliOnPath: typeof import("./path-env.js").ensureVikiClowCliOnPath;

describe("ensureVikiClowCliOnPath", () => {
  const envKeys = [
    "PATH",
    "VIKICLOW_PATH_BOOTSTRAPPED",
    "VIKICLOW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureVikiClowCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling vikiclow exists", () => {
    const tmp = abs("/tmp/vikiclow-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "vikiclow");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.VIKICLOW_PATH_BOOTSTRAPPED;

    ensureVikiClowCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.VIKICLOW_PATH_BOOTSTRAPPED = "1";
    ensureVikiClowCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/vikiclow-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "vikiclow");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/vikiclow-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "vikiclow");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "vikiclow");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/vikiclow-path/case-linuxbrew");

...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.VIKICLOW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.VIKICLOW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set VIKICLOW_APNS_TEAM_ID and VIKICLOW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.VIKICLOW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.VIKICLOW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.VIKICLOW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set VIKICLOW_APNS_PRIVATE_KEY_P8 or VIKICLOW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading VIKICLOW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createVikiClowPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    vikiclow: createVikiClowPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };

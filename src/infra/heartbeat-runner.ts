...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: VikiClowConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: VikiClowConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: VikiClowConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: VikiClowConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: VikiClowConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: VikiClowConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: VikiClowConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();

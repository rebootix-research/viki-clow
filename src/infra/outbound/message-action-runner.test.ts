...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred VikiClow tmp root", async () => {
    const tmpRoot = resolvePreferredVikiClowTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideVikiClow = path.join(os.tmpdir(), "outside-vikiclow", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: VikiClowConfig | null = null;
  ...
  get(key: string, cfg: VikiClowConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: VikiClowConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: VikiClowConfig): void {

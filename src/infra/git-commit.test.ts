...
async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `vikiclow-${label}-`));
}
...
describe("git commit resolution", () => {
  ...
  it("does not walk out of the vikiclow package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "vikiclow");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "vikiclow", version: "2026.3.9" }),
      "utf-8",
    );

...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__vikiclow_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "vikiclow-root");
...
describe("resolveVikiClowPackageRoot", () => {
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "vikiclow");
    const pkgRoot = path.join(project, "node_modules", "vikiclow");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "vikiclow" }));

    expect(resolveVikiClowPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "vikiclow");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "vikiclow.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "vikiclow" }));

    expect(resolveVikiClowPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveVikiClowPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveVikiClowPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveVikiClowPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-vikiclow package roots", async () => {
    const pkgRoot = fx("not-vikiclow");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-vikiclow" }));

    expect(resolveVikiClowPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveVikiClowPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveVikiClowPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});

...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __vikiclowDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__vikiclowDiagnosticEventsState) {
    globalStore.__vikiclowDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__vikiclowDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: VikiClowConfig): boolean {

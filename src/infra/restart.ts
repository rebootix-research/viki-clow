...
export function triggerVikiClowRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.VIKICLOW_SYSTEMD_UNIT,
      process.env.VIKICLOW_PROFILE,
    );
  ...
  const label =
    process.env.VIKICLOW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.VIKICLOW_PROFILE);

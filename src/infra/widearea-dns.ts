...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.VIKICLOW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*vikiclow-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "vikiclow";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "vikiclow");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "vikiclow-gw");
  ...
  records.push(`_vikiclow-gw._tcp IN PTR ${instanceLabel}._vikiclow-gw._tcp`);
  records.push(`${instanceLabel}._vikiclow-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._vikiclow-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; vikiclow-content-hash: ${contentHash}\n${contentBody}`;
}

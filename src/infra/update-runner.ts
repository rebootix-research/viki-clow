...
const DEFAULT_PACKAGE_NAME = "vikiclow";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["vikiclow", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "vikiclow.mjs");
    ...
    const doctorStep = await runStep(
      step("vikiclow doctor", doctorArgv, gitRoot, { VIKICLOW_UPDATE_IN_PROGRESS: "1" }),
    );

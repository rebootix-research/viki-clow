...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.VIKICLOW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "VikiClow";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(VikiClow\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.VIKICLOW_MDNS_HOSTNAME?.trim() ||
    process.env.VIKICLOWBOT_MDNS_HOSTNAME?.trim() ||
    "vikiclow";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")

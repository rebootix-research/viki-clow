...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.VIKICLOW_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for VikiClow. You can keep running the web gateway without it: \`${formatCliCommand("vikiclow gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();

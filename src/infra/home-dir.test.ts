...
describe("resolveEffectiveHomeDir", () => {
  it("prefers VIKICLOW_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(
      path.resolve("/srv/vikiclow-home"),
    );
  });
  ...
  it("expands VIKICLOW_HOME when set to ~", () => {
    const env = {
      VIKICLOW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for VIKICLOW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { VIKICLOW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when VIKICLOW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ VIKICLOW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { VIKICLOW_HOME: "/srv/vikiclow-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/vikiclow-home")}/x`);
  });

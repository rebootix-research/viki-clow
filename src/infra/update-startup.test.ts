...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["VIKICLOW_STATE_DIR", "NODE_ENV", "VITEST"]);
    ...
    if (!loaded) {
      ({ resolveVikiClowPackageRoot } = await import("./vikiclow-root.js"));
    ...
    vi.mocked(resolveVikiClowPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveVikiClowPackageRoot).mockResolvedValue("/opt/vikiclow");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/vikiclow/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });

...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipHealth) {
    ...
    try {
      ...
      await prompter.note(
        [
          "Docs:",
          "https://docs.vikiclow.ai/gateway/health",
          "https://docs.vikiclow.ai/gateway/troubleshooting",
        ].join("\n"),
        "Health check help",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.vikiclow.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.vikiclow/vikiclow.json (gateway.auth.token) or VIKICLOW_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("vikiclow config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("vikiclow doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("vikiclow dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control VikiClow."
            : "Copy/paste this URL in a browser on this machine to control VikiClow.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("vikiclow dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.vikiclow.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  const heartbeatEvery = nextConfig.agents?.defaults?.heartbeat?.every?.trim();
  await prompter.note(
    [
      "Autonomy defaults are ready.",
      `Heartbeat cadence: ${heartbeatEvery && heartbeatEvery.length > 0 ? heartbeatEvery : "disabled"}`,
      `Heartbeat file: ${path.join(resolveUserPath(options.workspaceDir), "HEARTBEAT.md")}`,
      "VikiClow stays quiet until HEARTBEAT.md contains real tasks.",
      "When you want proactive background work, turn that file into a tiny checklist.",
    ].join("\n"),
    "Autonomy",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.vikiclow.ai/security",
    "Security",
  );
    ...
    if (webSearchEnabled !== false && hasKey) {
      await prompter.note(
        [
          ...
          `Provider: ${label}`,
          ...(keySource ? [keySource] : []),
          "Docs: https://docs.vikiclow.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
    } else if (!hasKey) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("vikiclow configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.vikiclow.ai/tools/web"}`,
          "Docs: https://docs.vikiclow.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("vikiclow configure --section web")}`,
          "",
          "Docs: https://docs.vikiclow.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://vikiclow.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control VikiClow."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control VikiClow.",
  );

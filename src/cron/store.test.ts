...
const { makeStorePath } = createCronStoreHarness({ prefix: "vikiclow-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses VIKICLOW_HOME for tilde expansion", () => {
    vi.stubEnv("VIKICLOW_HOME", "/srv/vikiclow-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/vikiclow-home", "cron", "jobs.json"));
  });
});

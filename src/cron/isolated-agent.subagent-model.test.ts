...
async function writeSessionStore(home: string) {
  const dir = path.join(home, ".vikiclow", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<VikiClowConfig> = {},
): VikiClowConfig {
  const base: VikiClowConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<VikiClowConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<VikiClowConfig>,

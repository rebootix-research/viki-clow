...
describe("acp prompt cwd prefix", () => {
  ...
  it("redacts home directory in prompt prefix", async () => {
    const requestSpy = await runPromptWithCwd(path.join(os.homedir(), "vikiclow-test"));
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringMatching(/\[Working directory: ~[\\/]vikiclow-test\]/),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("keeps backslash separators when cwd uses them", async () => {
    const requestSpy = await runPromptWithCwd(`${os.homedir()}\\vikiclow-test`);
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        message: expect.stringContaining("[Working directory: ~\\vikiclow-test]"),
      }),
      { expectFinal: true },
    );
  });
  ...
  it("injects system provenance metadata when enabled", async () => {
    ...
    sessionStore.createSession({
      sessionId: "session-1",
      sessionKey: "agent:main:main",
      cwd: path.join(os.homedir(), "vikiclow-test"),
    });
  ...
  it("injects a system provenance receipt when requested", async () => {
    ...
    expect(requestSpy).toHaveBeenCalledWith(
      "chat.send",
      expect.objectContaining({
        systemProvenanceReceipt: expect.stringContaining("bridge=vikiclow-acp"),
      }),
      { expectFinal: true },
    );
    expect(requestSpy).toHaveBeenCalledWith(

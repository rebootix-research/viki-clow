...
const createTempDir = () => tempDirs.make("vikiclow-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets VIKICLOW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "vikiclow",
    });

    expect(env.VIKICLOW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("vikiclow");
  });

  it("overrides pre-existing VIKICLOW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      VIKICLOW_SHELL: "wrong",
    });
    expect(env.VIKICLOW_SHELL).toBe("acp-client");
  });
  ...
  it("preserves VIKICLOW_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        VIKICLOW_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["VIKICLOW_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default VikiClow bridge", () => {
    ...
    expect(env.VIKICLOW_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.VIKICLOW_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "vikiclow",
        serverArgs: ["acp"],
        defaultServerCommand: "vikiclow",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("VIKICLOW_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "vikiclow", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "vikiclow", "dist", "entry.js");
    const shimPath = path.join(dir, "vikiclow.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\vikiclow\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/vikiclow-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/vikiclow-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/vikiclow-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);

...
`;
    ...
    expect(parsed.vikiclow.emoji).toBe("💾");
    expect(parsed.vikiclow.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.vikiclow.emoji).toBe("📝");
    expect(parsed.vikiclow.events).toEqual(["command"]);
    expect(parsed.vikiclow.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.vikiclow.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"vikiclow": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"vikiclow": {"events": ["test"]}}');
  });
...
describe("resolveVikiClowMetadata", () => {
  it("extracts vikiclow metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveVikiClowMetadata(frontmatter);
  ...
  it("returns undefined when vikiclow key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        vikiclow: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with VikiClow" },
            { id: "npm", kind: "npm", package: "@vikiclow/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@vikiclow/hook");
  });
...
`;
    ...
    const vikiclow = resolveVikiClowMetadata(frontmatter);
    expect(vikiclow).toBeDefined();
    expect(vikiclow?.emoji).toBe("💾");
    expect(vikiclow?.events).toEqual(["command:new", "command:reset"]);
    expect(vikiclow?.requires?.config).toEqual(["workspace.dir"]);
    expect(vikiclow?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const vikiclow = resolveVikiClowMetadata(frontmatter);
    expect(vikiclow?.emoji).toBe("disk");
    expect(vikiclow?.events).toEqual(["command:new"]);
  });
});

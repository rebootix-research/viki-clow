...
homepage: https://docs.vikiclow.ai/automation/hooks#command-logger
metadata:
  {
    "vikiclow":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.vikiclow/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.vikiclow/logs/commands.log ~/.vikiclow/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/vikiclow`:
   ```
   /home/username/.vikiclow/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

...
This directory contains hooks that ship with VikiClow. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.vikiclow/vikiclow.json`:
...
2. Restart gateway: `pkill -9 -f 'vikiclow.*gateway' && pnpm vikiclow gateway`
3. Enable the hook: `vikiclow hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.vikiclow.ai/automation/hooks

...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "vikiclow-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop vikiclow!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@vikiclow_bot", { botUsername: "vikiclow_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@vikiclow_bot", { botUsername: "vikiclow_bot" })).toBe(true);

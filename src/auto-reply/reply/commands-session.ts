...
import { scheduleGatewaySigusr1Restart, triggerVikiClowRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting VikiClow in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerVikiClowRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting VikiClow via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

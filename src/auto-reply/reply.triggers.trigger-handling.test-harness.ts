...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    vikiclowHome: process.env.VIKICLOW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("VIKICLOW_HOME", snapshot.vikiclowHome);
  restoreKey("VIKICLOW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.VIKICLOW_STATE_DIR = join(home, ".vikiclow");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "vikiclow-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".vikiclow", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): VikiClowConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "vikiclow"),

...
export function isCommandEnabled(cfg: VikiClowConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: VikiClowConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as VikiClowConfig),
...
export function getCommandDetection(_cfg?: VikiClowConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: VikiClowConfig) {

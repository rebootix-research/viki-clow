...
let originalVikiClowLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.VIKICLOW_TEST_FILE_LOG;
  originalVikiClowLogLevel = process.env.VIKICLOW_LOG_LEVEL;
  delete process.env.VIKICLOW_TEST_FILE_LOG;
  delete process.env.VIKICLOW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.VIKICLOW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalVikiClowLogLevel === undefined) {
    delete process.env.VIKICLOW_LOG_LEVEL;
  } else {
    process.env.VIKICLOW_LOG_LEVEL = originalVikiClowLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
